function openHTMLEditor(e) {
    editorWin = window.open("htmledit.php?field=" + e, "_new", "width=770, height=500, scrollbars=no, resizable=yes"), editorWin.focus()
}
...
function switchRules(e) {
    return Set_Cookie("hideRules", 1 - e, 1e3), document.getElementById("rules").style.display = e ? "block" : "none", document.getElementById("norules").style.display = e ? "none" : "block", !1
}

function switchSide(e) {
    return Set_Cookie("hideSide", 1 - e, 1e3), e ? (document.body.classList.add("expanded"), document.body.classList.remove("collapsed")) : (document.body.classList.add("collapsed"), document.body.classList.remove("expanded")), !1
}
...
function beautifyTime(e) {
    return sec = pad(e % 60), minutes = pad(Math.floor(e / 60) % 60), s = minutes + ":" + sec, hours = Math.floor(e / 3600), hours > 0 && (s = hours + ":" + s), s
}
...
function timer() {
    ...
    try {
        document.answerForm.jstimerShow.value = beautifyTime(t), document.answerForm.jstimerShowPersonal.value = beautifyTime(Math.floor((Game.getTimer() + 500) / 1e3))
    } catch (e) { }
...
function setZoom(e) {
    ...
    if (t) {
        ...
        $("#buttonsBottom:visible").length && (i = s), $("#buttonsRight:visible").length && (a = s), $("#buttonsLeft:visible").length && (o = s), n && r && (t.style.transform = "scale(" + e + ")", t.style.marginLeft = Math.floor(1 + (n * e - n) / 2) + o * e + "px", t.style.marginRight = Math.floor(1 + (n * e - n) / 2) + a * e + "px", t.style.marginTop = Math.floor(1 + (r * e - r) / 2) + "px", t.style.marginBottom = Math.floor(1 + (r * e - r) / 2) + i * e + "px", Settings.set(Game.getSaveIdent() + ".saved-zoom", e), puzzleZoom = e)
    }
}

function zoomIn() {
    puzzleZoom < maxZoom && (puzzleZoom = 1.1 * puzzleZoom, setZoom(puzzleZoom))
}

function zoomOut() {
    puzzleZoom > minZoom && (puzzleZoom /= 1.1, setZoom(puzzleZoom))
}
...
function storeState(e) {
    stateIndex != -1 && states[stateIndex] == e || (stateIndex++, states[stateIndex] = e, states.splice(stateIndex + 1))
}
...
function redoMove() {
    states.length > stateIndex + 1 && (stateIndex++, setState(states[stateIndex]))
}

function switchNightMode(e) {
    return Set_Cookie("nightMode", 1 * e, 1e3), document.documentElement.className = e ? "nightmode" : "", !1
}

function switchASLMode(e) {
    return Set_Cookie("aslMode", 1 * e, 1e3), document.location.reload(), !1
}
...
function sumSolved(e) {
    var t = 0;
    for (var n in e) e.hasOwnProperty(n) && (t += e[n].nsolved);
...
function getPermalink4(e) {
    $.post("/permalink.php", {
        ...
        $("#shareLinkInput").val(t), $("#shareImgInput").val(n), $("#shareEmbedURLInput").val(i), $("#shareEmbedInput").val(o), $("#shareLinkFB").attr("href", "https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent(t) + "&quote=" + encodeURIComponent(r)), $("#shareLinkTW").attr("href", "https://twitter.com/intent/tweet?via=PuzzleTeamClub&text=" + encodeURIComponent(r) + " " + encodeURIComponent(t)), $("#shareContainer").show(), $("#shareContainerElements").show(), $("#shareContainerLoader").hide()
    })
}
...
function hideGlobalNotifications() {
    $(".global-alert-info").removeClass("show").find(".message").html(""), $(".global-alert-success").removeClass("show").find(".message").html(""), $(".global-alert-error").removeClass("show").find(".message").html("")
}
...
function getBrowserPrefix() {
    for (var e = 0; e < browserPrefixes.length; e++)
        if (getHiddenPropertyName(browserPrefixes[e]) in document) return browserPrefixes[e];
    return null
}

function onVisible() {
    isVisible || (isVisible = !0, "undefined" == typeof Game || Settings.get("global.hide-timer") || Game.resumeTimer())
}

function onHidden() {
    isVisible && (isVisible = !1, "undefined" == typeof Game || Settings.get("global.hide-timer") || Game.pauseTimer())
}
...
function puzzle(e) {
    this.maxColumnNumbers = 0, this.maxRowNumbers = 0, this.task = [], this.currentState = {
        cellStatus: [],
        taskStatus: [],
        lastMove: {},
        index: 0
        ...
        cells: []
    }, this.states = [], this.dom = {}, this.panTimeThreshold = e.panTimeThreshold ? e.panTimeThreshold : Config.panTimeThreshold, this.zoomingPanning = !1, this.solved = !1, this.drawing = !1, this.mouseDown = !1, this.dragMonitor = {
        decision: "unknown",
        timeStart: 0,
        touchstartX: 0,
        touchstartY: 0
    }, this.boardWidth = 0, this.boardHeight = 0, this.lastAppliedScale = 1, this.accumulated = 0, this.paused = 0, this.forcePaused = 0, this.lastTrackedTime = (new Date).getTime(), this.inputMode = "mouse", this.getBoardWidth = function () { }, this.getBoardHeight = function () { }, this.getTransform = function (e) {
        var t = /matrix\((.*)\)/,
        ...
        for (var n = [], r = 0, i = 0; i < e.length; i++)
            if ($.isNumeric(e[i])) {
                for (var o = ""; $.isNumeric(e[i]);) o += "" + e[i], i++;
                i--, t ? n[r] = this.toChar(o) : n[r] = parseInt(o), r++
            } else "_" != e[i] && (r += this.decodeChar(e[i]));
        return n
    ...
    }, this.parseTask = function () { }, this.setCellState = function (e, t) {
        ...
        this.currentState.lastMove = Util.clone(this.currentMove), this.storeCurrentState()
    }, this.resetCurrentMoveDraw = function () {
        for (var e = this.currentMove.cells.length, t = 0; t < e; t++) {
            var n = this.currentMove.cells[t];
            this.drawCellStatus(n, this.getCurrentStatus(n))
        }
    }, this.drawMoveHelpers = function () { }, this.hideMoveHelpers = function () {
        ...
        this.resetCurrentMoveDraw(), this.initCurrentMove(), this.hideMoveHelpers()
        ...
        return "rotate" == Settings.get(this.slug + ".draw-style") ? (t ? n-- : n++, (n + 3) % 3) : "white" == Settings.get(this.slug + ".draw-style") ? 0 : "black" == Settings.get(this.slug + ".draw-style") ? t ? 2 == n ? 0 : 2 : 1 == n ? 0 : 1 : "cross" == Settings.get(this.slug + ".draw-style") ? t ? 1 == n ? 0 : 1 : 2 == n ? 0 : 2 : n
    }, this.helpers = function () { }, this.startMove = function (e, t, n) {
        this.initCurrentMove(), this.drawing = !0, this.currentMove.invert = "undefined" != typeof t && t, this.currentMove.cellStatus = this.getNextStatus(e, t), this.currentMove.startPoint = {
            row: e.row,
            col: e.col
    ...
    }, this.performMove = function (e) { }, this.onEndMove = function () { }, this.endMove = function () {
        this.zoomingPanning || (this.applyCurrentMoveToState(), this.drawCurrentMove(), this.onEndMove(), this.drawHelperHighlightMove(this.currentMove), this.helpers(), this.hideMoveHelpers(), this.checkFinished()), this.drawing = !1
    }, this.getNumberOfDrawnCells = function () {
        return "implement me"
    }, this.showHelperNumberOfDrawnCells = function (t, n, r, i, o) {
        i || (i = this.dom.helperNumberOfDrawnCells.width()), o || (o = this.dom.helperNumberOfDrawnCells.height());
        var a = document.documentElement.clientWidth / window.innerWidth;
        if (a < 1 && (a = 1), e.relativeTo) {
            var s = $(e.relativeTo).offset();
            t += Layout.scrollLeft() - s.left, n -= s.top
        }
        var c = n + Layout.scrollTop() - 100 / a - o / a + (Layout.hiddenHeader() ? Config.headerHeight / a : 0),
            u = 0,
            d = 0;
        if (l < 50 && (u = 17), e.relativeTo && (u = -s.top, d = -s.left), Settings.get("global.left-handed")) {
            var l = t + i / a,
                f = Layout.clientWidth();
            Layout.portrait || (f -= Config.sidebarWidth), e.relativeTo || l + i + 5 > f && (l = f - i - 5)
        } else {
            var l = t - i / a - 15 / a;
            l < d && (l = d)
        }
        c < Layout.scrollTop() + u && (c = Layout.scrollTop() + u), this.dom.helperNumberOfDrawnCells.show().css({
            top: c + "px",
            left: l + "px"
        }).html(r), a != this.lastAppliedScale && (1 != a ? this.dom.helperNumberOfDrawnCells.css({
            transform: "scale(" + 1 / a + ")"
        ...
        if (Settings.get(this.slug + ".highlight-change") && ($(".cell-active").removeClass("cell-active"), e.cells))
            for (var t = 0; t < e.cells.length; t++) this.highlightCell(e.cells[t], "cell-active")
    }, this.initCrosshairHelper = function () {
        0 == $(".helper-crosshair").length && (this.dom.helperCrosshair = $('<div class="helper-crosshair"><i class="icon icon-cursor"></i><i class="icon icon-move"></i><div class="crosshair-btn"></div></div>'), $("#scene").append(this.dom.helperCrosshair)), Settings.get(this.slug + ".show-crosshair") ? this.dom.helperCrosshair.show() : this.dom.helperCrosshair.hide()
    }, this.drawCellStatus = function (e, t) { }, this.drawCurrentState = function () { }, this.draw = function () { }, this.getFitMatrix = function (t) {
        var n = 1;
        ...
        var o = this.boardWidth,
            ...
            s = o * n,
            c = a * n;
        t || ((s > r.width || c > i) && (n = Math.min(r.width / o, i / a)), n < e.minScale && (n = e.minScale));
        var u = (r.width - this.boardWidth) / 2,
            d = (n - 1) * this.boardHeight / 2 + (Config.headerHeight - Config.paddingTop);
        return d += 25, d += Layout.scrollTop(), "matrix(" + n + ", 0, 0, " + n + ", " + u + ", " + d + ")"
    }, this.fitCrosshair = function () {
        matrix = "matrix(1, 0, 0, 1, " + (Layout.clientWidth() - Config.crosshairWidth) / 2 + ", 100 )", this.dom.helperCrosshair.panzoom("option", {
        ...
        this.panzoom("option", {
            startTransform: t
        }), this.panzoom("reset"), this.panzoomAction(), this.fitCrosshair()
    }, this.repositionHelpers = function () { }, this.getCrosshairTarget = function () {
        ...
        return r.length && r.hasClass("selectable") || (r = this.findNearestCell(t, n, "selectable")), r.chLeft = t, r.chTop = n, r
    }, this.highlightCrosshairCell = function (e) {
        $(".cell-crosshair").removeClass("cell-crosshair"), e.length && e.addClass("cell-crosshair")
    }, this.eventsCrosshairMove = function (e) { }, this.crosshairPan = function () {
        var e = this.getCrosshairTarget();
        this.highlightCrosshairCell(e), e.length && this.eventsCrosshairMove(e)
    }, this.stateChangedBoard = function () { }, this.stateChangedTask = function () { }, this.stateChanged = function () {
        return this.currentState.index == -1 || (!!this.stateChangedBoard() || !!this.stateChangedTask())
    }, this.initState = function () { }, this.resetState = function () {
        this.initState(), this.redraw()
    }, this.loadState = function (e) { }, this.setState = function (e) {
        this.loadState({
            board: e
        }), this.drawCurrentState(), this.check()
    }, this.restoreCurrentState = function () {
        this.currentState = Util.clone(this.states[this.currentState.index]), this.drawCurrentState()
    }, this.storeCurrentState = function () {
        this.stateChanged() && (this.currentState.index++, this.states[this.currentState.index] = Util.clone(this.currentState), this.states.splice(this.currentState.index + 1), this.saveState())
    }, this.undo = function () {
        this.currentState.index && (this.currentState = Util.clone(this.states[this.currentState.index - 1]), this.cancelMove(), this.drawCurrentState(), this.saveState(!0))
    }, this.redo = function () {
        this.states.length > this.currentState.index + 1 && (this.currentState = Util.clone(this.states[this.currentState.index + 1]), this.cancelMove(), this.drawCurrentState(), this.saveState(!0))
    }, this.panzoomAction = function () {
        this.zoomingPanning = !0, Settings.set(this.slug + ".saved-scale", this.getScale()), this.cancelMove(), this.repositionHelpers()
        ...
        for (var o = !1, a = -i; a <= i; a += i)
            for (var s = -i; s <= i; s += i)
                if (target = $(document.elementFromPoint(t + a, n + s)), target.hasClass(r)) {
                    if (a && s && !o) {
                        o = target;
                        continue
                    }
        ...
        if ("touchstart" == e.type || "touchmove" == e.type) var n = e.target.getBoundingClientRect(),
            r = this.getScale(),
            i = (t.pageX - n.left - Layout.scrollLeft()) / r,
            o = (t.pageY - n.top - Layout.scrollTop()) / r;
        else var i = t.offsetX,
            o = t.offsetY;
        return {
            offsetX: i,
            offsetY: o
        }
    }, this.handleTouchStart = function (e) { }, this.eventsTouchStart = function (e) {
        if ("touchstart" == e.type) {
            var t = e.originalEvent.changedTouches[0],
                n = e.touches || e.originalEvent.touches;
            if (n.length > 1 || Settings.get($this.slug + ".show-crosshair")) $this.dragMonitor.decision = "panzoom", $this.cancelMove();
            else {
                if ("draw" == Settings.get($this.slug + ".touch-mode")) $this.dragMonitor.decision = "draw";
                else {
                    if ("move" == Settings.get($this.slug + ".touch-mode")) return $this.dragMonitor.decision = "panzoom", !1;
                    $this.dragMonitor.timeStart = (new Date).getTime(), $this.dragMonitor.touchstartX = t.clientX, $this.dragMonitor.touchstartY = t.clientY
                }
    ...
    }, this.eventsTouchMove = function (e) { }, this.eventsTouchEnd = function (e) { }, this.eventsClick = function (e) { }, this.eventsMove = function (e) { }, this.onMouseWheel = function (e) {
        if (e.originalEvent || (e.originalEvent = e), $("body").hasClass("hide-mainmenu") && $("body").hasClass("hide-puzzlemenu")) {
            e.preventDefault();
        ...
        $(window).on("orientationchange resize", function (e) {
            $this.fit(!0), $this.panzoomAction()
        }).on("scroll", function () {
            $this.panzoomAction()
        }).on("DOMMouseScroll MozMousePixelScroll", this.onMouseWheel), document.addEventListener("mousewheel", this.onMouseWheel, {
            passive: !1
        }), $this.fit(), $this.panzoomAction(), $this.on("mousedown touchstart", function (t) {
            e.readonly || ($this.mouseDown = !0, $this.zoomingPanning = !1, $this.eventsTouchStart(t))
        }), $this.on("dragstart", function (e) {
            e.preventDefault()
        }), $this.attr("unselectable", "on").on("selectstart", !1), $(document).on("mouseup touchend", function (e) {
            $this.eventsClick(e)
        }), $(document).on("mouseup touchend", function (e) {
            $this.mouseDown && (e.preventDefault(), "panzoom" != $this.dragMonitor.decision && "markTask" != $this.dragMonitor.decision && $this.endMove(), $this.mouseDown = !1, $this.dragMonitor = {
                decision: "unknown",
                timeStart: 0,
                touchstartX: 0,
                touchstartY: 0
            ...
            if ($this.mouseDown) {
                var t = $this.getChangedTouch(e);
                if ("touchmove" == e.type) var n = e.touches || e.originalEvent.touches;
                else var n = [t];
                ...
                "panzoom" != $this.dragMonitor.decision && e.stopPropagation(), "draw" == $this.dragMonitor.decision && (e.preventDefault(), $this.eventsTouchMove(e))
            } else $this.eventsMove(e)
            ...
            if (!e.readonly) {
                ...
                n.length && ($this.mouseDown = !0, $this.zoomingPanning = !1, $this.eventsCrosshairMouseDown(t, n))
            }
        }).on("mouseup touchend", function (e) {
            e.preventDefault(), $this.endMove(), $this.mouseDown = !1, $this.dragMonitor = {
                decision: "unknown",
                timeStart: 0,
                touchstartX: 0,
                touchstartY: 0
        ...
        e.readonly || this.paused || (this.tickTimer(), this.paused = 1, t && (this.forcePaused = 1), Settings.get("global.non-competitive-timer") && $("body").addClass("paused"))
    }, this.resumeTimer = function (t) {
        e.readonly || this.paused && (this.forcePaused && !t || (this.lastTrackedTime = (new Date).getTime(), this.forcePaused = 0, this.paused = 0, $("body").removeClass("paused")))
    }, this.resetTimer = function () {
        e.readonly || (this.accumulated = 0, this.lastTrackedTime = (new Date).getTime(), this.storeTimer())
    }, this.loadGame = function (t) {
        e = $.extend(e, t), this.parseTask(), this.initState(), this.initCurrentMove(), e.state && this.loadState(e.state), e.savedState && (this.currentState = e.savedState, this.currentState.index = -1, this.storeCurrentState()), this.saveState(), this.redraw(), e.onLoaded && e.onLoaded(this, t)
    }, this.loadNew = function () {
        $.ajax(e.baseUrl + "/get").done(function (t) {
            e.savedState = !1, $this.loadGame(t), $this.initState(), $this.saveState(), $this.redraw(), e.onLoadNew && e.onLoadNew()
        })
    }, this.serializeState = function () { }, this.serializeSolution = function () { }, this.getBaseSaveIdent = function () {
        return this.slug + ".save." + e.ident
    }, this.getSaveIdent = function () {
        var t = "";
        return e.special && (t = "." + e.specialDate), this.getBaseSaveIdent() + t
        ...
        if (this.stateModified() || !e.relativeTo || "undefeind" != typeof t)
            if (this.solved) Settings.remove(this.getSaveIdent());
            else {
    ...
    }, this.getMarkedCount = function () { }, this.checkFinished = function () {
        ...
        localStorage.setItem("timer-" + this.getSaveIdent(), 1 * localStorage.getItem("timer-" + this.getBaseSaveIdent())), localStorage.removeItem("timer-" + this.getBaseSaveIdent()), Settings.set(this.getSaveIdent() + ".saved-zoom", Settings.get(this.getBaseSaveIdent() + ".saved-zoom")), Settings.remove(this.getBaseSaveIdent() + ".saved-zoom"), Settings.set(this.getSaveIdent(), Settings.get(this.getBaseSaveIdent())), Settings.remove(this.getBaseSaveIdent())
    }, this.escapeRegExp = function (e) {
        return e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
    }, this.initGame = function () {
        $this = this, this.parseTask(), this.initState(), this.draw();
        ...
        try {
            if (e.special && !this.specialsHandled) {
                ...
                for (k in Settings.bag) (found = k.match(n)) && new Date(found[1]) < new Date(e.lastValidSpecial) && (Settings.remove(k), Settings.remove(k + ".saved-zoom"), localStorage.removeItem("timer-" + k));
...
function showXPTeaser(e) {
    $("#effect-xp").text("+" + e), $("#effect-xp").removeClass("animate"), setTimeout(function () {
        $("#effect-xp").addClass("animate")
    }, 10)
}

function checkLogin() {
    return !!User.logged || ($.featherlight("/partials/form.login.php", {}), !1)
}
...
function initAdserver() {
    ...
    }), pbjs.initAdserverSet = !0, __tcfapi("getTCData", 2, function (e, t) {
        if (globalTCData = e, !e.gdprApplies || e.purpose.consents[1] && e.vendor.consents[755]) window.googletag.pubads().getSlots().forEach(function (e) {
        ...
        pbjs.requestBids({
            ...
            bidsBackHandler: function () {
                if (__tcfapi("getTCData", 2, function (e, t) {
                    globalTCData = e
                }), !globalTCData.gdprApplies || globalTCData.purpose.consents[1] && globalTCData.vendor.consents[755]) pbjs.setTargetingForGPTAsync([e.getSlotElementId()]), googletag.pubads().refresh([e]);
                else {
...
var RightClick = 1;
navigator.userAgent.match(/Opera/gi) && (RightClick = 0), navigator.userAgent.match(/Safari/gi) && (RightClick = 0), navigator.userAgent.match(/MSIE/gi) || navigator.userAgent.match(/Mozilla/gi) || (RightClick = 0), navigator.userAgent.match(/Chrome/gi) && (RightClick = 1), (navigator.userAgent.match(/iPhone/gi) || navigator.userAgent.match(/iPad/gi) || navigator.userAgent.match(/Android/gi)) && (RightClick = 0), setTimeout(function () {
    var e = document.getElementById("bannerTop");
    e.parentNode.removeChild(e);
    var e = document.getElementById("bannerSide");
    e && e.parentNode.removeChild(e), document.getElementById("bannerTopSpacer").style.display = "block"
}, 6e5);
var minZoom = .2,
    maxZoom = 5,
...
! function (e, t) {
    ...
    function u(e, t, n, r) {
        var i, o = 1,
            ...
            d = (oe.cssNumber[t] || "px" !== u && +c) && ke.exec(oe.css(e, t));
        if (d && d[3] !== u) {
            u = u || d[3], n = n || [], d = +c || 1;
            do o = o || ".5", d /= o, oe.style(e, t, d + u); while (o !== (o = s() / c) && 1 !== o && --a)
        }
        return n && (d = +d || +c || 0, i = n[1] ? d + (n[1] + 1) * n[2] : +n[2], r && (r.unit = u, r.start = d, r.end = i)), i
    }
        ...
        for (var o, a, s, c, u, f, p = t.createDocumentFragment(), h = [], g = 0, m = e.length; m > g; g++)
            if (o = e[g], o || 0 === o)
                if ("object" === oe.type(o)) oe.merge(h, o.nodeType ? [o] : o);
                    ...
                    for (a = a || p.appendChild(t.createElement("div")), s = (Re.exec(o) || ["", ""])[1].toLowerCase(), c = Pe[s] || Pe._default, a.innerHTML = c[1] + oe.htmlPrefilter(o) + c[2], f = c[0]; f--;) a = a.lastChild;
                    oe.merge(h, a.childNodes), a = p.firstChild, a.textContent = ""
                } else h.push(t.createTextNode(o));
        for (p.textContent = "", g = 0; o = h[g++];)
            if (r && oe.inArray(o, r) > -1) i && i.push(o);
            else if (u = oe.contains(o.ownerDocument, o), a = d(p.appendChild(o), "script"), u && l(a), n)
                for (f = 0; o = a[f++];) Ue.test(o.type || "") && n.push(o);
        return p
    }
    ...
    function S(e, t) {
        ...
        if (1 === t.nodeType) {
            if (Ee.hasData(e) && (o = Ee.access(e), a = Ee.set(t, o), u = o.events)) {
                ...
                for (i in u)
                    for (n = 0, r = u[i].length; r > n; n++) oe.event.add(t, i, u[i][n])
            }
            Ie.hasData(e) && (s = Ie.access(e), c = oe.extend({}, s), Ie.set(t, c))
        }
    }
    ...
    function C(e, t, n, r) {
        ...
        var i, o, a, s, c, u, l = 0,
            ...
            g = t[0],
            m = oe.isFunction(g);
        if (m || p > 1 && "string" == typeof g && !re.checkClone && Fe.test(g)) return e.each(function (i) {
            var o = e.eq(i);
            m && (t[0] = g.call(this, i, o.html())), C(o, t, n, r)
        });
        if (p && (i = f(t, e[0].ownerDocument, !1, e, r), o = i.firstChild, 1 === i.childNodes.length && (i = o), o || r)) {
            for (a = oe.map(d(i, "script"), b), s = a.length; p > l; l++) c = i, l !== h && (c = oe.clone(c, !0, !0),
                s && oe.merge(a, d(c, "script"))), n.call(e[l], c, l);
            if (s)
                for (u = a[a.length - 1].ownerDocument, oe.map(a, y), l = 0; s > l; l++) c = a[l], Ue.test(c.type || "") && !Ee.access(c, "globalEval") && oe.contains(u, c) && (c.src ? oe._evalUrl && oe._evalUrl(c.src) : oe.globalEval(c.textContent.replace(We, "")))
        }
        return e
    }

    function T(e, t, n) {
        for (var r, i = t ? oe.filter(t, e) : e, o = 0; null != (r = i[o]); o++) n || 1 !== r.nodeType || oe.cleanData(d(r)), r.parentNode && (n && oe.contains(r.ownerDocument, r) && l(d(r, "script")), r.parentNode.removeChild(r));
        return e
    }
    ...
    function E(e) {
        var t = Y,
            n = Ve[e];
        return n || (n = x(e, t), "none" !== n && n || (Ge = (Ge || oe("<iframe frameborder='0' width='0' height='0'/>")).appendTo(t.documentElement), t = Ge[0].contentDocument, t.write(), t.close(), n = x(e, t), Ge.detach()), Ve[e] = n), n
    }

    function I(e, t, n) {
        var r, i, o, a, s = e.style;
        return n = n || Ke(e), a = n ? n.getPropertyValue(t) || n[t] : void 0, "" !== a && void 0 !== a || oe.contains(e.ownerDocument, e) || (a = oe.style(e, t)), n && !re.pixelMarginRight() && Ye.test(a) && Je.test(t) && (r = s.width, i = s.minWidth, o = s.maxWidth, s.minWidth = s.maxWidth = s.width = a, a = n.width, s.width = r, s.minWidth = i, s.maxWidth = o), void 0 !== a ? a + "" : a
    }
    ...
    function k(e, t, n, r, i) {
        for (var o = n === (r ? "border" : "content") ? 4 : "width" === t ? 1 : 0, a = 0; 4 > o; o += 2) "margin" === n && (a += oe.css(e, n + _e[o], !0, i)), r ? ("content" === n && (a -= oe.css(e, "padding" + _e[o], !0, i)), "margin" !== n && (a -= oe.css(e, "border" + _e[o] + "Width", !0, i))) : (a += oe.css(e, "padding" + _e[o], !0, i), "padding" !== n && (a += oe.css(e, "border" + _e[o] + "Width", !0, i)));
        return a
    }
    ...
    function _(e, t, n) {
        ...
        if (0 >= i || null == i) {
            if (i = I(e, t, o), (0 > i || null == i) && (i = e.style[t]), Ye.test(i)) return i;
    ...
    function D(e, t) {
        for (var n, r, i, o = [], a = 0, s = e.length; s > a; a++) r = e[a], r.style && (o[a] = Ee.get(r, "olddisplay"), n = r.style.display, t ? (o[a] || "none" !== n || (r.style.display = ""), "" === r.style.display && De(r) && (o[a] = Ee.access(r, "olddisplay", E(r.nodeName)))) : (i = De(r), "none" === n && i || Ee.set(r, "olddisplay", i ? n : oe.css(r, "display"))));
    ...
    function B(e, t, n) {
        ...
        })), 1 === e.nodeType && ("height" in t || "width" in t) && (n.overflow = [p.overflow, p.overflowX, p.overflowY], u = oe.css(e, "display"), d = "none" === u ? Ee.get(e, "olddisplay") || E(e.nodeName) : u, "inline" === d && "none" === oe.css(e, "float") && (p.display = "inline-block")), n.overflow && (p.overflow = "hidden", l.always(function () {
        ...
        if (oe.isEmptyObject(f)) "inline" === ("none" === u ? E(e.nodeName) : u) && (p.display = u);
            ...
            g ? "hidden" in g && (h = g.hidden) : g = Ee.access(e, "fxshow", {}), o && (g.hidden = !h), h ? oe(e).show() : l.done(function () {
                oe(e).hide()
            }), l.done(function () {
                var t;
                Ee.remove(e, "fxshow");
            ...
            for (r in f) a = P(h ? g[r] : 0, r, l), r in g || (g[r] = a.start, h && (a.end = a.start, a.start = "width" === r || "height" === r ? 1 : 0))
        }
    }
    ...
    function q(e, t, n) {
        var r, i, o = 0,
            ...
            c = function () {
                if (i) return !1;
                for (var t = it || R(), n = Math.max(0, u.startTime + u.duration - t), r = n / u.duration || 0, o = 1 - r, a = 0, c = u.tweens.length; c > a; a++) u.tweens[a].run(o);
        ...
        for (M(d, u.opts.specialEasing); a > o; o++)
            if (r = q.prefilters[o].call(u, e, d, u.opts)) return oe.isFunction(r.stop) && (oe._queueHooks(u.elem, u.opts.queue).stop = oe.proxy(r.stop, r)), r;
        return oe.map(d, P, u), oe.isFunction(u.opts.start) && u.opts.start.call(e, u), oe.fx.timer(oe.extend(c, {
            elem: e,
            anim: u,
            queue: u.opts.queue
    ...
    function W(e, t, n, r) {
        ...
        for (o = d.shift(); o;)
            if (e.responseFields[o] && (n[e.responseFields[o]] = t), !c && r && e.dataFilter && (t = e.dataFilter(t, e.dataType)), c = o, o = d.shift())
                if ("*" === o) o = c;
                else if ("*" !== c && c !== o) {
                    if (a = u[c + " " + o] || u["* " + o], !a)
                        for (i in u)
    ...
    oe.fn = oe.prototype = {
        ...
        var e, t, n, r, i, o, a = arguments[0] || {},
            ...
            c = arguments.length,
            u = !1;
        for ("boolean" == typeof a && (u = a, a = arguments[s] || {}, s++), "object" == typeof a || oe.isFunction(a) || (a = {}), s === c && (a = this, s--); c > s; s++)
            if (null != (e = arguments[s]))
                for (t in e) n = a[t], r = e[t], a !== r && (u && r && (oe.isPlainObject(r) || (i = oe.isArray(r))) ? (i ? (i = !1, o = n && oe.isArray(n) ? n : []) : o = n && oe.isPlainObject(n) ? n : {}, a[t] = oe.extend(u, o, r)) : void 0 !== r && (a[t] = r));
        return a
    }, oe.extend({
    ...
    var de = function (e) {
        function t(e, t, n, r) {
            var i, o, a, s, c, u, l, p, h = t && t.ownerDocument,
                g = t ? t.nodeType : 9;
            if (n = n || [], "string" != typeof e || !e || 1 !== g && 9 !== g && 11 !== g) return n;
            if (!r && ((t ? t.ownerDocument || t : z) !== D && _(t), t = t || D, R)) {
                if (11 !== g && (u = ve.exec(e)))
                    if (i = u[1]) {
                        if (9 === g) {
                    ...
                    } if (w.qsa && !W[e + " "] && (!U || !U.test(e))) {
                        if (1 !== g) h = t, p = e;
                        else if ("object" !== t.nodeName.toLowerCase()) {
                            for ((s = t.getAttribute("id")) ? s = s.replace(ye, "\\$&") : t.setAttribute("id", s = q), l = E(e), o = l.length, c = fe.test(s) ? "#" + s : "[id='" + s + "']"; o--;) l[o] = c + " " + f(l[o]);
                            p = l.join(","), h = be.test(e) && d(t.parentNode) || t
                        }
                        if (p) try {
        ...
        function p(e, t, n) {
            ...
            return t.first ? function (t, n, o) {
                ...
                if (a) {
                    ...
                    for (; t = t[r];)
                        if (1 === t.nodeType || i) {
                            if (u = t[q] || (t[q] = {}), c = u[t.uniqueID] || (u[t.uniqueID] = {}), (s = c[r]) && s[0] === L && s[1] === o) return d[2] = s[2];
                            if (c[r] = d, d[2] = e(t, n, a)) return !0
                        }
            }
        }
        ...
        function v(e, t, n, i, o, a) {
            return i && !i[q] && (i = v(i)), o && !o[q] && (o = v(o, a)), r(function (r, a, s, c) {
                var u, d, l, f = [],
                    p = [],
                    h = a.length,
                    v = r || g(t || "*", s.nodeType ? [s] : s, []),
                    b = !e || !r && t ? v : m(v, f, e, s, c),
                    y = n ? o || (r ? e : h || i) ? [] : a : b;
                if (n && n(b, y, s, c), i)
                    for (u = m(y, p), i(u, [], s, c), d = u.length; d--;)(l = u[d]) && (y[p[d]] = !(b[p[d]] = l));
                if (r) {
                    if (o || e) {
                        if (o) {
                            for (u = [], d = y.length; d--;)(l = y[d]) && u.push(b[d] = l);
                            o(null, y = [], u, c)
                        }
                        for (d = y.length; d--;)(l = y[d]) && (u = o ? ee(r, l) : f[d]) > -1 && (r[u] = !(a[u] = l))
                    }
                } else y = m(y === a ? y.splice(h, y.length) : y), o ? o(null, a, y, c) : Q.apply(a, y)
            })
        }
            ...
            var i = n.length > 0,
                ...
                a = function (r, a, s, c, u) {
                    var d, l, f, p = 0,
                        ...
                        b = O,
                        y = r || o && C.find.TAG("*", u),
                        S = L += null == b ? 1 : Math.random() || .1,
                        w = y.length;
                    for (u && (O = a === D || a || u); h !== w && null != (d = y[h]); h++) {
                        if (o && d) {
                            for (l = 0, a || d.ownerDocument === D || (_(d), s = !R); f = e[l++];)
                                if (f(d, a || D, s)) {
                                    c.push(d);
                                    break
                                } u && (L = S)
                        }
                        i && ((d = !f && d) && p--, r && g.push(d))
                    }
                    if (p += h, i && h !== p) {
                        for (l = 0; f = n[l++];) f(g, v, a, s);
                        if (r) {
                            if (p > 0)
                                for (; h--;) g[h] || v[h] || (v[h] = K.call(c));
                            v = m(v)
                        }
                        Q.apply(c, v), u && !r && v.length > 0 && p + n.length > 1 && t.uniqueSort(c)
                    }
                    return u && (L = S, O = b), g
                };
            return i ? r(a) : a
        }
        var S, w, C, T, x, E, I, A, O, j, k, _, D, N, R, U, P, B, M, q = "sizzle" + 1 * new Date,
            z = e.document,
            L = 0,
            $ = 0,
            F = n(),
        ...
        w = t.support = {}, x = t.isXML = function (e) {
            ...
            return r !== D && 9 === r.nodeType && r.documentElement ? (D = r, N = D.documentElement, R = !x(D), (n = D.defaultView) && n.top !== n && (n.addEventListener ? n.addEventListener("unload", Ce, !1) : n.attachEvent && n.attachEvent("onunload", Ce)), w.attributes = i(function (e) {
            ...
            }, P = [], U = [], (w.qsa = me.test(D.querySelectorAll)) && (i(function (e) {
                N.appendChild(e).innerHTML = "<a id='" + q + "'></a><select id='" + q + "-\r\\' msallowcapture=''><option selected=''></option></select>", e.querySelectorAll("[msallowcapture^='']").length && U.push("[*^$]=" + ne + "*(?:''|\"\")"), e.querySelectorAll("[selected]").length || U.push("\\[" + ne + "*(?:value|" + te + ")"), e.querySelectorAll("[id~=" + q + "-]").length || U.push("~="), e.querySelectorAll(":checked").length || U.push(":checked"), e.querySelectorAll("a#" + q + "+*").length || U.push(".#.+[+~]")
            }), i(function (e) {
                ...
                w.disconnectedMatch = B.call(e, "div"), B.call(e, "[s!='']:x"), P.push("!=", oe)
            }), U = U.length && new RegExp(U.join("|")), P = P.length && new RegExp(P.join("|")), t = me.test(N.compareDocumentPosition), M = t || me.test(N.contains) ? function (e, t) {
                ...
                return n ? n : (n = (e.ownerDocument || e) === (t.ownerDocument || t) ? e.compareDocumentPosition(t) : 1, 1 & n || !w.sortDetached && t.compareDocumentPosition(e) === n ? e === D || e.ownerDocument === z && M(z, e) ? -1 : t === D || t.ownerDocument === z && M(z, t) ? 1 : j ? ee(j, e) - ee(j, t) : 0 : 4 & n ? -1 : 1)
            } : function (e, t) {
                if (e === t) return k = !0, 0;
                var n, r = 0,
                    i = e.parentNode,
                    ...
                    s = [e],
                    c = [t];
                if (!i || !o) return e === D ? -1 : t === D ? 1 : i ? -1 : o ? 1 : j ? ee(j, e) - ee(j, t) : 0;
                if (i === o) return a(e, t);
            ...
            if ((e.ownerDocument || e) !== D && _(e), n = n.replace(de, "='$1']"), w.matchesSelector && R && !W[n + " "] && (!P || !P.test(n)) && (!U || !U.test(n))) try {
                var r = B.call(e, n);
                if (r || w.disconnectedMatch || e.document && 11 !== e.document.nodeType) return r
            } catch (e) { }
            return t(n, D, null, [e]).length > 0
        }, t.contains = function (e, t) {
            return (e.ownerDocument || e) !== D && _(e), M(e, t)
        }, t.attr = function (e, t) {
            ...
            var t, n = [],
                r = 0,
                i = 0;
            if (k = !w.detectDuplicates, j = !w.sortStable && e.slice(0), e.sort(G), k) {
            ...
            preFilter: {
                ...
                PSEUDO: function (e) {
                    var t, n = !e[6] && e[2];
                    return pe.CHILD.test(e[0]) ? null : (e[3] ? e[2] = e[4] || e[5] || "" : n && le.test(n) && (t = E(n, !0)) && (t = n.indexOf(")", n.length - t) - n.length) && (e[0] = e[0].slice(0, t), e[2] = n.slice(0, t)), e.slice(0, 3))
                }
            },
            filter: {
                TAG: function (e) {
                ...
                CHILD: function (e, t, n, r, i) {
                    ...
                    return 1 === r && 0 === i ? function (e) {
                        ...
                        var u, d, l, f, p, h, g = o !== a ? "nextSibling" : "previousSibling",
                            m = t.parentNode,
                        ...
                        if (m) {
                            ...
                            if (h = [a ? m.firstChild : m.lastChild], a && b) {
                                for (f = m, l = f[q] || (f[q] = {}), d = l[f.uniqueID] || (l[f.uniqueID] = {}), u = d[e] || [], p = u[0] === L && u[1], y = p && u[2], f = p && m.childNodes[p]; f = ++p && f && f[g] || (y = p = 0) || h.pop();)
                                    if (1 === f.nodeType && ++y && f === t) {
                                        d[e] = [L, p, y];
                                        break
                                    }
                            } else if (b && (f = t, l = f[q] || (f[q] = {}), d = l[f.uniqueID] || (l[f.uniqueID] = {}), u = d[e] || [], p = u[0] === L && u[1], y = p), y === !1)
                                for (;
                                    (f = ++p && f && f[g] || (y = p = 0) || h.pop()) && ((s ? f.nodeName.toLowerCase() !== v : 1 !== f.nodeType) || !++y || (b && (l = f[q] || (f[q] = {}), d = l[f.uniqueID] || (l[f.uniqueID] = {}), d[e] = [L, y]), f !== t)););
        ...
        return l.prototype = C.filters = C.pseudos, C.setFilters = new l, E = t.tokenize = function (e, n) {
            ...
            var n, r = [],
                ...
                o = W[e + " "];
            if (!o) {
                for (t || (t = E(e)), n = t.length; n--;) o = b(t[n]), o[q] ? r.push(o) : i.push(o);
                o = W(e, y(i, r)), o.selector = e
            }
            return o
        }, A = t.select = function (e, t, n, r) {
            var i, o, a, s, c, u = "function" == typeof e && e,
                l = !r && E(e = u.selector || e);
            if (n = n || [], 1 === l.length) {
                if (o = l[0] = l[0].slice(0), o.length > 2 && "ID" === (a = o[0]).type && w.getById && 9 === t.nodeType && R && C.relative[o[1].type]) {
                    if (t = (C.find.ID(a.matches[0].replace(Se, we), t) || [])[0], !t) return n;
            ...
            return (u || I(e, l))(r, t, !R, n, !t || be.test(e) && d(t.parentNode) || t), n
        }, w.sortStable = q.split("").sort(G).join("") === q, w.detectDuplicates = !!k, _(), w.sortDetached = i(function (e) {
    ...
    oe.fn.extend({
        ...
        oe.fn[e] = function (n, r) {
            ...
            return "Until" !== e.slice(-5) && (r = n), r && "string" == typeof r && (i = oe.filter(r, i)), this.length > 1 && (Se[e] || oe.uniqueSort(i), ye.test(e) && i.reverse()), this.pushStack(i)
        }
    });
    var we = /\S+/g;
    oe.Callbacks = function (e) {
        ...
        var t, n, r, i, a = [],
            ...
            u = function () {
                for (i = e.once, r = t = !0; s.length; c = -1)
                    for (n = s.shift(); ++c < a.length;) a[c].apply(n[0], n[1]) === !1 && e.stopOnFalse && (c = a.length, n = !1);
            ...
            d = {
                ...
                remove: function () {
                    return oe.each(arguments, function (e, t) {
                        for (var n;
                            (n = oe.inArray(t, a, n)) > -1;) a.splice(n, 1), c >= n && c--
                    }), this
                },
                has: function (e) {
        ...
        when: function (e) {
            var t, n, r, i = 0,
                ...
                u = function (e, n, r) {
                    return function (i) {
                        n[e] = this, r[e] = arguments.length > 1 ? K.call(arguments) : i, r === t ? c.notifyWith(n, r) : --s || c.resolveWith(n, r)
                    }
                };
            if (a > 1)
                for (t = new Array(a), n = new Array(a), r = new Array(a); a > i; i++) o[i] && oe.isFunction(o[i].promise) ? o[i].promise().progress(u(i, n, t)).done(u(i, r, o)).fail(c.reject) : --s;
    ...
    oe.fn.ready = function (e) {
        ...
        return Ce || (Ce = oe.Deferred(), "complete" === Y.readyState || "loading" !== Y.readyState && !Y.documentElement.doScroll ? e.setTimeout(oe.ready) : (Y.addEventListener("DOMContentLoaded", a), e.addEventListener("load", a))), Ce.promise(t)
    }, oe.ready.promise();
    ...
    oe.extend({
        ...
        data: function (e, t) {
            var n, r, i, o = this[0],
                a = o && o.attributes;
            if (void 0 === e) {
                if (this.length && (i = Ie.get(o), 1 === o.nodeType && !Ee.get(o, "hasDataAttrs"))) {
                    for (n = a.length; n--;) a[n] && (r = a[n].name, 0 === r.indexOf("data-") && (r = oe.camelCase(r.slice(5)), c(o, r, i[r])));
            ...
            return "object" == typeof e ? this.each(function () {
                ...
                var n, r;
                if (o && void 0 === t) {
                    if (n = Ie.get(o, e) || Ie.get(o, e.replace(Oe, "-$&").toLowerCase()), void 0 !== n) return n;
                    if (r = oe.camelCase(e), n = Ie.get(o, r), void 0 !== n) return n;
    ...
    oe.event = {
        ...
        add: function (e, t, n, r, i) {
            ...
            if (m)
                for (n.handler && (o = n, n = o.handler, i = o.selector), n.guid || (n.guid = oe.guid++), (c = m.events) || (c = m.events = {}), (a = m.handle) || (a = m.handle = function (t) {
                    return "undefined" != typeof oe && oe.event.triggered !== t.type ? oe.event.dispatch.apply(e, arguments) : void 0
                }), t = (t || "").match(we) || [""], u = t.length; u--;) s = ze.exec(t[u]) || [], p = g = s[1], h = (s[2] || "").split(".").sort(), p && (l = oe.event.special[p] || {}, p = (i ? l.delegateType : l.bindType) || p, l = oe.event.special[p] || {}, d = oe.extend({
                    type: p,
                    origType: g,
                    data: r,
                    handler: n,
                    ...
                    namespace: h.join(".")
                }, o), (f = c[p]) || (f = c[p] = [], f.delegateCount = 0, l.setup && l.setup.call(e, r, h, a) !== !1 || e.addEventListener && e.addEventListener(p, a)), l.add && (l.add.call(e, d), d.handler.guid || (d.handler.guid = n.guid)), i ? f.splice(f.delegateCount++, 0, d) : f.push(d), oe.event.global[p] = !0)
        },
        remove: function (e, t, n, r, i) {
            var o, a, s, c, u, d, l, f, p, h, g, m = Ee.hasData(e) && Ee.get(e);
            if (m && (c = m.events)) {
                for (t = (t || "").match(we) || [""], u = t.length; u--;)
                    if (s = ze.exec(t[u]) || [], p = g = s[1], h = (s[2] || "").split(".").sort(), p) {
                        for (l = oe.event.special[p] || {}, p = (r ? l.delegateType : l.bindType) || p, f = c[p] || [], s = s[2] && new RegExp("(^|\\.)" + h.join("\\.(?:.*\\.|)") + "(\\.|$)"), a = o = f.length; o--;) d = f[o], !i && g !== d.origType || n && n.guid !== d.guid || s && !s.test(d.namespace) || r && r !== d.selector && ("**" !== r || !d.selector) || (f.splice(o, 1), d.selector && f.delegateCount--, l.remove && l.remove.call(e, d));
                        a && !f.length && (l.teardown && l.teardown.call(e, h, m.handle) !== !1 || oe.removeEvent(e, p, m.handle), delete c[p])
                    } else
        ...
        dispatch: function (e) {
            ...
            if (s[0] = e, e.delegateTarget = this, !u.preDispatch || u.preDispatch.call(this, e) !== !1) {
                for (a = oe.event.handlers.call(this, e, c), t = 0;
            ...
            if (s && c.nodeType && ("click" !== e.type || isNaN(e.button) || e.button < 1))
                for (; c !== this; c = c.parentNode || this)
                    if (1 === c.nodeType && (c.disabled !== !0 || "click" !== e.type)) {
                        for (r = [], n = 0; s > n; n++) o = t[n], i = o.selector + " ", void 0 === r[i] && (r[i] = o.needsContext ? oe(i, this).index(c) > -1 : oe.find(i, this, null, [c]).length), r[i] && r.push(o);
                        r.length && a.push({
                            elem: c,
                            handlers: r
                        })
        ...
        fix: function (e) {
            ...
            var t, n, r, i = e.type,
                o = e,
                a = this.fixHooks[i];
            for (a || (this.fixHooks[i] = a = qe.test(i) ? this.mouseHooks : Me.test(i) ? this.keyHooks : {}), r = a.props ? this.props.concat(a.props) : this.props, e = new oe.Event(o), t = r.length; t--;) n = r[t], e[n] = o[n];
    ...
    oe.extend({
        ...
        clone: function (e, t, n) {
            var r, i, o, a, s = e.cloneNode(!0),
                c = oe.contains(e.ownerDocument, e);
            if (!(re.noCloneChecked || 1 !== e.nodeType && 11 !== e.nodeType || oe.isXMLDoc(e)))
                for (a = d(s), o = d(e), r = 0, i = o.length; i > r; r++) w(o[r], a[r]);
            if (t)
                if (n)
                    for (o = o || d(e), a = a || d(s), r = 0, i = o.length; i > r; r++) S(o[r], a[r]);
                else S(e, s);
            return a = d(s, "script"), a.length > 0 && l(a, !c && d(e, "script")), s
        },
        ...
        html: function (e) {
            return Te(this, function (e) {
                ...
                if ("string" == typeof e && !$e.test(e) && !Pe[(Re.exec(e) || ["", ""])[1].toLowerCase()]) {
                    e = oe.htmlPrefilter(e);
                    try {
                        for (; r > n; n++) t = this[n] || {}, 1 === t.nodeType && (oe.cleanData(d(t, !1)), t.innerHTML = e);
                        t = 0
                    } catch (e) { }
                }
    ...
    oe.extend({
        ...
        style: function (e, t, n, r) {
            if (e && 3 !== e.nodeType && 8 !== e.nodeType && e.style) {
                var i, o, a, s = oe.camelCase(t),
                    c = e.style;
                return t = oe.cssProps[s] || (oe.cssProps[s] = O(s) || s), a = oe.cssHooks[t] || oe.cssHooks[s], void 0 === n ? a && "get" in a && void 0 !== (i = a.get(e, !1, r)) ? i : c[t] : (o = typeof n, "string" === o && (i = ke.exec(n)) && i[1] && (n = u(e, t, i), o = "number"), void (null != n && n === n && ("number" === o && (n += i && i[3] || (oe.cssNumber[s] ? "" : "px")), re.clearCloneStyle || "" !== n || 0 !== t.indexOf("background") || (c[t] = "inherit"), a && "set" in a && void 0 === (n = a.set(e, n, r)) || (c[t] = n))))
            }
        },
        css: function (e, t, n, r) {
            var i, o, a, s = oe.camelCase(t);
            return t = oe.cssProps[s] || (oe.cssProps[s] = O(s) || s), a = oe.cssHooks[t] || oe.cssHooks[s], a && "get" in a && (i = a.get(e, !0, n)), void 0 === i && (i = I(e, t, r)), "normal" === i && t in tt && (i = tt[t]), "" === n || n ? (o = parseFloat(i), n === !0 || isFinite(o) ? o || 0 : i) : i
        }
        ...
        oe.cssHooks[t] = {
            ...
            set: function (e, n, r) {
                var i, o = r && Ke(e),
                    a = r && k(e, t, r, "border-box" === oe.css(e, "boxSizing", !1, o), o);
                return a && (i = ke.exec(n)) && "px" !== (i[3] || "px") && (e.style[t] = n, n = oe.css(e, t)), j(e, n, a)
            }
        }
    ...
    oe.fn.extend({
        addClass: function (e) {
            ...
            if ("string" == typeof e && e)
                for (t = e.match(we) || []; n = this[c++];)
                    if (i = z(n), r = 1 === n.nodeType && (" " + i + " ").replace(ft, " ")) {
        ...
        toggleClass: function (e, t) {
            ...
            return "boolean" == typeof t && "string" === n ? t ? this.addClass(e) : this.removeClass(e) : oe.isFunction(e) ? this.each(function (n) {
                ...
                else void 0 !== e && "boolean" !== n || (t = z(this), t && Ee.set(this, "__className__", t), this.setAttribute && this.setAttribute("class", t || e === !1 ? "" : Ee.get(this, "__className__") || ""));
            })
        },
        hasClass: function (e) {
            var t, n, r = 0;
            ...
            select: {
                get: function (e) {
                    for (var t, n, r = e.options, i = e.selectedIndex, o = "select-one" === e.type || 0 > i, a = o ? null : [], s = o ? i + 1 : r.length, c = 0 > i ? s : o ? i : 0; s > c; c++)
                        if (n = r[c], (n.selected || c === i) && (re.optDisabled ? !n.disabled : null === n.getAttribute("disabled")) && (!n.parentNode.disabled || !oe.nodeName(n.parentNode, "optgroup"))) {
    ...
    oe.extend(oe.event, {
        trigger: function (t, n, r, i) {
            var o, a, s, c, u, d, l, f = [r || Y],
                p = ne.call(t, "type") ? t.type : t,
                h = ne.call(t, "namespace") ? t.namespace.split(".") : [];
            if (a = s = r = r || Y, 3 !== r.nodeType && 8 !== r.nodeType && !gt.test(p + oe.event.triggered) && (p.indexOf(".") > -1 && (h = p.split("."), p = h.shift(), h.sort()), u = p.indexOf(":") < 0 && "on" + p, t = t[oe.expando] ? t : new oe.Event(p, "object" == typeof t && t), t.isTrigger = i ? 2 : 3, t.namespace = h.join("."), t.rnamespace = t.namespace ? new RegExp("(^|\\.)" + h.join("\\.(?:.*\\.|)") + "(\\.|$)") : null, t.result = void 0, t.target || (t.target = r), n = null == n ? [t] : oe.makeArray(n, [t]), l = oe.event.special[p] || {}, i || !l.trigger || l.trigger.apply(r, n) !== !1)) {
                if (!i && !l.noBubble && !oe.isWindow(r)) {
                    for (c = l.delegateType || p, gt.test(c + p) || (a = a.parentNode); a; a = a.parentNode) f.push(a), s = a;
                    s === (r.ownerDocument || Y) && f.push(s.defaultView || s.parentWindow || e)
                }
                for (o = 0;
                    (a = f[o++]) && !t.isPropagationStopped();) t.type = o > 1 ? c : l.bindType || p, d = (Ee.get(a, "events") || {})[t.type] && Ee.get(a, "handle"), d && d.apply(a, n), d = u && a[u], d && d.apply && xe(a) && (t.result = d.apply(a, n), t.result === !1 && t.preventDefault());
                return t.type = p, i || t.isDefaultPrevented() || l._default && l._default.apply(f.pop(), n) !== !1 || !xe(r) || u && oe.isFunction(r[p]) && !oe.isWindow(r) && (s = r[u], s && (r[u] = null), oe.event.triggered = p, r[p](), oe.event.triggered = void 0, s && (r[u] = s)), t.result
            }
        },
    ...
    Ot.href = mt.href, oe.extend({
        ...
        ajax: function (t, n) {
            function r(t, n, r, s) {
                var u, l, b, y, w, T = n;
                2 !== S && (S = 2, c && e.clearTimeout(c), i = void 0, a = s || "", C.readyState = t > 0 ? 4 : 0, u = t >= 200 && 300 > t || 304 === t, r && (y = H(f, C, r)), y = W(f, y, C, u), u ? (f.ifModified && (w = C.getResponseHeader("Last-Modified"), w && (oe.lastModified[o] = w), w = C.getResponseHeader("etag"), w && (oe.etag[o] = w)), 204 === t || "HEAD" === f.type ? T = "nocontent" : 304 === t ? T = "notmodified" : (T = y.state, l = y.data, b = y.error, u = !b)) : (b = T, !t && T || (T = "error", 0 > t && (t = 0))), C.status = t, C.statusText = (n || T) + "", u ? g.resolveWith(p, [l, T, C]) : g.rejectWith(p, [C, T, b]), C.statusCode(v), v = void 0, d && h.trigger(u ? "ajaxSuccess" : "ajaxError", [C, f, u ? l : b]), m.fireWith(p, [C, T]), d && (h.trigger("ajaxComplete", [C, f]), --oe.active || oe.event.trigger("ajaxStop")))
            }
            "object" == typeof t && (n = t, t = void 0), n = n || {};
            var i, o, a, s, c, u, d, l, f = oe.ajaxSetup({}, n),
                p = f.context || f,
                ...
                C = {
                    ...
                    setRequestHeader: function (e, t) {
                        var n = e.toLowerCase();
                        return S || (e = y[n] = y[n] || e, b[e] = t), this
                    },
            ...
            if (g.promise(C).complete = m.add, C.success = C.done, C.error = C.fail, f.url = ((t || f.url || mt.href) + "").replace(yt, "").replace(xt, mt.protocol + "//"), f.type = n.method || n.type || f.method || f.type, f.dataTypes = oe.trim(f.dataType || "*").toLowerCase().match(we) || [""], null == f.crossDomain) {
                u = Y.createElement("a");
                try {
            ...
            if (f.data && f.processData && "string" != typeof f.data && (f.data = oe.param(f.data, f.traditional)), $(Et, f, n, C), 2 === S) return C;
            d = oe.event && f.global, d && 0 === oe.active++ && oe.event.trigger("ajaxStart"), f.type = f.type.toUpperCase(), f.hasContent = !Tt.test(f.type), o = f.url, f.hasContent || (f.data && (o = f.url += (bt.test(o) ? "&" : "?") + f.data, delete f.data), f.cache === !1 && (f.url = St.test(o) ? o.replace(St, "$1_=" + vt++) : o + (bt.test(o) ? "&" : "?") + "_=" + vt++)), f.ifModified && (oe.lastModified[o] && C.setRequestHeader("If-Modified-Since", oe.lastModified[o]), oe.etag[o] && C.setRequestHeader("If-None-Match", oe.etag[o])), (f.data && f.hasContent && f.contentType !== !1 || n.contentType) && C.setRequestHeader("Content-Type", f.contentType), C.setRequestHeader("Accept", f.dataTypes[0] && f.accepts[f.dataTypes[0]] ? f.accepts[f.dataTypes[0]] + ("*" !== f.dataTypes[0] ? ", " + At + "; q=0.01" : "") : f.accepts["*"]);
            for (l in f.headers) C.setRequestHeader(l, f.headers[l]);
            if (f.beforeSend && (f.beforeSend.call(p, C, f) === !1 || 2 === S)) return C.abort();
            w = "abort";
            for (l in {
                success: 1,
                error: 1,
            ...
            if (i = $(It, f, n, C)) {
                if (C.readyState = 1, d && h.trigger("ajaxSend", [C, f]), 2 === S) return C;
    ...
    var Rt = {
        ...
        Ut = oe.ajaxSettings.xhr();
    re.cors = !!Ut && "withCredentials" in Ut, re.ajax = Ut = !!Ut, oe.ajaxTransport(function (t) {
        var n, r;
        return re.cors || Ut && !t.crossDomain ? {
            send: function (i, o) {
                var a, s = t.xhr();
    ...
    oe.ajaxSetup({
        ...
        return s || "jsonp" === t.dataTypes[0] ? (i = t.jsonpCallback = oe.isFunction(t.jsonpCallback) ? t.jsonpCallback() : t.jsonpCallback, s ? t[s] = t[s].replace(Bt, "$1" + i) : t.jsonp !== !1 && (t.url += (bt.test(t.url) ? "&" : "?") + t.jsonp + "=" + i), t.converters["script json"] = function () {
    ...
    oe.fn.load = function (e, t, n) {
        ...
        setOffset: function (e, t, n) {
            var r, i, o, a, s, c, u, d = oe.css(e, "position"),
                l = oe(e),
                f = {};
            "static" === d && (e.style.position = "relative"), s = l.offset(), o = oe.css(e, "top"), c = oe.css(e, "left"), u = ("absolute" === d || "fixed" === d) && (o + c).indexOf("auto") > -1, u ? (r = l.position(), a = r.top, i = r.left) : (a = parseFloat(o) || 0, i = parseFloat(c) || 0), oe.isFunction(t) && (t = t.call(e, n, oe.extend({}, s))), null != t.top && (f.top = t.top - s.top + a), null != t.left && (f.left = t.left - s.left + i), "using" in t ? t.using.call(e, f) : l.css(f)
        }
    }, oe.fn.extend({
        offset: function (e) {
        ...
        oe.cssHooks[t] = A(re.pixelPosition, function (e, n) {
            return n ? (n = I(e, t), Ye.test(n) ? oe(e).position()[t] + "px" : n) : void 0
        })
    }), oe.each({
        Height: "height",
        Width: "width"
    ...
    return oe.noConflict = function (t) {
        return e.$ === oe && (e.$ = zt), t && e.jQuery === oe && (e.jQuery = qt), oe
    }, t || (e.jQuery = e.$ = oe), oe
}), ! function (e) {
    ...
    if ("undefined" == typeof e) return void ("console" in window && void 0);
    if (e.fn.jquery.match(/-ajax/)) return void ("console" in window && void 0);
    var i = [],
        o = function (t) {
        ...
        getContent: function () {
            ...
            if (!a) {
                ...
                if (o = null, e.each(t.contentFilters, function () {
                    return a = n[this], a.test && (o = a.test(c)), !o && a.regex && c.match && c.match(a.regex) && (o = c), !o
                }), !o) return "console" in window && void 0, !1
            }
        ...
        attach: function (t, n, r) {
            ...
            var o, a = r.namespace || i.defaults.namespace,
                ...
                c = function (a) {
                    ...
                    "shared" === d.persist ? o = d : d.persist !== !1 && c.data("featherlight-persisted", d), u.$currentTarget.blur && u.$currentTarget.blur(), d.open(a)
                };
...
jQuery.cachedScript = function (e, t) {
    ...
    $("#btnShare").click(function (e) {
        e.preventDefault(), window.scrollBy(0, 50), $("#shareContainer").show(), $("#shareContainerElements").hide(), $("#shareContainerLoader").show(), getPermalink1()
    })
}),
    function () {
        function e() {
            function e() {
                ...
                if (!n)
                    if (t.body) {
                        ...
                        r.style.cssText = "display:none", r.name = i, t.body.appendChild(r)
                    } else setTimeout(e, 5);
                return !n
            }
            ...
            r || (e(), a.__tcfapi = t, a.addEventListener("message", n, !1))
        }
        var t = window.top.location.hostname.replace(/^.{1,4}\./, "").replace(/\.test$/, ""),
            n = "www." + t,
            r = document.createElement("script"),
            i = document.getElementsByTagName("script")[0],
            o = ((new Date).getTime(), "https://quantcast.mgr.consensu.org".concat("/choice/", "vptV-UyUkzTeZ", "/", n, "/choice.js")),
            a = 0,
            s = 3;
        r.async = !0, r.type = "text/javascript", r.src = o, i.parentNode.insertBefore(r, i), e();
        var c = function () {
            var e = arguments;
            ...
            u = function () {
                a++, window.__uspapi === c && a < s || clearInterval(d)
            };
...
$(window).on("orientationchange resize", function (e) {
    ...
    $(document).on("click", ".js-box-close", function () {
        ...
        hideGlobalNotifications()
    }), $.featherlight._callbackChain.beforeOpen = function (e) { }, $.featherlight._callbackChain.afterClose = function (e) { }
}), $.ajaxSetup({
    xhrFields: {
        withCredentials: !0
    },
    ...
    t.responseJSON && (t.responseJSON.user && User.load(t.responseJSON.user), MVVM && MVVM.apply())
...
document.addEventListener(visibilityEventName, handleVisibilityChange, !1), document.addEventListener("focus", function () {
    handleVisibilityChange(!0)
}, !1), document.addEventListener("blur", function () {
...
var Layout = {
    ...
    setOrientation: function () {
        this.portrait = window.innerHeight > window.innerWidth, $("body").toggleClass("landscape", !this.portrait).toggleClass("portrait", this.portrait).toggleClass("lefthanded", Settings.get("global.left-handed")), setZoom(getZoom())
    },
    init: function () {
        this.setOrientation()
    },
    ...
    toggleHeader: function () {
        $("body").toggleClass("header-hidden"), window.ga && ga("send", "event", "Layout", "Header", this.hiddenHeader() ? "hide" : "show")
    },
...
var User = {
    ...
    xp: 0,
    init: function () {
        this.logged = 0, this.patron = 0, this.email = "", this.nick = "", this.xp = 0, MVVM.apply()
    },
    ...
    load: function (e) {
        this.logged = 1;
        for (k in e) this[k] = e[k];
        if (this.store(), $("#user-logged-badge .progress").animate({
            width: e.levelXP / e.totalLevelXP * 100 + "%"
        }, 1e3), "undefined" != typeof puzzleSlug && ($(".quest-" + puzzleSlug).each(function () {
            $(this).find(".quest-tier").text("1"), $(this).find(".progress").css({
                width: "0%"
            })
        }), e.achievements[puzzleSlug])) {
            var t = e.achievements[puzzleSlug];
            for (size in t) $(".quest-" + puzzleSlug + "-" + size).each(function () {
                var e = 100;
                puzzleSlug + "-" + size == "skyscrapers-4x4-easy" && (e = 3), t[size].level > e ? ($(this).find(".quest-tier").html("&check;"), $(this).find(".quest-tier-badge").css({
                    color: "#66ff66"
                }), $(this).find(".progress").animate({
                    width: "100%"
                })) : ($(this).find(".quest-tier").text(t[size].level), $(this).find(".progress").animate({
    ...
    loadFromAjax: function (e) {
        ...
        $.ajax({
            ...
            success: function (n) {
                ...
                var r = t.patron ? 1 : 0;
                t.load(n), r != n.patron && window.location.reload(), e || $.featherlight.close()
            },
            error: function (n) {
                t.reset(), e && $.featherlight.close()
            }
        })
    },
    showLogin: function () {
        return this.init(), $.featherlight.close(), $.featherlight("/partials/form.login.php", {}), !1
    },
    reset: function () {
        eraseCookie("api_token"), this.logged = 0, this.patron = 0, this.email = "", this.achievements = {}, this.nick = "", this.xp = 0, this.api_token = "", this.store(), MVVM.apply()
    },
    logout: function () {
        var e = this;
        $.ajax({
    ...
    update: function (e) {
        $.post("https://" + mobile_host + "/api/profile", e).done(function (e) {
            ...
            if (e.responseJSON) var t = $.map(e.responseJSON, function (e) {
                return e
            }).join("<br>");
            else var t = e.responseText;
...
window.addEventListener("message", function (e) {
    ...
    if (User.arrFavorites.indexOf(e) != -1) {
        ...
        $.post("/api/favorite", {
            ...
            t.closest("li").prependTo(t.closest("ul")).find(".favorite").addClass("on"), User.arrFavorites.push(e)
        })
    }
...
! function (e) {
    ...
    window.pbjsChunk = function (r, o, a) {
        for (var s, c, u, d = 0, l = []; d < r.length; d++) c = r[d], i[c] && l.push(i[c][0]), i[c] = 0;
    ...
    0: function (e, t, n) {
        ...
        function b() {
            S() && me && console.error.apply(console, y(arguments, "ERROR:")), ve.emit(ie.EVENTS.AUCTION_DEBUG, {
                type: "ERROR",
                arguments: arguments
            })
        }
        ...
        function O(e, t) {
            if (!A(e)) {
                if (C(e.forEach)) return e.forEach(t, this);
                var n = 0,
                    r = e.length;
                if (0 < r)
                    for (; n < r; n++) t(e[n], n, e);
        ...
        function _(e, t) {
            var n = new Image;
            t && be.isFn(t) && (n.addEventListener("load", t), n.addEventListener("error", t)), n.src = e
        }
        ...
        function D(e, t) {
            ...
            r.innerHTML = n;
            var i = r.firstChild;
            t && be.isFn(t) && (i.addEventListener("load", t), i.addEventListener("error", t)), be.insertElement(i, document, "html", !0)
        }

        function N(e) {
            var t = 2 < arguments.length && void 0 !== arguments[2] ? arguments[2] : "";
            return e ? ((!(1 < arguments.length && void 0 !== arguments[1]) || arguments[1]) && (e = encodeURI(e)), t = t && 'sandbox="'.concat(t, '"'), "<iframe ".concat(t, ' id="').concat(c(), '"\n      frameborder="0"\n      allowtransparency="true"\n      marginheight="0" marginwidth="0"\n      width="0" hspace="0" vspace="0" height="0"\n      style="height:0px;width:0px;display:none;"\n      scrolling="no"\n      src="').concat(e, '">\n    </iframe>')) : ""
        }
        ...
        function q() {
            if (window.googletag && C(window.googletag.pubads) && C(window.googletag.pubads().getSlots)) return !0
        }
        ...
        Object.defineProperty(t, "__esModule", {
            ...
            return Se
        }), t.getUniqueIdentifierStr = c, t.generateUUID = function e(t) {
            ...
            if ("string" == typeof e) {
                var n = e.split(","),
                    r = /^(\d)+x(\d)+$/i;
                if (n)
                    for (var i in n) j(n, i) && n[i].match(r) && t.push(n[i])
            } else if ("object" === s(e)) {
                var o = e.length;
                if (0 < o)
            ...
            return t
        }, t.parseGPTSingleSizeArray = d, t.parseGPTSingleSizeArrayToRtbSize = function (e) {
            if (l(e)) return {
                w: e[0],
                h: e[1]
            }
        }, t.getWindowTop = f, t.getWindowSelf = p, t.getWindowLocation = h, t.logMessage = g, t.logInfo = m, t.logWarn = v, t.logError = b, t.hasConsoleLogger = function () {
            return pe
        }, t.debugTurnedOn = S, t.createInvisibleIframe = function () {
            var e = document.createElement("iframe");
            return e.id = c(), e.height = 0, e.width = 0, e.border = "0px", e.hspace = "0", e.vspace = "0", e.marginWidth = "0", e.marginHeight = "0", e.style.border = "0", e.scrolling = "no", e.frameBorder = "0", e.src = "about:blank", e.style.display = "none", e
        }, t.getParameterByName = function (e) {
            return W(h().search)[e] || ""
        }, t.isA = w, t.isFn = C, t.isStr = T, t.isArray = x, t.isNumber = E, t.isPlainObject = I, t.isBoolean = function (e) {
            return w(e, de)
        }, t.isEmpty = A, t.isEmptyStr = function (e) {
            return T(e) && (!e || 0 === e.length)
        }, t._each = O, t.contains = function (e, t) {
            if (A(e)) return !1;
            if (C(e.indexOf)) return -1 !== e.indexOf(t);
            for (var n = e.length; n--;)
                if (e[n] === t) return !0;
            return !1
        }, t._map = function (e, t) {
            if (A(e)) return [];
            if (C(e.map)) return e.map(t);
            var n = [];
            return O(e, function (r, i) {
                n.push(t(r, i, e))
            }), n
        }, t.hasOwn = j, t.insertElement = k, t.triggerPixel = _, t.callBurl = function (e) {
            var t = e.source,
                n = e.burl;
            ...
            if (e) {
                ...
                t.id = c(), t.width = 0, t.height = 0, t.hspace = "0", t.vspace = "0", t.marginWidth = "0", t.marginHeight = "0", t.style.display = "none", t.style.height = "0px", t.style.width = "0px", t.scrolling = "no", t.frameBorder = "0", t.allowtransparency = "true", be.insertElement(t, document, "body"), t.contentWindow.document.open(), t.contentWindow.document.write(e), t.contentWindow.document.close()
            }
        }, t.insertUserSyncIframe = D, t.createTrackPixelHtml = function (e) {
            if (!e) return "";
            var t = encodeURI(e),
                n = '<div style="position:absolute;left:0px;top:0px;visibility:hidden;">';
            return n += '<img src="' + t + '"></div>'
        }, t.createTrackPixelIframeHtml = N, t.getValueString = R, t.uniques = U, t.flatten = P, t.getBidRequest = function (e, t) {
            ...
            var n
        }, t.getKeys = B, t.getValue = M, t.getKeyByValue = function (e, t) {
            for (var n in e)
        ...
        }, t.isGptPubadsDefined = q, n.d(t, "getHighestCpm", function () {
            return Ce
        ...
        }, t.deepClone = L, t.inIframe = function () {
            try {
        ...
        }, t.checkCookieSupport = $, t.delayExecution = function (e, t) {
        ...
        }, t.isSlotMatchingAdUnitCode = F, t.getGptSlotInfoForAdUnitCode = function (e) {
            var t;
            return q() && (t = Q()(window.googletag.pubads().getSlots(), F(e))), t ? {
        ...
        }, t.isInteger = H, t.convertCamelToUnderscore = function (e) {
            ...
            return "object" === s(e) ? t.reduce(function (n, r, i) {
                ...
                a && (r = a[1], o = a[2]);
                var s = e[r];
                return "function" == typeof t[i + 1] && (s = t[i + 1](s, n)), void 0 !== s && (n[o] = s), n
            }, {}) : {}
            ...
            return O(e, function (e, r) {
                if (x(e)) {
                    var i = [];
                    O(e, function (e) {
                        !(e = R(t + "." + r, e)) && "" !== e || i.push(e)
                    }), e = i
                } else {
                    if (!T(e = R(t + "." + r, e))) return;
                    e = [e]
                }
                n.push({
                    key: r,
            ...
            return Object.keys(e).forEach(function (n) {
                var r, i;
                t[n] && (C(e[n]) ? t[n] = e[n](t[n]) : t[n] = (r = e[n], i = t[n], "string" === r ? i && i.toString() : "number" === r ? Number(i) : i), isNaN(t[n]) && delete t.key)
            }), t
        ...
        }, t.parseQS = W, t.formatQS = G, t.parseUrl = function (e, t) {
        ...
        var re, ie = n(5),
            ...
            fe = Boolean(window.console),
            pe = Boolean(fe && window.console.log),
            he = Boolean(fe && window.console.info),
            ge = Boolean(fe && window.console.warn),
            me = Boolean(fe && window.console.error),
            ve = n(8),
            be = {
                checkCookieSupport: $,
    ...
    1: function (e, t, n) {
        ...
        function a(e) {
            ...
            function n(t) {
                return !!e.isBidRequestValid(t) || (Object(T.logWarn)("Invalid bid sent to bidder ".concat(e.code, ": ").concat(JSON.stringify(t))), !1)
            }
            return o(new u.a(e.code), {
                ...
                callBids: function (r, a, s, u, d, l) {
                    function p() {
                        s(), y.a.emit(v.a.EVENTS.BIDDER_DONE, r), t(g, r.gdprConsent, r.uspConsent)
                    }
                                ...
                                S.forEach(function (t) {
                                    function n(n, i) {
                                        function s(t) {
                                            ...
                                            if (s) {
                                                t.originalCpm = t.cpm, t.originalCurrency = t.currency, t.meta = t.meta || o({}, t[s.bidder]);
                                                var u = o(Object(f.a)(v.a.STATUS.GOOD, s), t);
                                                n = s.adUnitCode, i = u, h[n] = !0, c(n, i, [r]) && a(n, i)
                                            } else Object(T.logWarn)("Bidder ".concat(e.code, " made bid for unknown request ID: ").concat(t.requestId, ". Ignoring."))
                                        }
                                        d(e.code);
                                        try {
                                            n = JSON.parse(n)
                                        ...
                                        try {
                                            ...
                                            return Object(T.logError)("Bidder ".concat(e.code, " failed to interpret the server's response. Continuing without bids"), null, t), void w()
                                        }
                                        u && (Object(T.isArray)(u) ? u.forEach(s) : s(u)), w(u)
                                    }

                                    function s(t) {
                                        d(e.code), Object(T.logError)("Server call for ".concat(e.code, " failed: ").concat(t, ". Continuing without bids.")), w()
                                    }
                                    switch (t.method) {
                                        case "GET":
                                        ...
                                        case "POST":
                                            ...
                                            break;
                                        default:
                                            Object(T.logWarn)("Skipping invalid request from ".concat(e.code, ". Request type ").concat(t.type, " must be GET or POST")), w()
                                    }
                                })
                            } else p()
                        } else p()
        ...
        function c(e, t, n) {
            ...
            return e ? t ? (o = Object.keys(t), O.every(function (e) {
        ...
        Object.defineProperty(t, "__esModule", {
            ...
            t(e), Array.isArray(e.aliases) && e.aliases.forEach(function (n) {
                var r, i = n;
                Object(T.isPlainObject)(n) && (i = n.code, r = n.gvlid), d.default.aliasRegistry[i] = e.code, t(o({}, e, {
                    code: i,
                    gvlid: r
                }))
            })
            ...
            return k
        }), t.preloadBidderMappingFile = s, t.getIabSubCategory = function (e, t) {
        ...
        var u = n(90),
            ...
            k = Object(E.b)("async", function (e, t, n, r) {
                ...
                if (e.getUserSyncs && (i || !d.default.aliasRegistry[e.code])) {
                    ...
                    a && (Array.isArray(a) || (a = [a]), a.forEach(function (t) {
    ...
    11: function (e, t, n) {
        ...
        function r(e) {
            var t = this,
                ...
                d = e.loaded,
                l = e.adUnitCode;
            this.url = n, this.config = r, this.handlers = {}, this.id = a, this.loaded = d, this.cmd = [], this.push = function (e) {
                ...
                var e, t, r;
                e = l, t = pbjs.adUnits, !((r = s()(t, function (t) {
                    return t.code === e
                })) && r.renderer && r.renderer.url && r.renderer.render) || o.isBoolean(r.renderer.backupOnly) && r.renderer.backupOnly ? Object(i.a)(n, c, this.callback) : o.logWarn("External Js not loaded by Renderer since renderer url and callback is already defined on adUnit ".concat(l)), this._render ? this._render.apply(this, arguments) : o.logWarn("No render function was provided, please use .setRender on the renderer")
            }.bind(this)
        }
    ...
    110: function (e, t, n) {
        ...
        e.exports = function (e, t, n) {
            ...
            } else g = n.getConstructor(t, e, b, S), o.REQUIRED = !0;
            return l(g, e, !1, !0), T[e] = g, r({
                global: !0,
                forced: !0
            ...
            getConstructor: function (e, t, n, u) {
                function d(e, t, n) {
                    ...
                    return a ? a.value = n : (o.last = a = {
                        ...
                        next: void 0,
                        removed: !1
                    }, o.first || (o.first = a), r && (r.next = a), l ? o.size++ : e.size++, "F" !== i && (o.index[i] = a)), e
                }
    ...
    116: function (e, t, n) {
        ...
        e.exports = r ? Object.defineProperties : function (e, t) {
            o(e);
            for (var n, r = a(t), s = r.length, c = 0; c < s;) i.f(e, n = r[c++], t[n]);
            return e
        }
    },
    117: function (e, t, n) {
    ...
    132: function (e, t, n) {
        ...
        var r = n(18),
            ...
            o = n(17);
        e.exports = function (e, t, n) {
            var a, s, c, u, d = arguments.length,
                l = 1 < d ? t : void 0;
            return r(this), (a = void 0 !== l) && r(l), null == e ? new this : (s = [], a ? (c = 0, u = i(l, 2 < d ? n : void 0, 2), o(e, function (e) {
                s.push(u(e, c++))
            })) : o(e, s.push, s), new this(s))
        }
    },
    133: function (e, t, n) {
        n(14)({
    ...
    138: function (e, t, n) {
        ...
        var r = n(15),
            i = n(18);
        e.exports = function () {
            for (var e, t = r(this), n = i(t.delete), o = !0, a = 0, s = arguments.length; a < s; a++) e = n.call(t, arguments[a]), o = o && e;
            return !!o
        }
    },
    139: function (e, t, n) {
    ...
    14: function (e, t, n) {
        ...
        var i = n(24),
            ...
            d = n(26);
        e.exports = function (e, t) {
            var n, l, f, p, h, g, m, v, b = e.target,
                y = e.global,
                S = e.stat,
                w = e.proto,
                C = y ? i : S ? i[b] : (i[b] || {}).prototype,
                T = y ? s : s[b] || (s[b] = {}),
                x = T.prototype;
            for (f in t) n = !a(y ? f : b + (S ? "." : "#") + f, e.forced) && C && d(C, f), h = T[f], n && (g = e.noTargetGet ? (v = o(C, f)) && v.value : C[f]), p = n && g ? g : t[f], n && typeof h == typeof p || (m = e.bind && n ? c(p, i) : e.wrap && n ? r(p) : w && "function" == typeof p ? c(Function.call, p) : p, (e.sham || p && p.sham || h && h.sham) && u(m, "sham", !0), T[f] = m, w && (d(s, l = b + "Prototype") || u(s, l, {}), s[l][f] = p, e.real && x && !x[f] && u(x, f, p)))
        }
    },
    140: function (e, t, n) {
        "use strict";
        ...
        r({
            ...
            isSubsetOf: function (e) {
                var t = c(this),
                    n = a(e),
                    r = n.has;
                return "function" != typeof r && (n = new (o("Set"))(e), r = s(n.has)), !u(t, function (e) {
    ...
    155: function (e, t, n) {
        ...
        e.exports = function (e, t, n) {
            ...
            if (S && (y = r(y, 2 < b ? n : void 0, 2)), null == w || v == Array && a(w))
                for (l = new v(d = s(m.length)); C < d; C++) g = S ? y(m[C], C) : m[C], c(l, C, g);
            else
                for (h = (p = w.call(m)).next, l = new v; !(f = h.call(p)).done; C++) g = S ? o(p, y, [f.value, C], !0) : f.value, c(l, C, g);
            return l.length = C, l
        }
    },
    156: function (e, t, n) {
        ...
        function r(e) {
            ...
            function u(e) {
                ...
                return s.call(t, function (e, n, r) {
                    var i = e[n],
                        o = !1;
                    return i || (r === t.length - 1 ? (f || h.push(function () { }), e[n] = d(function (t) {
                        e[n] = t, o = !0
                    })) : e[n] = {})
                }, p)
            }
            ...
            function d(e) {
                function t(e, t, o, a) {
                    var s = {
                        ...
                        priority: a || 10,
                        remove: function () {
                            var t = e.indexOf(s); - 1 !== t && (e.splice(t, 1), i(n, r))
                        }
                    };
                ...
                var n = [],
                    ...
                    s = {
                        install: function (t, o, a) {
                            this.type = t, (i = a)(n, r), e && e(o)
                        }
                    };
                return a.set(o.after, s), o
            }
            ...
            function l(t, o, s) {
                ...
                if (p) {
                    if (p.type !== t) throw i + ": recreated hookable with different type";
                    return o
                }
                var g, m, v = s ? u(s) : d(),
                    b = {
                ...
                return f || h.push(l), e.useProxy && "function" == typeof Proxy && Proxy.revocable ? m = new Proxy(o, b) : c(m = function () {
                    return b.apply ? b.apply(o, this, n(arguments)) : o.apply(this, arguments)
                }, v), a.get(m.after).install(t, m, function (e, r) {
                    function i(e) {
                        a.push(e.hook)
                    }
                    var o, a = [];
                    g = e.length || r.length ? (e.forEach(i), o = a.push(void 0) - 1, r.forEach(i), function (e, r, i) {
                        function s(e) {
                        ...
                        function c(e) {
                            if (a[d]) {
                                var i = n(arguments);
                                return c.bail = s, i.unshift(c), a[d++].apply(r, i)
                            }
                            "sync" === t ? u = e : l && l.apply(null, arguments)
                        }
                        var u, d = 0,
                            l = "async" === t && "function" == typeof i[i.length - 1] && i.pop();
                        return a[o] = function () {
                            var i = n(arguments, 1);
                            "async" === t && l && (delete c.bail, i.push(c));
                            var o = e.apply(r, i);
                            "sync" === t && c(o)
                        }, c.apply(null, i), u
                    }) : void 0, l()
            ...
            if (l) f = e;
                ...
                if (o(p)) {
                    for (h = 0, g = a(e.length); h < g; h++)
                        if ((m = d ? y(i(b = e[h])[0], b[1]) : y(e[h])) && m instanceof r) return m;
                    return new r((!1))
                }
                f = p.call(e)
            }
            for (v = f.next; !(b = v.call(f)).done;)
                if ("object" == typeof (m = u(f, y, b.value, d)) && m && m instanceof r) return m;
            return new r((!1))
        }).stop = function (e) {
            return new r((!0), e)
        }
    ...
    19: function (e, t, n) {
        ...
        e.exports = function (e) {
            return o(u, e) || (s && o(d, e) ? u[e] = d[e] : u[e] = l("Symbol." + e)), u[e]
        }
    },
    2: function (e, t, n) {
        "use strict";
                ...
                do {
                    ...
                    if (g)
                        if (p) {
                            var v = f.context;
                            try {
                                t = m = v.sourceUrl, d = !0, e === i.top && (c = !0), v.canonicalUrl && (n = v.canonicalUrl)
                            } catch (e) { }
                        } else {
                            ...
                            try {
                                ...
                                b && (m = b, e === i.top && (c = !0))
                            } catch (e) { } !m && s && s[u - 1] && (m = s[u - 1]), m && !d && (t = m)
                        }
                    else {
                        if (h && (t = m = h, d = !1, e === i.top)) {
                            c = !0;
                            var y = r(e.document);
                            y && (n = y)
                        }
    ...
    227: function (e, t, n) {
        ...
        function r(e) {
            var t, n, r, i, s, d, p, m, v, b = e.message ? "message" : "data",
                y = {};
            try {
                y = JSON.parse(e[b])
            } catch (e) {
            ...
            if (y && y.adId) {
                ...
                if (S && "Prebid Request" === y.message && (n = e, r = (t = S).adId, i = t.ad, s = t.adUrl, d = t.width, p = t.height, m = t.renderer, v = t.cpm, Object(f.c)(m) ? Object(f.b)(m, t) : r && (function (e) {
                    function t(e) {
                        var t, i, o = (t = n, i = r, window.googletag ? function (e) {
                    ...
                    ["div", "iframe"].forEach(function (e) {
                        ...
                        if (n) {
                            var a = n.style;
                            a.width = i + "px", a.height = o + "px"
                        } else Object(c.logWarn)("Unable to locate matching page element for adUnitCode ".concat(r, ".  Can't resize it to ad's dimensions.  Please review setup."))
                    })
                    ...
                    width: d,
                    height: p
                }), n.origin)), u.a.addWinningBid(S), o.a.emit(g, S)), S && "Prebid Native" === y.message) {
                    if ("assetRequest" === y.action) {
                        var w = Object(a.c)(y, S);
                        return void e.source.postMessage(JSON.stringify(w), e.origin)
                    }
                    if ("click" === Object(a.b)(y, S)) return;
                    u.a.addWinningBid(S), o.a.emit(g, S)
                }
            }
        }
        t.a = function () {
    ...
    228: function (e, t, n) {
        ...
        function o(e) {
            ...
            }.bind(e), m.c.before(f, 5), p = function (e, t) {
                var n = this,
                    r = t.filter(function (e) {
                        return !u(n.bidders, e.bidderCode) || (i("bidRequest '".concat(e.bidderCode, "' excluded from auction by bidder overrides")), !1)
                    });
        ...
        function s(e, t) {
            ...
            h.b.setConfig({
                debug: !0
            }), a(), o(e), r("bidder overrides enabled".concat(n ? " from session" : ""))
        }
        ...
        function d(e, t, n) {
            return Object.keys(e).filter(function (e) {
                ...
                return r("bidder overrides changed '".concat(t.adUnitCode, "/").concat(t.bidderCode, "' ").concat(n, ".").concat(i, " from '").concat(t[i], ".js' to '").concat(e[i], "'")), t[i] = e[i], t
            }, t)
        }
        ...
        t.a = function (e) {
            var t;
            try {
                e = e || window.sessionStorage, t = JSON.parse(e.getItem(v))
            } catch (e) { }
            t && s(t, !0)
        };
        var f, p, h = n(3),
    ...
    23: function (e, t, n) {
        ...
        var r, i, o = n(0),
            ...
            c = n.n(s),
            u = n(5),
            d = (r = [], (i = {}).addWinningBid = function (e) {
                var t = c()(r, function (t) {
                    return t.getAuctionId() === e.auctionId
                });
                t ? (e.status = u.BID_STATUS.RENDERED, t.addWinningBid(e)) : Object(o.logWarn)("Auction not found when adding winning bid")
                ...
                if (n && (n.status = t), n && t === u.BID_STATUS.BID_TARGETING_SET) {
    ...
    3: function (e, t, n) {
        ...
        function i() {
            ...
            function t(t) {
                if (t) {
                    if ("string" == typeof t) e(t) || v.logWarn("Prebid Warning: setPriceGranularity was called with invalid setting, using `medium` as default.");
                    else if (v.isPlainObject(t) && !Object(c.b)(t)) return void v.logError("Invalid custom price value passed to `setPriceGranularity()`");
                    return 1
                }
            ...
            var n = {
                ...
                set priceGranularity(n) {
                    t(n) && ("string" == typeof n ? this._priceGranularity = e(n) ? n : j.MEDIUM : v.isPlainObject(n) && (this._customPriceBucket = n, this._priceGranularity = j.CUSTOM, v.logMessage("Using custom price granularity")))
                },
                ...
                set mediaTypePriceGranularity(n) {
                    ...
                    this._mediaTypePriceGranularity = Object.keys(n).reduce(function (i, o) {
                        return t(n[o]) ? "string" == typeof n ? i[o] = e(n[o]) ? n[o] : r._priceGranularity : v.isPlainObject(n) && (i[o] = n[o], v.logMessage("Using custom price granularity for ".concat(o))) : v.logWarn("Invalid price granularity for media type: ".concat(o)), i
                    }, {})
                },
                _sendAllBids: !0,
        ...
        function o() {
            if (A && E && v.isPlainObject(E[A])) {
                ...
                return m(t).reduce(function (t, n) {
                    return void 0 === e[n] ? t[n] = x[n] : void 0 !== x[n] && v.isPlainObject(e[n]) ? t[n] = Object(g.mergeDeep)({}, x[n], e[n]) : t[n] = e[n], t
                }, {})
            }
            return r({}, x)
        }
        ...
        function a(e) {
            ...
            I.filter(function (e) {
                ...
                var n, r, i;
                t.callback((n = {}, r = t.topic, i = e[t.topic], r in n ? Object.defineProperty(n, r, {
                    value: i,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                ...
                setConfig: function (e) {
                    if (v.isPlainObject(e)) {
                        ...
                        t.forEach(function (t) {
                            var i = e[t];
                            v.isPlainObject(T[t]) && v.isPlainObject(i) && (i = r({}, T[t], i)), n[t] = x[t] = i
                        }), a(n)
                    } else v.logError("setConfig options must be an object")
                },
                setDefaults: function (e) {
                    v.isPlainObject(T) ? (r(T, e), r(x, e)) : v.logError("defaults must be an object")
                },
                resetConfig: i,
                runWithBidder: s,
                ...
                setBidderConfig: function (e) {
                    try {
                        ! function (e) {
                            ...
                            E[t] || (E[t] = {}), Object.keys(e.config).forEach(function (n) {
                                var i = e.config[n];
    ...
    37: function (e, t, n) {
        ...
        n.d(t, "e", function () {
            ...
            return e && e.type && function (e) {
                return e && s()(Object.keys(l), e) || (Object(o.logError)("".concat(e, " nativeParam is not supported")), !1)
            }(e.type) ? l[e.type] : e
        }, t.f = function (e, t) {
            var n = Object(o.getBidRequest)(e.requestId, t);
            if (!n) return !1;
            if (!Object(o.deepAccess)(e, "native.clickUrl")) return !1;
            if (Object(o.deepAccess)(e, "native.image") && (!Object(o.deepAccess)(e, "native.image.height") || !Object(o.deepAccess)(e, "native.image.width"))) return !1;
            if (Object(o.deepAccess)(e, "native.icon") && (!Object(o.deepAccess)(e, "native.icon.height") || !Object(o.deepAccess)(e, "native.icon.width"))) return !1;
            ...
            return Object.keys(e.native).forEach(function (r) {
                var a = c.NATIVE_KEYS[r],
                    s = i(e.native[r]);
                Object(o.deepAccess)(t, "mediaTypes.native.".concat(r, ".sendId")) && (s = "".concat(a, ":").concat(e.adId)), a && s && (n[a] = s)
            }), n
        }, t.c = function (e, t) {
            var n = {
            ...
            return function (t, u, d) {
                ...
                try {
                    ...
                    if ("function" == typeof u && (g.success = u), (f = new window.XMLHttpRequest).onreadystatechange = function () {
                        ...
                        r(m.search, d), t = s.buildUrl(m)
                    }
                    f.open(p, t, !0), a.b.getConfig("disableAjaxTimeout") || (f.timeout = e), l.withCredentials && (f.withCredentials = !0), s._each(l.customHeaders, function (e, t) {
                        f.setRequestHeader(t, e)
                    }), l.preflight && f.setRequestHeader("X-Requested-With", "XMLHttpRequest"), f.setRequestHeader("Content-Type", l.contentType || "text/plain"), "function" == typeof n && n(h.origin), "POST" === p && d ? f.send(d) : f.send()
                } catch (e) {
    ...
    40: function (e, t, n) {
        "use strict";
        t.a = function (e, t, n) {
            return t && e ? i()(s, t) ? a[e] ? (n && "function" == typeof n && (a[e].loaded ? n() : a[e].callbacks.push(n)), a[e].tag) : (a[e] = {
                loaded: !1,
                tag: null,
                callbacks: []
            }, n && "function" == typeof n && a[e].callbacks.push(n), o.logWarn("module ".concat(t, " is loading external JavaScript")), function (t, n) {
    ...
    41: function (e, t, n) {
                ...
                if (e.bidderCode && (0 < e.cpm || e.dealId)) {
                    ...
                    n = function (e, t, n) {
                        ...
                        if (o) {
                            ...
                            u(r, a, t), e && o[e] && o[e][A.JSON_MAPPING.ADSERVER_TARGETING] && (u(r, o[e], t), t.sendStandardTargeting = o[e].sendStandardTargeting)
                        }
                        return t.native && (r = i({}, r, Object(p.d)(t, n))), r
                    }(e.bidderCode, e, r)
                }
                e.adserverTargeting = i(e.adserverTargeting || {}, n)
            }(t, r), I.emit(A.EVENTS.BID_RESPONSE, t), e.addBidReceived(t), o(e, t)
        }
        ...
        function s(e, t, n) {
            if (e && n) {
                if (e === C.d) {
                    ...
                    if (n["".concat(C.d, "-").concat(r)]) return n["".concat(C.d, "-").concat(r)]
                }
                return n[e]
            }
        }
        ...
        function c(e, t, n) {
            ...
            if (a[A.JSON_MAPPING.BD_SETTING_STANDARD] || (a[A.JSON_MAPPING.BD_SETTING_STANDARD] = {}), a[A.JSON_MAPPING.BD_SETTING_STANDARD][A.JSON_MAPPING.ADSERVER_TARGETING] || (a[A.JSON_MAPPING.BD_SETTING_STANDARD][A.JSON_MAPPING.ADSERVER_TARGETING] = [r(i.BIDDER, "bidderCode"), r(i.AD_ID, "adId"), r(i.PRICE_BUCKET, z(o)), r(i.SIZE, "size"), r(i.DEAL, "dealId"), r(i.SOURCE, "source"), r(i.FORMAT, "mediaType")]), "video" === e) {
        ...
        function u(e, t, n) {
            ...
            return n.size = n.getSize(), x._each(r, function (r) {
                var i = r.key,
                    o = r.val;
                if (e[i] && x.logWarn("The key: " + i + " is getting ovewritten"), x.isFn(o)) try {
                    o = o(n)
                } catch (e) {
        ...
        n.d(t, "b", function () {
            ...
            function n(e, n) {
                if (n && clearTimeout(b), void 0 === h) {
                    var r = [];
                    e && (x.logMessage("Auction ".concat(Y, " timedOut")), i = Z, (r = G.map(function (e) {
                    ...
                    })).length && I.emit(A.EVENTS.BID_TIMEOUT, r)), v = k, h = Date.now(), I.emit(A.EVENTS.AUCTION_END, t()), B(F, function () {
                        try {
                            if (null != K) {
                                var t = W,
                                    n = V.filter(x.bind.call(l.adUnitsFilter, this, t)).reduce(d, {});
                                K.apply(pbjs, [n, e, Y]), K = null
                            }
                        } catch (e) {
            ...
            function o() {
                x.logInfo("Bids Received for Auction with id: ".concat(Y), V), v = k, n(!1, !0)
            }
            ...
            function u(e) {
                function u(e) {
                    ...
                    return e.bidRequests.some(function (e) {
                        ...
                        return N[i] && (!1 === N[i].SRA && (r = Math.min(e.bids.length, n)), D[N[i].origin] + r > n && (t = !1)), !t
                    }), t && e.run(), t
                }
                ...
                var h = {},
                    y = {
                        ...
                        run: function () {
                            ...
                            var C, T;
                            C = n.bind(null, !0), T = setTimeout(C, X), b = T, v = j, I.emit(A.EVENTS.AUCTION_INIT, t());
                            var O, k, _, P, B, q, z = (O = o, k = p, _ = 0, P = !1, B = new Set, q = {}, {
                                addBidResponse: function (e, t) {
                                    ...
                                    var n = function (e) {
                                        ...
                                        u.timeToRespond = u.responseTimestamp - u.requestTimestamp, I.emit(A.EVENTS.BID_ADJUSTMENT, u);
                                        ...
                                        return u.pbLg = v.low, u.pbMg = v.med, u.pbHg = v.high, u.pbAg = v.auto, u.pbDg = v.dense, u.pbCg = v.custom, u
                                    }({
                                        adUnitCode: e,
                                        bid: t,
                                        bidderRequest: this,
                                    ...
                                    "video" === n.mediaType ? function (e, t, n, r) {
                                        ...
                                        m.b.getConfig("cache.url") && c !== w.b && (t.videoCacheKey ? t.vastUrl || (x.logError("videoCacheKey specified but not required vastUrl for video bid"), i = !1) : (i = !1, M(e, t, r, o))), i && (a(e, t), r())
                                    }(k, n, this, y) : (a(k, n), y())
                                },
                                adapterDone: function () {
                                    B.add(this), P = k.getBidRequests().every(function (e) {
                                        return B.has(e)
                                    }), this.bids.forEach(function (e) {
                                        q[e.bidId] || (k.addNoBid(e), I.emit(A.EVENTS.NO_BID, e))
                                    }), P && 0 === _ && O()
                                }
                            });
                            E.callBids(F, e, function () {
                                ...
                                request: function (e, t) {
                                    ...
                                    D[e]--, R[0] && u(R[0]) && R.shift()
                                }
                            }, X, c)
                        }
                    };
                u(y) || (x.logWarn("queueing auction due to limited endpoint capacity"), R.push(y))
            }
            var p, h, v, b, y = e.adUnits,
                C = e.adUnitCodes,
                q = e.callback,
                z = e.cbTimeout,
                L = e.labels,
            ...
            return {
                ...
                callBids: function () {
                    ...
                    x.logInfo("Bids Requested for Auction with id: ".concat(Y), e), e.length < 1 ? (x.logWarn("No valid bid requests returned for auction"), o()) : P.call({
                        dispatch: u,
                        context: this
                    }, e)
                },
                addWinningBid: function (e) {
                    Q = Q.concat(e), E.callBidWonBidder(e.bidder, e, y)
                },
        ...
        I.on(A.EVENTS.BID_ADJUSTMENT, function (e) {
            ! function (e) {
                var t, n = e.bidderCode,
                    r = e.cpm;
                if (pbjs.bidderSettings && (n && pbjs.bidderSettings[n] && "function" == typeof pbjs.bidderSettings[n].bidCpmAdjustment ? t = pbjs.bidderSettings[n].bidCpmAdjustment : pbjs.bidderSettings[A.JSON_MAPPING.BD_SETTING_STANDARD] && "function" == typeof pbjs.bidderSettings[A.JSON_MAPPING.BD_SETTING_STANDARD].bidCpmAdjustment && (t = pbjs.bidderSettings[A.JSON_MAPPING.BD_SETTING_STANDARD].bidCpmAdjustment), t)) try {
                    r = t(e.cpm, i({}, e))
                } catch (e) {
        ...
        var _ = 4,
            ...
            N = {},
            R = [],
            U = Object(b.b)("async", function (e, t) {
    ...
    43: function (e, t, n) {
                    ...
                    ! function () {
                        y.image && s(m.image, function (e) {
                            var t = r(e, 2),
                                n = t[0],
                                i = t[1];
                            u.logMessage("Invoking image pixel user sync for bidder: ".concat(n)), u.triggerPixel(i)
                        })
                    }(),
                        function () {
                            y.iframe && s(m.iframe, function (e) {
                                var t = r(e, 2),
                                    n = t[0],
                                    i = t[1];
                                u.logMessage("Invoking iframe user sync for bidder: ".concat(n)), u.insertUserSyncIframe(i)
                            })
                        }()
                } catch (e) {
        ...
        function c(e, t) {
            ...
            if (function (e, t) {
                if (e.all && e[t]) return void u.logWarn('Detected presence of the "filterSettings.all" and "filterSettings.'.concat(t, '" in userSync config.  You cannot mix "all" with "iframe/image" configs; they are mutually exclusive.'));
                ...
                if (n) {
                    ...
                    return i && "include" !== i && "exclude" !== i ? void u.logWarn('UserSync "filterSettings.'.concat(r, ".filter\" setting '").concat(i, "' is not a valid option; use either 'include' or 'exclude'.")) : "*" === o || Array.isArray(o) && 0 < o.length && o.every(function (e) {
                        return u.isStr(e) && "*" !== e
                    }) || (u.logWarn('Detected an invalid setup in userSync "filterSettings.'.concat(r, ".bidders\"; use either '*' (to represent all bidders) or an array of bidders.")), !1)
                }
            }(n, e)) {
                y[e] = !0;
        ...
        var h, g, m, v, b, y, S, w = Object(p.a)("usersync"),
            C = !u.isSafariBrowser() && w.cookiesAreEnabled(),
            T = (h = {
                ...
                image: !0,
                iframe: !(b = {})
            }, S = h.config, d.b.getConfig("userSync", function (e) {
                if (e.userSync) {
                ...
                return v.has(t) ? u.logMessage('already fired syncs for "'.concat(t, '", ignoring registerSync call')) : S.syncEnabled && u.isArray(m[e]) ? t ? 0 !== S.syncsPerBidder && Number(b[t]) >= S.syncsPerBidder ? u.logWarn('Number of user syncs exceeded for "'.concat(t, '"')) : g.canBidderRegisterSync(e, t) ? (m[e].push([t, n]), (r = b)[i = t] ? r[i] += 1 : r[i] = 1, void (b = r)) : u.logWarn('Bidder "'.concat(t, '" not permitted to register their "').concat(e, '" userSync pixels.')) : u.logWarn("Bidder is required for registering sync") : u.logWarn('User sync type "'.concat(e, '" not supported'));
                var r, i
    ...
    44: function (e, t, n) {
        ...
        function f(e, t, n, r) {
            return Object.keys(t.adserverTargeting).filter(p()).forEach(function (n) {
                ...
                e.length && e.filter((i = n, function (e) {
                    ...
                    A.isArray(e.adserverTargeting[r]) || (e.adserverTargeting[r] = [e.adserverTargeting[r]]), e.adserverTargeting[r] = e.adserverTargeting[r].concat(t.adserverTargeting[r]).filter(y.uniques), delete t.adserverTargeting[r]
                }))
            }), e.push(t), e
        }
        ...
        var m, v, b, y = n(0),
            ...
            O = n(5),
            j = [],
            k = Object.keys(O.TARGETING_KEYS).map(function (e) {
            ...
            N = (m = C.a, b = {}, (v = {}).setLatestAuctionForAdUnit = function (e, t) {
                b[e] = t
                ...
                var n, p, b, C, T, x, E, _, D, N, R = 1 < arguments.length && void 0 !== arguments[1] ? arguments[1] : d(),
                    U = u(e),
                    P = (E = U, _ = R, D = v.getWinningBids(E, _), N = l(), (D = D.map(function (e) {
                        return i({}, e.adUnitCode, Object.keys(e.adserverTargeting).filter(function (t) {
                            ...
                            if (n !== O.TARGETING_KEYS.DEAL) return [].concat(o(t), [a]);
                            var s = i({}, "".concat(n, "_").concat(e.bidderCode).substring(0, 20), r);
                            return [].concat(o(t), [a, s])
                        }, []))
                    })).concat((x = U, R.filter(function (e) {
                        return I()(x, e.adUnitCode)
                    }).map(function (e) {
                        return r({}, e)
                    }).reduce(f, []).map(h).filter(function (e) {
                        return e
                    }))).concat(S.b.getConfig("enableSendAllBids") ? (p = U, b = R, C = k.concat(w.a), T = S.b.getConfig("sendBidsControl.bidLimit"), a(b, y.getHighestCpm, T).map(function (e) {
                        if (c(e, p)) return i({}, e.adUnitCode, g(e, C.filter(function (t) {
                            return void 0 !== e.adserverTargeting[t]
                        })))
                    }).filter(function (e) {
                        return e
                    })) : function (e, t) {
                        if (!0 !== S.b.getConfig("targetingControls.alwaysIncludeDeals")) return [];
                        var n = k.concat(w.a);
                        return a(t, y.getHighestCpm).map(function (t) {
                            if (t.dealId && c(t, e)) return i({}, t.adUnitCode, g(t, n.filter(function (e) {
                                return void 0 !== t.adserverTargeting[e]
                            })))
                        }).filter(function (e) {
                            return e
                        })
                    }(U, R)).concat((n = U, m.getAdUnits().filter(function (e) {
                        return I()(n, e.code) && t(e)
                    }).map(function (e) {
                        return i({}, e.code, (n = t(e), Object.keys(n).map(function (e) {
                ...
                Array.isArray(B) && 0 < B.length && (P = function (e, t) {
                    ...
                    return Object(y.logInfo)("allowTargetingKeys - removed keys [ ".concat(a.join(", "), " ]")), e.filter(function (e) {
                ...
                return L && (Object(y.logInfo)("Detected 'targetingControls.auctionKeyMaxChars' was active for this auction; set with a limit of ".concat(L, " characters.  Running checks on auction keys...")), M = P, q = L, z = Object(y.deepClone)(M), P = Object.keys(z).map(function (e) {
                    return {
                        adUnitCode: e,
                    ...
                    var a = t.adUnitCode,
                        s = o.length;
                    return s <= q ? (q -= s, Object(y.logInfo)("AdUnit '".concat(a, "' auction keys comprised of ").concat(s, " characters.  Deducted from running threshold; new limit is ").concat(q), z[a]), e[a] = z[a]) : Object(y.logWarn)("The following keys for adUnitCode '".concat(a, "' exceeded the current limit of the 'auctionKeyMaxChars' setting.\nThe key-set size was ").concat(s, ", the current allotted amount was ").concat(q, ".\n"), z[a]), n + 1 === r.length && 0 === Object.keys(e).length && Object(y.logError)("No auction targeting keys were permitted due to the setting in setConfig(targetingControls.auctionKeyMaxChars).  Please review setup and consider adjusting."), e
                ...
                window.googletag.pubads().getSlots().forEach(function (n) {
                    Object.keys(e).filter((t || Object(y.isAdUnitCodeMatchingSlot))(n)).forEach(function (t) {
                        return Object.keys(e[t]).forEach(function (r) {
                            var i = e[t][r];
                            "string" == typeof i && (i = i.split(",")), (i = 1 < i.length ? [i] : i).map(function (e) {
                                return A.logMessage("Attempting to set key value for slot: ".concat(n.getSlotElementId(), " key: ").concat(r, " value: ").concat(e)), e
                            }).forEach(function (e) {
                                n.setTargeting(r, e)
                            })
                        })
                ...
                Object.keys(t).forEach(function (e) {
                    return Object.keys(t[e]).forEach(function (n) {
                        if (A.logMessage("Attempting to set targeting for targetId: ".concat(e, " key: ").concat(n, " value: ").concat(t[e][n])), A.isStr(t[e][n]) || A.isArray(t[e][n])) {
                            var r = {};
    ...
    45: function (e, t, n) {
        ...
        function i(e, t, n) {
            ...
            var i, a, c, d, l, f, p, h, g, m = t.buckets.reduce(function (e, t) {
                ...
                v = 0,
                b = s()(t.buckets, function (t) {
                    if (e > m.max * n) {
                        var i = t.precision;
                        void 0 === i && (i = u), r = (t.max * n).toFixed(i)
                    } else {
                        if (e <= t.max * n && v * n <= e) return t.min = v, t;
                        v = t.max
                    }
                });
            return b && (i = e, c = n, d = void 0 !== (a = b).precision ? a.precision : u, l = a.increment * c, f = a.min * c, p = Math.pow(10, d + 2), h = (i * p - f * p) / (l * p), g = Math.floor(h) * l + f, r = (g = Number(g.toFixed(10))).toFixed(d)), r
        }
    ...
    66: function (e, t, n) {
        ...
        e.exports = function (e, t, n, l, h, S, w) {
            ...
            o(n, t, l);
            var T, x, E, I = t + " Iterator",
                A = !1,
                O = e.prototype,
                j = O[v] || O["@@iterator"] || h && O[h],
                k = !m && j || C(h),
                _ = "Array" == t && O.entries || j;
            if (_ && (T = a(_.call(new e)), g !== Object.prototype && T.next && (f || a(T) === g || (s ? s(T, g) : "function" != typeof T[v] && u(T, v, r)), c(T, I, !0, !0), f && (p[I] = r))), h == b && j && j.name !== b && (A = !0, k = function () {
                return j.call(this)
            }), f && !w || O[v] === k || u(O, v, k), p[t] = k, h)
                if (x = {
                    values: C(b),
                    keys: S ? k : C("keys"),
                    entries: C(y)
                }, w)
                    for (E in x) !m && !A && E in O || d(O, E, x[E]);
                else i({
                    target: t,
                    proto: !0,
                    forced: m || A
    ...
    68: function (e, t, n) {
        ...
        function a(e, t) {
            ...
            }) ? n = e : Object(g.isArrayOfNums)(e, 2) && n.push(e)), n
        }
        ...
        function s(e) {
            var t = _.deepClone(e),
                ...
                r = a(n.sizes);
            return 0 < r.length ? (n.sizes = r, t.sizes = r) : (_.logError("Detected a mediaTypes.banner object without a proper sizes field.  Please ensure the sizes are listed like: [[300, 250], ...].  Removing invalid mediaTypes.banner object from request."), delete t.mediaTypes.banner), t
        }
        ...
        function c(e) {
            ...
            if (n.playerSize) {
                var r = "number" == typeof n.playerSize[0] ? 2 : 1,
                    i = a(n.playerSize, r);
                0 < i.length ? (2 == r && _.logInfo("Transforming video.playerSize from [640,480] to [[640,480]] so it's in the proper format."), n.playerSize = i, t.sizes = i) : (_.logError("Detected incorrect configuration of mediaTypes.video.playerSize.  Please specify only one set of dimensions in a format like: [[640, 480]]. Removing invalid mediaTypes.video.playerSize property from request."), delete t.mediaTypes.video.playerSize)
            }
            return t
        }
        ...
        function u(e) {
            ...
            return n.image && n.image.sizes && !Array.isArray(n.image.sizes) && (_.logError("Please use an array of sizes for native.image.sizes field.  Removing invalid mediaTypes.native.image.sizes property from request."), delete t.mediaTypes.native.image.sizes), n.image && n.image.aspect_ratios && !Array.isArray(n.image.aspect_ratios) && (_.logError("Please use an array of sizes for native.image.aspect_ratios field.  Removing invalid mediaTypes.native.image.aspect_ratios property from request."), delete t.mediaTypes.native.image.aspect_ratios), n.icon && n.icon.sizes && !Array.isArray(n.icon.sizes) && (_.logError("Please use an array of sizes for native.icon.sizes field.  Removing invalid mediaTypes.native.icon.sizes property from request."), delete t.mediaTypes.native.icon.sizes), t
        }
        ...
        function l(e) {
            ...
            r && (o.bid = r), i && (o.adId = i), _.logError(n), N.emit(z, o)
        }
        ...
        var h = n(21),
            ...
            D = n(9).default,
            N = n(8),
            R = v.a.triggerUserSyncs,
            U = k.EVENTS,
            P = U.ADD_AD_UNITS,
            B = U.BID_WON,
            M = U.REQUEST_BIDS,
            q = U.SET_TARGETING,
            z = U.AD_RENDER_FAILED,
        ...
        Object(C.a)(), j.bidderSettings = j.bidderSettings || {}, j.libLoaded = !0, j.version = "v4.14.0", _.logInfo("Prebid.js v4.14.0 loaded"), j.adUnits = j.adUnits || [], j.triggerUserSyncs = R;
        var J = {
                ...
                return e.forEach(function (e) {
                    ...
                    if (d && _.isArray(d))
                        if (a && 0 !== Object.keys(a).length) {
                            a.banner && (n = s(e)), a.video && (r = c(n || e)), a.native && (o = u(r || n || e));
                            var l = i({}, n, r, o);
                            t.push(l)
                        } else _.logError("Detected adUnit.code '".concat(e.code, "' did not have a 'mediaTypes' object defined.  This is a required field for the auction, so this adUnit has been removed."));
                    else _.logError("Detected adUnit.code '".concat(e.code, "' did not have 'adUnit.bids' defined or 'adUnit.bids' is not an array. Removing adUnit from auction."))
                }), t
            }, "checkAdUnitSetup");
        j.getAdserverTargetingForAdUnitCodeStr = function (e) {
            ...
            return _.logInfo("Invoking pbjs.getAdserverTargeting", arguments), S.a.getAllTargeting(e)
        }, j.getNoBids = function () {
            return _.logInfo("Invoking pbjs.getNoBids", arguments), d("getNoBids")
        }, j.getBidResponses = function () {
            return _.logInfo("Invoking pbjs.getBidResponses", arguments), d("getBidsReceived")
            ...
            if (_.logInfo("Invoking pbjs.setTargetingForGPTAsync", arguments), Object(g.isGptPubadsDefined)()) {
                var n = S.a.getAllTargeting(e);
                S.a.resetPresetTargeting(e, t), S.a.setTargetingForGPT(n, t), Object.keys(n).forEach(function (e) {
                    Object.keys(n[e]).forEach(function (t) {
                        "hb_adid" === t && y.a.setStatusForBids(n[e][t], k.BID_STATUS.BID_TARGETING_SET)
                    })
                }), N.emit(q, n)
            } else _.logError("window.googletag is not defined on the page")
        }, j.setTargetingForAst = function (e) {
            _.logInfo("Invoking pbjs.setTargetingForAn", arguments), S.a.isApntagDefined() ? (S.a.setTargetingForAst(e), N.emit(q, S.a.getAllTargeting())) : _.logError("window.apntag is not defined on the page")
            ...
            if (_.logInfo("Invoking pbjs.renderAd", arguments), _.logMessage("Calling renderAd with adId :" + t), e && t) try {
                ...
                if (r) {
                    if (r.ad = _.replaceAuctionPrice(r.ad, r.cpm), r.adUrl = _.replaceAuctionPrice(r.adUrl, r.cpm), n && n.clickThrough) {
                        var i = n.clickThrough;
                        r.ad = _.replaceClickThrough(r.ad, i), r.adUrl = _.replaceClickThrough(r.adUrl, i)
                    }
                    y.a.addWinningBid(r), N.emit(B, r);
                    var a = r.height,
                        s = r.width,
                        c = r.ad,
                        u = r.mediaType,
                    ...
                    if (_.insertElement(p, e, "body"), Object(I.c)(f)) Object(I.b)(f, r);
                        ...
                        e.write(c), e.close(), o(e, s, a), _.callBurl(r)
                    } else if (d) {
                        var m = _.createInvisibleIframe();
                        m.height = a, m.width = s, m.style.display = "inline", m.style.overflow = "hidden", m.src = d, _.insertElement(m, e, "body"), o(e, s, a), _.callBurl(r)
                    } else {
            ...
            _.logInfo("Invoking pbjs.removeAdUnit", arguments), e ? (_.isArray(e) ? e : [e]).forEach(function (e) {
                for (var t = j.adUnits.length - 1; 0 <= t; t--) j.adUnits[t].code === e && j.adUnits.splice(t, 1)
            }) : j.adUnits = []
            ...
            if (r = r || j.adUnits, _.logInfo("Invoking pbjs.requestBids", arguments), r = Y(r), i && i.length ? r = r.filter(function (e) {
                return x()(i, e.code)
                ...
                e.transactionId = _.generateUUID(), a.forEach(function (n) {
                    ...
                    t.some(function (e) {
                        return x()(a, e)
                    }) ? E.a.incrementBidderRequestsCounter(e.code, n) : (_.logWarn(_.unsupportedBidderMessage(e, n)), e.bids = e.bids.filter(function (e) {
                        return e.bidder !== n
                    }))
                ...
                var c = y.a.createAuction({
                    ...
                    u = r.length;
                15 < u && _.logInfo("Current auction ".concat(c.getAuctionId(), " contains ").concat(u, " adUnits."), r), i.forEach(function (e) {
        ...
        }), j.requestBids.before(f, 49), j.addAdUnits = function (e) {
            _.logInfo("Invoking pbjs.addAdUnits", arguments), _.isArray(e) ? j.adUnits.push.apply(j.adUnits, e) : "object" === r(e) && j.adUnits.push(e), N.emit(P)
        }, j.onEvent = function (e, t, n) {
            _.logInfo("Invoking pbjs.onEvent", arguments), _.isFn(t) ? !n || V[e].call(null, n) ? N.on(e, t, n) : _.logError('The id provided is not valid for event "' + e + '" and no handler was set.') : _.logError('The event handler provided is not a function and was not set on event "' + e + '".')
        }, j.offEvent = function (e, t, n) {
            _.logInfo("Invoking pbjs.offEvent", arguments), n && !V[e].call(null, n) || N.off(e, t, n)
        }, j.getEvents = function () {
            return _.logInfo("Invoking pbjs.getEvents"), N.getEvents()
            ...
            return _.logInfo("Invoking pbjs.createBid", arguments), Object(A.a)(e)
        };
        var K = [],
            X = Object(w.b)("async", function (e) {
                e && !_.isEmpty(e) ? (_.logInfo("Invoking pbjs.enableAnalytics for: ", e), D.enableAnalytics(e)) : _.logError("pbjs.enableAnalytics should be called with option {}")
            }, "enableAnalyticsCb");
        j.enableAnalytics = function (e) {
            K.push(X.bind(this, e))
        }, j.aliasBidder = function (e, t, n) {
            _.logInfo("Invoking pbjs.aliasBidder", arguments), e && t ? D.aliasBidAdapter(e, t, n) : _.logError("bidderCode and alias must be passed as arguments", "pbjs.aliasBidder")
            ...
            }) : _.logWarn("Improper use of markWinningBidAsUsed. It needs an adUnitCode or an adId to function."), 0 < t.length && (t[0].status = k.BID_STATUS.RENDERED)
        }, j.getConfig = b.b.getConfig, j.setConfig = b.b.setConfig, j.setBidderConfig = b.b.setBidderConfig, j.que.push(function () {
            return Object(m.a)()
        ...
        function r(e) {
            function t(e) {
                ...
                return d(r, i, {
                    ...
                    if (n && n.hasEnforcementHook) t = e(n);
                    else {
                        var r = {
                            hasEnforcementHook: !1,
                        ...
                        if (t && t.valid) {
                            ...
                            if (o.hasDeviceAccess())
                                for (var r = document.cookie.split(";"); r.length;) {
                                    var i = r.pop(),
                                        a = i.indexOf("=");
                                    a = a < 0 ? i.length : a, 0 <= decodeURIComponent(i.slice(0, a).replace(/^\s+/, "")).indexOf(e) && n.push(decodeURIComponent(i.slice(a + 1)))
                                }
                            return n
                        }
                    }
    ...
    8: function (e, t, n) {
        ...
        var i, o, a = n(0),
            ...
            d = a._map(s.EVENTS, function (e) {
                return e
            }),
            l = s.EVENT_ID_PATHS,
            f = [];
        e.exports = (i = {}, (o = {}).on = function (e, t, n) {
            if (o = e, a.contains(d, o)) {
                var r = i[e] || {
                    que: []
                };
                n ? (r[n] = r[n] || {
                    que: []
                }, r[n].que.push(t)) : r.que.push(t), i[e] = r
            } else a.logError("Wrong event name : " + e + " Valid event names :" + d);
            var o
        }, o.emit = function (e) {
            ! function (e, t) {
                ...
                f.push({
                    ...
                    args: n,
                    id: r
                }), r && a.contains(s, r) && u.apply(c, o[r].que), u.apply(c, o.que), a._each(c, function (e) {
                    if (e) try {
                        e.apply(null, t)
                    } catch (e) {
        ...
        var o, a = n(15),
            ...
            m = function () {
                ...
                m = o ? function (e) {
                    ...
                    return e = null, t
                }(o) : ((t = l("iframe")).style.display = "none", d.appendChild(t), t.src = String("javascript:"), (e = t.contentWindow.document).open(), e.write(i("document.F=Object")), e.close(), e.F);
                for (var n = c.length; n--;) delete m[p][c[n]];
                return m()
            };
        u[g] = !0, e.exports = Object.create || function (e, t) {
            var n;
    ...
    85: function (e, t, n) {
        ...
        var r, i, o, a = n(86),
            ...
            l = u("iterator"),
            f = !1;
        [].keys && ("next" in (o = [].keys()) ? (i = a(a(o))) !== Object.prototype && (r = i) : f = !0), null == r && (r = {}), d || c(r, l) || s(r, l, function () {
            return this
        }), e.exports = {
            IteratorPrototype: r,
    ...
    9: function (e, t, n) {
        ...
        function a(e, t, n) {
            try {
                var r = E[e].getSpec();
                r && r[t] && "function" == typeof r[t] && (w.logInfo("Invoking ".concat(e, ".").concat(t)), p.b.runWithBidder(e, c.bind.call(r[t], r, n)))
            } catch (n) {
            ...
            j = Object(h.b)("sync", function (e) {
                ...
                return o.reduce(function (e, o) {
                    var l = Object(u.b)(Object(u.a)(o, a), o.mediaTypes, o.sizes),
                        ...
                        p = l.mediaTypes,
                        h = l.filterResults;
                    return f ? h && w.logInfo('Size mapping filtered adUnit "'.concat(o.code, '" banner sizes from '), h.before, "to ", h.after) : w.logInfo('Size mapping disabled adUnit "'.concat(o.code, '"')), f && e.push(o.bids.filter(function (e) {
                        return e.bidder === t
                    }).reduce(function (e, t) {
                        ...
                        var f = Object(u.b)(Object(u.a)(t, a), p),
                            ...
                            g = f.mediaTypes,
                            m = f.filterResults;
                        return h ? m && w.logInfo('Size mapping filtered adUnit "'.concat(o.code, '" bidder "').concat(t.bidder, '" banner sizes from '), m.before, "to ", m.after) : w.logInfo('Size mapping deactivated adUnit "'.concat(o.code, '" bidder "').concat(t.bidder, '"')), w.isValidMediaTypes(g) ? t = i({}, t, {
                            mediaTypes: g
                        }) : w.logError("mediaTypes is not correctly configured for adunit ".concat(o.code)), h && e.push(i({}, t, {
                            adUnitCode: o.code,
        ...
        x.makeBidRequests = Object(h.b)("sync", function (e, t, n, r, i) {
            ...
            if (A.enabled) {
                ...
                v = u.filter(function (e) {
                    return !m()(I, e) || m()(y, e)
                }), Boolean(o() && A.testServerOnly) && (h = e, Boolean(b()(h, function (e) {
                    return b()(e.bids, function (e) {
                        return (e.bidSource || A.bidderControl && A.bidderControl[e.bidder]) && e.finalSource === s.SERVER
                    })
                }))) && (v.length = 0);
                var O = (d = e, l = A.bidders, (f = w.deepClone(d)).forEach(function (e) {
            ...
            var N, R, U = (N = e, (R = w.deepClone(N)).forEach(function (e) {
                e.bids = e.bids.filter(function (e) {
            ...
            return v.forEach(function (e) {
                var o = w.getUniqueIdentifierStr(),
                    ...
                    c = E[e];
                c || w.logError("Trying to make a request for bidder that does not exist: ".concat(e)), c && s.bids && 0 !== s.bids.length && a.push(s)
        ...
        }, "makeBidRequests"), x.callBids = function (e, t, n, i, o, a, s) {
            if (t.length) {
                ...
                if (l.length) {
                    ...
                    if (v) {
                        ...
                        if (S.ad_units.length) {
                            var x = l.map(function (e) {
                                return e.start = Object(c.timestamp)(), i.bind(e)
                            }),
                    ...
                    } else w.logError("missing " + A.adapter)
                }
                d.forEach(function (e) {
                    e.start = Object(c.timestamp)();
                    var t = E[e.bidderCode];
                    w.logMessage("CALLING BIDDER ======= ".concat(e.bidderCode)), T.emit(C.EVENTS.BID_REQUESTED, e);
        ...
        }, x.videoAdapters = [], x.registerBidAdapter = function (e, t) {
            var n = (2 < arguments.length && void 0 !== arguments[2] ? arguments[2] : {}).supportedMediaTypes,
                r = void 0 === n ? [] : n;
            e && t ? "function" == typeof e.callBids ? (E[t] = e, m()(r, "video") && x.videoAdapters.push(t), m()(r, "native") && d.e.push(t)) : w.logError("Bidder adaptor error for bidder code: " + t + "bidder must implement a callBids() function") : w.logError("bidAdaptor or bidderCode not specified")
            ...
            if (void 0 === E[t]) {
                ...
                if (void 0 === a) {
                    ...
                    var u, f = (r = e, o = [], m()(x.videoAdapters, r) && o.push("video"), m()(d.e, r) && o.push("native"), o);
                    if (a.constructor.prototype != Object.prototype) (u = new a.constructor).setBidderCode(t);
                    else {
                        var h = a.getSpec(),
                            g = n && n.gvlid;
            ...
            } else w.logMessage('alias name "' + t + '" has been already specified.')
        }, x.registerAnalyticsAdapter = function (e) {
            var t = e.adapter,
                n = e.code,
                r = e.gvlid;
            t && n ? "function" == typeof t.enableAnalytics ? (t.code = n, O[n] = {
                adapter: t,
                gvlid: r
            ...
            t = t.map(function (t) {
                return t.params = w.getUserConfiguredParams(e, t.adUnitCode, t.bidder), t.timeout = n, t
            }), t = w.groupBy(t, "bidder"), Object.keys(t).forEach(function (e) {
                a(e, "onTimeout", t[e])
            })
        }, x.callBidWonBidder = function (e, t, n) {
            t.params = w.getUserConfiguredParams(n, t.adUnitCode, t.bidder), y.a.incrementBidderWinsCounter(t.adUnitCode, t.bidder), a(e, "onBidWon", t)
            ...
            return e.reduce(function (e, t) {
                if ("object" === r(t) && "string" == typeof t.mediaQuery) {
                    ...
                    if ("" === t.mediaQuery) n = !0;
                        ...
                        Object(a.logWarn)("Unfriendly iFrame blocks sizeConfig from being correctly evaluated"), n = matchMedia(t.mediaQuery).matches
                    }
                ...
                } else Object(a.logWarn)('sizeConfig rule missing required property "mediaQuery"');
                return e
            }, {
                labels: {},
                sizesSupported: {},
    ...
    260: function (e, t, n) {
        ...
        function u(e) {
            ...
            return Object(h._each)(e, function (e) {
                t[e.bidId] = function (e) {
                    ...
                    return "string" == typeof i && 0 < i.length && (s.i = i), s
                }(e)
            }), t
        }
        ...
        function l(e) {
            ...
            if (null == t || null != t.querySelector("parsererror")) return null;
        ...
        var f = n(1),
            ...
            x = {
                ...
                buildRequests: function (e, t) {
                    var n, r, i, o, a, s = (n = t, Object(h.parseUrl)(Object(h.deepAccess)(n, "refererInfo.canonicalUrl", location.href))),
                        c = Object(h.deepAccess)(e[0], "params.tagId", null),
                        d = Object(h.deepAccess)(e[0], "params.testMode", 0),
                        l = null != e[0] ? e[0] : {
                    ...
                    return {
                        data: {
                            ...
                            tm: d,
                            V: "4.14.0",
                            i: d && null != c ? c : (o = (i = s).hostname.split("."), a = o.slice(o.length - (b.test(i.host) ? 3 : 2)).join("."), btoa(a).replace(/=+$/, "")),
                            l: {},
                            f: .01,
                            cv: "pba1.2",
                            st: "prebid",
                ...
                interpretResponse: function (e, t) {
                    ...
                    }(n.am), a(Object.keys(n.r), function (e) {
                        return a(n.r[e], function (n) {
                            return n.b.map(function (n) {
                                var r, o, a, c = s(n),
                                    u = c === p.b ? (o = n, a = Object(h.deepAccess)(o, "ext.himp", []).concat(null != o.nurl ? [o.nurl] : []).filter(function (e) {
                                ...
                                return i(r = {
                                    ...
                                    currency: "USD",
                                    netRevenue: !0
                                }, c === p.d ? "vastXml" : "ad", u), i(r, "meta", {
                                    advertiserDomains: n.adomain,
                                    mediaType: c
                                }), i(r, "ttl", c === p.d ? 90 : 70), r
                            })
                        }).filter(function (e) {
                            return null != e
                        })
    ...
    272: function (e, t, n) {
        ...
        function u(e) {
            ...
            if (r.sizes = d(e.sizes), r.primary_size = r.sizes[0], r.ad_types = [], r.uuid = e.bidId, e.params.placementId ? r.id = parseInt(e.params.placementId, 10) : r.code = e.params.invCode, r.allow_smaller_sizes = e.params.allowSmallerSizes || !1, r.use_pmt_rule = e.params.usePaymentRule || !1, r.prebid = !0, r.disable_psa = !0, e.params.reserve && (r.reserve = e.params.reserve), e.params.position && (r.position = {
                above: 1,
                below: 2
            }[e.params.position] || 0), e.params.trafficSourceCode && (r.traffic_source_code = e.params.trafficSourceCode), e.params.privateSizes && (r.private_sizes = d(e.params.privateSizes)), e.params.supplyType && (r.supply_type = e.params.supplyType), e.params.pubClick && (r.pubclick = e.params.pubClick), e.params.extInvCode && (r.ext_inv_code = e.params.extInvCode), e.params.publisherId && (r.publisher_id = parseInt(e.params.publisherId, 10)), e.params.externalImpId && (r.external_imp_id = e.params.externalImpId), !w.isEmpty(e.params.keywords)) {
                var o = w.transformBidderParamKeywords(e.params.keywords);
                0 < o.length && o.forEach(s), r.keywords = o
            }
            if ((e.mediaType === x.c || w.deepAccess(e, "mediaTypes.".concat(x.c))) && (r.ad_types.push(x.c), 0 === r.sizes.length && (r.sizes = d([1, 1])), e.nativeParams)) {
                var a = (t = e.nativeParams, n = {}, Object.keys(t).forEach(function (e) {
                    var r = q[e] && q[e].serverName || q[e] || e,
                        o = q[e] && q[e].requiredParams;
                    if (n[r] = i({}, o, t[e]), (r === q.image.serverName || r === q.icon.serverName) && n[r].sizes) {
                        var a = n[r].sizes;
            ...
            r.hb_source = c && "adpod" === u ? 7 : 1, e.mediaType !== x.d && !c || r.ad_types.push(x.d), (e.mediaType === x.d || c && "outstream" !== u) && (r.require_asset_url = !0), e.params.video && (r.video = {}, Object.keys(e.params.video).filter(function (e) {
                return j()(R, e)
            }).forEach(function (t) {
                switch (t) {
                    case "context":
                    case "playback_method":
                        ...
                        n = w.isArray(n) ? n[0] : n, r.video[t] = M[t][n];
                        break;
                    case "frameworks":
                        break;
                    default:
            ...
            }), e.params.video.frameworks && w.isArray(e.params.video.frameworks) && (r.video_frameworks = e.params.video.frameworks)), e.renderer && (r.video = i({}, r.video, {
            ...
            return l && l.mediaTypes && l.mediaTypes.banner && r.ad_types.push(x.b), 0 === r.ad_types.length && delete r.ad_types, r
        }

        function d(e) {
            var t = [],
                n = {};
            if (w.isArray(e) && 2 === e.length && !w.isArray(e[0])) n.width = parseInt(e[0], 10), n.height = parseInt(e[1], 10), t.push(n);
            else if ("object" === r(e))
                for (var i = 0; i < e.length; i++) {
                    var o = e[i];
                    (n = {}).width = parseInt(o[0], 10), n.height = parseInt(o[1], 10), t.push(n)
                }
            return t
        }
        ...
        function v(e) {
            var t = !1,
                n = e.params,
                r = e.params.video;
            return n.frameworks && w.isArray(n.frameworks) && (t = j()(e.params.frameworks, 6)), !t && r && r.frameworks && w.isArray(r.frameworks) && (t = j()(e.params.video.frameworks, 6)), t
        }

        function b(e, t, n) {
            w.isEmpty(e.video) && (e.video = {}), e.video[t] = n
        }

        function y(e) {
            var t, n;
            t = e.adUnitCode, (n = document.getElementById(t).querySelectorAll("div[id^='google_ads']"))[0] && n[0].style.setProperty("display", "none"), e.renderer.push(function () {
        ...
        var S = n(11),
            ...
            R = ["id", "minduration", "maxduration", "skippable", "playback_method", "frameworks", "context", "skipoffset"],
            U = ["age", "externalUid", "segments", "gender", "dnt", "language"],
            P = ["geo", "device_id"],
            ...
            H = {
                ...
                buildRequests: function (e, t) {
                    ...
                    if (k && (_.iab_support = {
                        omidpn: "Appnexus",
                        omidpv: "4.14.0"
                    }), 0 < I && (_.member_id = I), s && (_.device = a), d && (_.app = c), C.b.getConfig("adpod.brandCategoryExclusion") && (_.brand_category_uniqueness = !0), S.enabled && (_.debug = S, w.logInfo("AppNexus Debug Auction Settings:\n\n" + JSON.stringify(S, null, 4))), t && t.gdprConsent && (_.gdpr_consent = {
                        consent_string: t.gdprConsent.consentString,
                        consent_required: t.gdprConsent.gdprApplies
                    }), t && t.uspConsent && (_.us_privacy = t.uspConsent), t && t.refererInfo) {
                        var D = {
                        ...
                        var t = function (e, t) {
                            ...
                            if (i) {
                                ...
                                r.forEach(function (e, t) {
                                    l[t].map(function (t) {
                                        b(t, "minduration", e), b(t, "maxduration", e)
                                    })
                                })
                            } else u.map(function (e) {
                    ...
                    var R = w.deepAccess(e[0], "userId.criteoId");
                    if (R) {
                        var M = [];
                        M.push({
                            provider: "criteo",
                    ...
                    return z && q.push({
                        ...
                        id: z,
                        rti_partner: "TDID"
                    }), q.length && (_.eids = q), n[0].publisher_id && (_.publisher_id = n[0].publisher_id),
                        function (e, t) {
                            var n = [],
                                r = {};
                            if (! function (e) {
                                var t = !0;
                                return e && e.gdprConsent && e.gdprConsent.gdprApplies && 2 === e.gdprConsent.apiVersion && (t = !(!0 !== w.deepAccess(e.gdprConsent, "vendorData.purpose.consents.1"))), t
                            }(t) && (r = {
                                withCredentials: !1
                            }), 15 < e.tags.length) {
                                var i = w.deepClone(e);
                ...
                interpretResponse: function (e, t) {
                    ...
                    if (!e || e.error) {
                        ...
                        return e && e.error && (a += ": ".concat(e.error)), w.logError(a), o
                    }
                    if (e.tags && e.tags.forEach(function (e) {
                        ...
                        if (s && 0 !== s.cpm && j()(n.supportedMediaTypes, s.ad_type)) {
                                ...
                                if (t.advertiser_id && (o.meta = i({}, o.meta, {
                                    ...
                                    ttl: 3600
                                        ...
                                        case x.a:
                                            ...
                                            break;
                                        case k.b:
                                            if (o.adResponse = e, o.adResponse.ad = o.adResponse.ads[0], o.adResponse.ad.video = o.adResponse.ad.rtb.video, o.vastXml = t.rtb.video.content, t.renderer_url) {
                                        ...
                                        null == f ? f = l : w.isStr(f) ? f = [f, l] : f.push(l), o[x.c] = {
                                            title: d.title,
                                            body: d.desc,
                                            body2: d.desc2,
                                            cta: d.ctatext,
                            ...
                            c.mediaType = (a = s.ad_type) === x.d ? x.d : a === x.c ? x.c : x.b, o.push(c)
                        }
                    }), e.debug && e.debug.debug_info) {
                        var s = "AppNexus Debug Auction for Prebid\n\n" + e.debug.debug_info;
                        s = s.replace(/(<td>|<th>)/gm, "\t").replace(/(<\/td>|<\/th>)/gm, "\n").replace(/^<br>/gm, "").replace(/(<br>\n|<br>)/gm, "\n").replace(/<h1>(.*)<\/h1>/gm, "\n\n===== $1 =====\n\n").replace(/<h[2-6]>(.*)<\/h[2-6]>/gm, "\n\n*** $1 ***\n\n").replace(/(<([^>]+)>)/gim, ""), w.logMessage("https://console.appnexus.com/docs/understanding-the-debug-auction"), w.logMessage(s)
                    }
                    return o
                },
                ...
                transformBidParams: function (e, t) {
                    return e = w.convertTypes({
                        ...
                        publisherId: "number"
                    }, e), t && (e.use_pmt_rule = "boolean" == typeof e.usePaymentRule && e.usePaymentRule, e.usePaymentRule && delete e.usePaymentRule, a(e.keywords) && e.keywords.forEach(s), Object.keys(e).forEach(function (t) {
                                ...
                                try {
                                    ...
                                    if (d)
                                        for (var l = d.getElementsByTagName("script"), f = 0; f < l.length && !a; f++) {
                                            var p = l[f];
                                            p.getAttribute("data-src") == r && (p.setAttribute("src", i), p.setAttribute("data-src", ""), p.removeAttribute && p.removeAttribute("data-src"), a = !0)
                                        }
                                } catch (e) {
    ...
    352: function (e, t, n) {
        ...
        function i(e, t) {
            ...
            return h ? (g.logInfo("User consent information already known.  Pulling internally stored information..."), c(null, n)) : y()(Object.keys(O), d) ? (O[d].call(this, o, a, n), void (n.haveExited || (0 === l ? o(void 0, n) : n.timer = setTimeout(function (e) {
                a("CMP workflow exceeded timeout threshold.", e)
            }.bind(null, n), l)))) : (g.logWarn("CMP framework (".concat(d, ") is not a supported framework.  Aborting consentManagement module and resuming auction.")), n.nextFn.apply(n.context, n.args))
        }
        ...
        function o(e, t) {
            ...
            E.definedInConfig && 2 === I ? g.logWarn("'allowAuctionWithoutConsent' ignored for TCF 2") : E.definedInConfig || 1 !== I || g.logInfo("'allowAuctionWithoutConsent' using system default: (".concat(x, ").")), g.isFn(n) ? n(e) ? a("CMP returned unexpected value during lookup process.", t, e) : (clearTimeout(t.timer), s(e), c(null, t)) : a("Unable to derive CMP version to process data.  Consent object does not conform to TCF v1 or v2 specs.", t, e)
        }

        function a(e, t, n) {
            clearTimeout(t.timer), E.value && 1 === I && s(void 0), c(e, t, n)
        }
        ...
        function s(e) {
            ...
            }, e && e.addtlConsent && g.isStr(e.addtlConsent) && (h.addtlConsent = e.addtlConsent)), h.apiVersion = I, v.gdprDataHandler.setConsentData(h)
        }
        ...
        function c(e, t, n) {
            if (!1 === t.haveExited) {
                ...
                var r = t.context,
                    i = t.args,
                    o = t.nextFn;
                e ? E.value && 1 === I ? (g.logWarn(e + " 'allowAuctionWithoutConsent' activated.", n), o.apply(r, i)) : (g.logError(e + " Canceling auction as per consentManagement config.", n), "function" == typeof t.bidsBackHandler ? t.bidsBackHandler() : g.logError("Error executing bidsBackHandler")) : o.apply(r, i)
            }
        }

        function u(e) {
            (e = e && (e.gdpr || e.usp ? e.gdpr : e)) && "object" === r(e) ? (g.isStr(e.cmpApi) ? d = e.cmpApi : (d = C, g.logInfo("consentManagement config did not specify cmp.  Using system default setting (".concat(C, ")."))), g.isNumber(e.timeout) ? l = e.timeout : (l = T, g.logInfo("consentManagement config did not specify timeout.  Using system default setting (".concat(T, ")."))), "boolean" == typeof e.allowAuctionWithoutConsent && (E.value = e.allowAuctionWithoutConsent, E.definedInConfig = !0), f = !0 === e.defaultGdprScope, g.logInfo("consentManagement module has been activated..."), "static" === d && (g.isPlainObject(e.consentData) ? (p = e.consentData, l = 0) : g.logError("consentManagement config with cmpApi: 'static' did not specify consentData. No consents will be available to adapters.")), A || pbjs.requestBids.before(i, 50), A = !0) : g.logWarn("consentManagement config not defined, exiting consent manager")
        }
        Object.defineProperty(t, "__esModule", {
            value: !0
            ...
            h = void 0, d = void 0, I = 0, v.gdprDataHandler.setConsentData(null)
        ...
        var d, l, f, p, h, g = n(0),
            ...
            O = {
                iab: function (e, t, n) {
                    function r(r, i) {
                        g.logInfo("Received a response from CMP", r), i ? !1 !== r.gdprApplies && "tcloaded" !== r.eventStatus && "useractioncomplete" !== r.eventStatus || e(r, n) : t("CMP unable to register callback function.  Please check CMP setup.", n)
                    }
                    ...
                    function o(e, t, n) {
                        ...
                        window[r] = function (e, n, i) {
                            ...
                            1 !== I && (l[d].version = I), s[u] = i, t.postMessage(l, "*")
                        }, window.addEventListener("message", function (e) {
                        ...
                        }, !1), window[r](e, void 0, n)
                    }
                    var a = function () {
                        function t() {
                            r.getConsentData && r.getVendorConsents && (g.logInfo("Received all requested responses from CMP", r), e(r, n))
                        }
                        var r = {};
                        return {
                            consentDataCallback: function (e) {
                                r.getConsentData = e, t()
                            },
                            vendorConsentsCallback: function (e) {
                                r.getVendorConsents = e, t()
                            }
                        }
                    }(),
                        s = {},
                            ...
                            for (var e, t, n = window; !e;) {
                                try {
                                    if ("function" == typeof n.__tcfapi || "function" == typeof n.__cmp) {
                                        t = "function" == typeof n.__tcfapi ? (I = 2, n.__tcfapi) : (I = 1, n.__cmp), e = n;
                                        break
                                    }
                                } catch (e) { }
                                try {
                        ...
                        u = c.cmpFrame,
                        d = c.cmpFunction;
                    return u ? void (g.isFn(d) ? (g.logInfo("Detected CMP API is directly accessible, calling it now..."), 1 === I ? (d("getConsentData", null, a.consentDataCallback), d("getVendorConsents", null, a.vendorConsentsCallback)) : 2 === I && d("addEventListener", I, r)) : 1 === I && window.$sf && window.$sf.ext && "function" == typeof window.$sf.ext.cmp ? (g.logInfo("Detected Prebid.js is encased in a SafeFrame and CMP is registered, calling it now..."), i("getConsentData", a.consentDataCallback), i("getVendorConsents", a.vendorConsentsCallback)) : (g.logInfo("Detected CMP is outside the current iframe where Prebid.js is located, calling it now..."), 1 === I ? (o("getConsentData", u, a.consentDataCallback), o("getVendorConsents", u, a.vendorConsentsCallback)) : 2 === I && o("addEventListener", u, r))) : t("CMP not found.", n)
                },
    ...
    360: function (e, t, n) {
        ...
        function i(e, t) {
            ...
            return y[u] ? (y[u].call(this, o, a, n), void (n.haveExited || (0 === d ? o(void 0, n) : n.timer = setTimeout(function (e) {
                a("USPAPI workflow exceeded timeout threshold.", e)
            }.bind(null, n), d)))) : (p.logWarn("USP framework (".concat(u, ") is not a supported framework. Aborting consentManagement module and resuming auction.")), n.nextFn.apply(n.context, n.args))
        }

        function o(e, t) {
            var n;
            e && e.usPrivacy ? (clearTimeout(t.timer), (n = e) && n.usPrivacy && (f = n.usPrivacy, g.uspDataHandler.setConsentData(f)), s(null, t)) : a("USPAPI returned unexpected value during lookup process.", t, e)
        }

        function a(e, t, n) {
            clearTimeout(t.timer), s(e, t, n)
        }
        ...
        function s(e, t, n) {
            if (!1 === t.haveExited) {
                ...
                var r = t.context,
                    i = t.args,
                    o = t.nextFn;
                e && p.logWarn(e + " Resuming auction without consent data as per consentManagement config.", n), o.apply(r, i)
            }
        }

        function c(e) {
            (e = e && e.usp) && "object" === r(e) ? (p.isStr(e.cmpApi) ? u = e.cmpApi : (u = m, p.logInfo("consentManagement.usp config did not specify cmpApi. Using system default setting (".concat(m, ")."))), p.isNumber(e.timeout) ? d = e.timeout : (d = v, p.logInfo("consentManagement.usp config did not specify timeout. Using system default setting (".concat(v, ")."))), p.logInfo("USPAPI consentManagement module has been activated..."), "static" === u && (p.isPlainObject(e.consentData) && p.isPlainObject(e.consentData.getUSPData) ? (e.consentData.getUSPData.uspString && (l = {
                usPrivacy: e.consentData.getUSPData.uspString
            }), d = 0) : p.logError("consentManagement config with cmpApi: 'static' did not specify consentData. No consents will be available to adapters.")), b || pbjs.requestBids.before(i, 50), b = !0) : p.logWarn("consentManagement.usp config not defined, exiting usp consent manager")
        }
        Object.defineProperty(t, "__esModule", {
            value: !0
            ...
            f = void 0, u = void 0, g.uspDataHandler.setConsentData(null)
            ...
            y = {
                iab: function (e, t, n) {
                    var r = function () {
                        ...
                        return {
                            consentDataCallback: function (i, o) {
                                o && i.uspString && (r.usPrivacy = i.uspString), r.usPrivacy ? e(r, n) : t("Unable to get USP consent string.", n)
                            }
                        }
                    }(),
                        i = {};
                    try {
                        ...
                        if (!o) return t("USP CMP not found.", n);
                        ! function (e, t, n) {
                            function r(e) {
                            ...
                            window.__uspapi = function (e, n, r) {
                                ...
                                i[o] = r, t.postMessage(a, "*")
                            }, window.addEventListener("message", r, !1), window.__uspapi(e, 1, function (e, t) {
                                window.removeEventListener("message", r, !1), n(e, t)
                            })
    ...
    366: function (e, t, n) {
        ...
        var a = n(0),
            ...
            f = {
                ...
                isBidRequestValid: function (e) {
                    if (!e || !e.params) return a.logWarn(l + ": Missing bid parameters"), !1;
                    if (!a.isStr(e.params.site_id)) return a.logWarn(l + ": site_id must be specified as a string"), !1;
                    if (i(e))
                        if (e.params.mimes) {
                            if (!a.isArray(e.params.mimes) || !e.params.mimes.every(function (e) {
                                return a.isStr(e)
                            })) return a.logWarn(l + ": mimes must be an array of strings"), !1
                        } else a.logWarn(l + ": mimes should be specified for videos");
                    return !0
                },
                buildRequests: function (e, t) {
                    var n, s = t && t.refererInfo ? t.refererInfo.referer : "",
                        c = "",
                        u = "",
                        l = null,
                        f = "_pubcid",
                        p = "https://web.hb.ad.cpe.dotomi.com/cvx/client/hb/ortb/25",
                        h = e.map(function (e) {
                            var t = a.getBidIdParameter("bidfloor", e.params);
                            c = a.getBidIdParameter("site_id", e.params) || c, f = a.getBidIdParameter("pubcid_name", e.params) || f, u = e.auctionId;
                            var n = {
                                id: e.bidId,
                                secure: 1,
                                bidfloor: t || 0,
                            ...
                            if (o(e.params.tag_id, n, "tagid"), i(e)) {
                                var s = a.deepAccess(e, "mediaTypes.video") || {},
                                    d = r(s.playerSize || e.sizes),
                                    h = {};
                                d && d[0] && (o(d[0].w, h, "w"), o(d[0].h, h, "h")), o(e.params.position, h, "pos"), o(e.params.mimes || s.mimes, h, "mimes"), o(e.params.maxduration, h, "maxduration"), o(e.params.protocols || s.protocols, h, "protocols"), o(e.params.api || s.api, h, "api"), n.video = h
                            } else {
                                var g = {
                                    format: r((a.deepAccess(e, "mediaTypes.banner") || {}).sizes || e.sizes)
                                };
                                o(e.params.position, g, "pos"), n.banner = g
                            }
                            return e.userId && e.userId.pubcid ? l = e.userId.pubcid : e.crumbs && e.crumbs.pubcid && (l = e.crumbs.pubcid), e.params.white_label_url && (p = e.params.white_label_url), n
                        }),
                        g = {
                            id: u,
                            imp: h,
                        ...
                        m = {};
                    t && (t.gdprConsent && (m.consent = t.gdprConsent.consentString, "boolean" == typeof t.gdprConsent.gdprApplies && a.deepSetValue(g, "regs.ext.gdpr", t.gdprConsent.gdprApplies ? 1 : 0)), t.uspConsent && a.deepSetValue(g, "regs.ext.us_privacy", t.uspConsent)), (l = l || function (e) {
                        var t;
                        try {
                            if (!(t = d.getCookie(e))) {
                    ...
                    return 0 < v.length && (m.eids = v), a.isEmpty(m) || (g.user = {
                        ext: m
                    }), {
                        method: "POST",
                        url: p,
                ...
                interpretResponse: function (e, t) {
                    var n = [],
                        r = {};
                    return e = e.body, t && t.data && t.data.imp && a._each(t.data.imp, function (e) {
                        return r[e.id] = e
                        ...
                        a._each(t.bid, function (t) {
                            ...
                            if (0 < i && t.impid) {
                                ...
                                s.video ? ("<" === o.charAt(0) ? c.vastXml = o : c.vastUrl = o, c.mediaType = "video", c.width = s.video.w, c.height = s.video.h) : (c.ad = o + '<img src="' + a + '" />', c.width = t.w, c.height = t.h), n.push(c)
                            }
                        })
                    }), n
                },
    ...
    379: function (e, t, n) {
        ...
        function o(e, t) {
            e && t && (p.setCookie(e, t, b), p.setDataInLocalStorage(e, t))
        }
        ...
        function a(e, t) {
            var n, i, a, d, l, f, b = function () {
                ...
                return p.setCookie(m, "", v), e
            }(),
                y = r(Object(u.a)().referer),
                S = r(document.location.href, !0),
                w = "undefined" != typeof criteo_pubtag,
                C = (n = y, i = S, a = e.bundle, d = b, l = w, f = t, "https://gum.criteo.com/sid/json?origin=prebid" + "".concat(n ? "&topUrl=" + encodeURIComponent(n) : "") + "".concat(i ? "&domain=" + encodeURIComponent(i) : "") + "".concat(a ? "&bundle=" + encodeURIComponent(a) : "") + "".concat(f ? "&gdprString=" + encodeURIComponent(f) : "") + "".concat(d ? "&cw=1" : "") + "".concat(l ? "&pbt=1" : ""));
            c.b()(C, function (e) {
                var t, n = JSON.parse(e);
                n.bidId ? o(h, n.bidId) : (t = h, p.setCookie(t, "", v), p.removeDataFromLocalStorage(t)), n.acwsUrl ? ("string" == typeof n.acwsUrl ? [n.acwsUrl] : n.acwsUrl).forEach(function (e) {
    ...
    415: function (e, t, n) {
        ...
        function o(e) {
            ...
            (t = {
                name: e.id,
                div: e
            }).div && (a(g + t.name), function () {
                function e(e, t, n) {
                    var r, i = 0;
                    return r = n, e < t || r < 0 || (i = t < 0 ? n / (n - t) : n < e ? 1 : (e - t) / (n - t)), i
                }
                ...
                function n(n) {
                    ...
                    return i = o, e(t().innerHeight, i.top, i.bottom) * (r = o, e(t().innerWidth, r.left, r.right)) > c
                }
                ...
                function i(e) {
                    return function (e) {
                        ...
                        }() && 0 < (s = e).offsetHeight && 0 < s.offsetWidth && (a = e, "hidden" !== t().getComputedStyle(a).visibility) && function e(t) {
                            var n = t.style,
                                r = t.parentNode;
        ...
        function a(e) {
            var t;
            try {
                t = (t = d.getDataFromLocalStorage(e)) ? window.parseInt(t, 10) + 1 : 1, d.setDataInLocalStorage(e, t)
            } catch (e) {
                return !1
            }
            return !0
            ...
            v = {
                ...
                buildRequests: function (e, t) {
                    var n, r, a, s, c, u, h, v, b, y = function (e) {
                        ...
                        w = (a = e, s = y.ml, c = function (e) {
                            var t = {};
                            return e.forEach(function (e) {
                                var n = i(e, !0);
                                t[n] = t[n] ? t[n] : [], t[n].push(e)
                            }), t
                        }(a), (u = {
                            str: "",
                            vs: "",
                            ...
                            return c[e].map(function (e, t) {
                                u.vs += function (e) {
                                    ...
                                    }(e), n += 4 <= t.render ? t.ratio.toString(16) : "F") : n += "F", n
                                }(e);
                                var n = s ? e.adUnitCode.replace(/_|\.|-|\//g, "").replace(/\)\(|\(|\)|:/g, "_").replace(/^_+|_+$/g, "") : i(e, !0) + "_" + t;
                                return u.map[n] = e.bidId, n + ":" + i(e)
                            }).join("+")
                        }).join("+"), u),
                        C = t.refererInfo.referer;
                    if (y.t) n = "https://" + y.isv + "/layers/t_pbjs_2.json", r = {};
                    else {
                        n = "https://" + (y.sv || "ads.us.e-planning.net") + "/hb/1/" + y.ci + "/1/" + (v = C, (b = document.createElement("a")).href = v, b.hostname) + "/ROS";
                        var T = t.refererInfo.referer.reachedTop ? window.top.document.referrer : t.refererInfo.referer;
                        d.hasLocalStorage() && (h = [], e.forEach(function (e) {
                            ...
                            var t = new MutationObserver(function (t, n) {
                                t && Array.isArray(t) && t.forEach(function (t) {
                                    t && t.addedNodes && Array.isArray(t.addedNodes) && t.addedNodes.forEach(function (t) {
                                        ...
                                        0 <= r && (o(t), e.splice(r, 1), e.length || n.disconnect())
                                    })
                                })
                            });
                            ...
                            ncb: "1",
                            vs: w.vs
                        }, S && (r.crs = S), T && (r.fr = T), t && t.gdprConsent && void 0 !== t.gdprConsent.gdprApplies && (r.gdpr = t.gdprConsent.gdprApplies ? "1" : "0", void 0 !== t.gdprConsent.consentString && (r.gdprcs = t.gdprConsent.consentString)), t && t.uspConsent && (r.ccpa = t.uspConsent)
                    }
                    return {
                        method: "GET",
                        url: n,
        ...
        t.a = function (e) {
            ...
            function n(e) {
                ...
                "object" !== i(e) || "object" !== i(e.options) || void 0 === e.options.sampling || Math.random() < parseFloat(e.options.sampling) ? (c.getEvents().forEach(function (e) {
                    ...
                    t.config = "object" === i(e) && e.options || {}, a.enqueue({
                        eventType: l,
                        args: t
                    })
                }), o = n, u._each(o, function (e, t) {
                    c.on(t, e)
                })) : u.logMessage('Analytics adapter for "'.concat(A, '" disabled by sampling')), this._oldEnable = this.enableAnalytics, this.enableAnalytics = function () {
            ...
            return d !== E && !I || function () {
                ...
                track: function (e) {
                    var t = e.eventType,
                        n = e.args;
                    this.getAdapterType() === I && window[A](O, t, n), this.getAdapterType() === E && function (e) {
                        var t = e.eventType,
                            n = e.args,
                            r = e.callback;
    ...
    775: function (e, t, n) {
        ...
        var s = n(6),
            ...
            A = function (e) {
                ...
                } else r = n.location.href;
                return r
            }(function () {
                var e = window;
                try {
                    for (; e.parent.document !== e.document;) {
                        if (!e.parent.document) throw new Error;
                        e = e.parent
                    }
                } catch (e) { }
                return {
            ...
            O = a(Object(s.a)({
                ...
                track: function (e) {
                    var t = e.eventType,
                        n = e.args;
                    try {
                        if (t === x) return void new j(this.sovrnId, n).send();
                        if (n && n.auctionId && I[n.auctionId] && "complete" === I[n.auctionId].status) throw new Error("Event Received after Auction Close Auction Id " + n.auctionId);
                        switch (n && n.auctionId && void 0 === I[n.auctionId] && (I[n.auctionId] = new k(this.sovrnId, n.auctionId)), t) {
                            case w:
        ...
        O.getAuctions = function () {
            return I
        }, O.originEnableAnalytics = O.enableAnalytics, O.enableAnalytics = function (e) {
            var t = "";
            e && e.options && (e.options.sovrnId || e.options.affiliateId) ? (t = e.options.sovrnId || e.options.affiliateId, O.sovrnId = t, e.options.pbaUrl && (E = e.options.pbaUrl), O.originEnableAnalytics(e)) : f.logError("Need Sovrn Id to log auction results. Please contact a Sovrn representative if you do not know your Sovrn Id.")
        ...
        var j = function () {
            function e(t, n) {
                r(this, e), this.body = {}, this.body.prebidVersion = "prebid_prebid_4.14.0", this.body.sovrnId = t, this.body.winningBid = JSON.parse(JSON.stringify(n)), this.body.url = A, this.body.payload = "winner", delete this.body.winningBid.ad
            }
            return o(e, [{
                key: "send",
                value: function () {
                    this.body.ts = f.timestamp(), b(E, null, JSON.stringify(this.body), {
                        contentType: "application/json",
                        method: "POST"
                    })
                }
            ...
            k = function () {
                function e(t, n) {
                    r(this, e), this.auction = {}, this.auction.prebidVersion = "prebid_prebid_4.14.0", this.auction.sovrnId = t, this.auction.auctionId = n, this.auction.payload = "auction", this.auction.timeouts = {
                        buffer: p.b.getConfig("timeoutBuffer"),
                        bidder: p.b.getConfig("bidderTimeout")
                    }, this.auction.priceGranularity = p.b.getConfig("priceGranularity"), this.auction.url = A, this.auction.requests = [], this.auction.unsynced = [], this.dropBidFields = ["auctionId", "ad", "requestId", "bidderCode"], setTimeout(function (e) {
                        delete I[e]
                ...
                return o(e, [{
                    ...
                    value: function (e) {
                        ...
                        delete t.doneCbCallCount, delete t.auctionId, this.auction.requests.push(t)
                    }
                }, {
                    key: "findBid",
                    value: function (e) {
                        ...
                        return n || (e.unmatched = !0, t.bids.push(JSON.parse(JSON.stringify(e)))), n
                    }
                }, {
                    key: "originalBid",
                    value: function (e) {
                        var t = this.findBid(e);
                        t && (a(t, JSON.parse(JSON.stringify(e))), this.dropBidFields.forEach(function (e) {
                            return delete t[e]
                        }))
                    }
                }, {
                    ...
                    value: function (e) {
                        ...
                        n && (n.originalValues = Object.keys(e).reduce(function (r, i) {
                            return JSON.stringify(n[i]) === JSON.stringify(e[i]) || v()(t.dropBidFields, i) || (r[i] = n[i], n[i] = e[i]), r
                        }, {}))
                    }
                }, {
                    key: "send",
                    value: function () {
                        ...
                        }), this.auction.ts = f.timestamp(), b(E, function () {
            ...
            _ = function () {
                function e(t, n, i) {
                    r(this, e), this.error = {}, this.error.payload = "error", this.error.message = t.message, this.error.stack = t.stack, this.error.data = i, this.error.prebidVersion = "prebid_prebid_4.14.0", this.error.sovrnId = n, this.error.url = A, this.error.userAgent = navigator.userAgent
                }
                return o(e, [{
                    key: "send",
                    value: function () {
                        ...
                        }), ErrorEvent.data && this.error.data.ad && delete this.error.data.ad, this.error.ts = f.timestamp(), b(E, null, JSON.stringify(this.error), {
                            contentType: "application/json",
                            method: "POST"
                        })
                    }
        ...
        function r(e, t) {
            ...
            if (n && e) {
                ...
                if (i.isStr(a)) {
                    ...
                    if (r.uids = [s], i.isFn(n.getEidExt)) {
                        var u = n.getEidExt(e);
                        u && (r.ext = u)
                    }
                    return r
                    ...
                    try {
                        ...
                        r._each(e, function (e) {
                            !o && e.userId && (o = Object(a.a)(e.userId)).forEach(function (e) {
                                e.uids && e.uids[0] && ("criteo.com" === e.source && (s = e.uids[0].id), u.push({
                                    source: e.source,
                                    uid: e.uids[0].id
                                }))
                            }), e.schain && (i = i || e.schain), n = n || r.getBidIdParameter("iv", e.params);
                        ...
                        }), t.gdprConsent && (r.deepSetValue(f, "regs.ext.gdpr", +t.gdprConsent.gdprApplies), r.deepSetValue(f, "user.ext.consent", t.gdprConsent.consentString)), t.uspConsent && r.deepSetValue(f, "regs.ext.us_privacy", t.uspConsent), o && (r.deepSetValue(f, "user.ext.eids", o), r.deepSetValue(f, "user.ext.tpid", u), s && r.deepSetValue(f, "user.ext.prebid_criteoid", s));
                        ...
                        if (t && 0 !== t.length) {
                            if (e.iframeEnabled) {
                                var a = t.filter(function (e) {
                                    ...
                                    s = [];
                                n && n.gdprApplies && "string" == typeof n.consentString && s.push(["gdpr_consent", n.consentString]), i && s.push(["us_privacy", i]), a[0] && (s.push(["informer", a[0]]), o.push({
                                    type: "iframe",
    ...
    837: function (e, t, n) {
        ...
        function r(e, t) {
            ...
            try {
                ...
                n.type === M ? (F.setCookie(n.name, i, o, "Lax", r), "number" == typeof n.refreshInSeconds && F.setCookie("".concat(n.name, "_last"), (new Date).toUTCString(), o, "Lax", r)) : n.type === q && (F.setDataInLocalStorage("".concat(n.name, "_exp"), o), F.setDataInLocalStorage(n.name, encodeURIComponent(i)), "number" == typeof n.refreshInSeconds && F.setDataInLocalStorage("".concat(n.name, "_last"), (new Date).toUTCString()))
            } catch (e) {
                j.logError(e)
            }
        }
        ...
        function i(e, t) {
            var n, r = 1 < arguments.length && void 0 !== t ? t : void 0,
                i = r ? "".concat(e.name, "_").concat(r) : e.name;
            try {
                if (e.type === M) n = F.getCookie(i);
                else if (e.type === q) {
        ...
        function o(e) {
            ...
            return e && (t.consentString = e.consentString, t.gdprApplies = e.gdprApplies, t.apiVersion = e.apiVersion), j.cyrb53Hash(JSON.stringify(t))
        }
        ...
        function c(e) {
            if (e && "boolean" == typeof e.gdprApplies && e.gdprApplies) {
                ...
                if (2 === e.apiVersion && !1 === j.deepAccess(e, "vendorData.purpose.consents.1")) return
            }
            return 1
        }
        ...
        function u(e, t) {
            ...
            e.forEach(function (e) {
                e.callback(function (t) {
                    t ? (e.config.storage && r(e, t), e.idObj = e.submodule.decode(t, e.config)) : j.logInfo("".concat(B, ": ").concat(e.submodule.name, " - request id responded with an empty value")), n()
                }), e.callback = void 0
            }), clearTimeout(w)
        }
            ...
            if (void 0 === S && (S = function (e, t) {
                ...
                if (n.length)
                    if (e && 0 < T) {
                        ...
                        j.logInfo("".concat(B, " - auction delayed by ").concat(T, " at most to fetch ids")), w = setTimeout(i, T), u(n, i)
                    } else O.a.on(N.a.EVENTS.AUCTION_END, function e() {
                        O.a.off(N.a.EVENTS.AUCTION_END, e), 0 < C ? setTimeout(function () {
                            u(n)
                        }, C) : u(n)
                    })
            }
        ...
        function f(e, t) {
            l(function () {
                ...
                }(t.adUnits || Object(k.a)().adUnits, S), e.call(this, t)
            })
        }
                ...
                if (i.hasValidated || c(e)) {
                    ...
                    try {
                        for (var h, g = o[Symbol.iterator](); !(l = (h = g.next()).done); l = !0) {
                            var v = h.value;
                            0 < n.length && -1 === n.indexOf(v.submodule.name) || (j.logInfo("".concat(B, " - refreshing ").concat(v.submodule.name)), m(v, e, r, !0), j.isFn(v.callback) && d.push(v))
                        }
                    } catch (e) {
                        f = !0, p = e
                    } finally {
                    ...
                    0 < d.length && u(d), t && t()
                } else j.logWarn("".concat(B, " - gdpr permission not valid for local storage or cookies, exit module"))
            })
        }

        function m(e, t, n, a) {
            if (e.config.storage) {
                var s, c = i(e.config.storage),
                    u = !1;
                if ("number" == typeof e.config.storage.refreshInSeconds) {
                    var d = new Date(i(e.config.storage, "last"));
                    u = d && Date.now() - d.getTime() > 1e3 * e.config.storage.refreshInSeconds
                } !c || u || a || (p = t, null != (f = n) && f !== o(p)) ? s = e.submodule.getId(e.config, t, c) : "function" == typeof e.submodule.extendId && (s = e.submodule.extendId(e.config, c)), j.isPlainObject(s) && (s.id && (r(e, s.id), c = s.id), "function" == typeof s.callback && (e.callback = s.callback)), c && (e.idObj = e.submodule.decode(c, e.config))
            } else if (e.config.value) e.idObj = e.config.value;
            else {
                var l = e.submodule.getId(e.config, t, void 0);
                j.isPlainObject(l) && ("function" == typeof l.callback && (e.callback = l.callback), l.id && (e.idObj = e.submodule.decode(l.id, e.config)))
            }
            var f, p
        }

        function v() {
            var e, t, n = (e = V, t = H, Array.isArray(e) ? e.reduce(function (e, n) {
                return !n || j.isEmptyStr(n.name) || (!n.storage || j.isEmptyStr(n.storage.type) || j.isEmptyStr(n.storage.name) || -1 === t.indexOf(n.storage.type)) && !j.isPlainObject(n.value) && (n.storage || n.value) || e.push(n), e
            }, []) : []);
            if (n.length) {
                ...
                G = r.map(function (e) {
                    ...
                    return null !== e
                }), !W && G.length && (Object(k.a)().requestBids.before(f, 40), j.logInfo("".concat(B, " - usersync config updated for ").concat(G.length, " submodules")), W = !0)
            }
        }
        ...
        function y(e) {
            G = [], W = !(V = []), S = void 0, -1 === (H = [F.localStorageIsEnabled() ? q : null, F.cookiesAreEnabled() ? M : null].filter(function (e) {
                return null !== e
                ...
                var t = e.userSync;
                t && t.userIds && (V = t.userIds, C = j.isNumber(t.syncDelay) ? t.syncDelay : z, T = j.isNumber(t.auctionDelay) ? t.auctionDelay : L, v())
            }), Object(k.a)().getUserIds = p, Object(k.a)().getUserIdsAsEids = h, Object(k.a)().refreshUserIds = g) : j.logInfo("".concat(B, " - opt-out localStorage found, exit module")) : j.logInfo("".concat(B, " - opt-out cookie found, exit module"))
        }
        Object.defineProperty(t, "__esModule", {
            value: !0
            ...
            J = e
        }, t.setStoredValue = r, t.setStoredConsentData = a, t.requestBidsHook = f, n.d(t, "validateGdprEnforcement", function () {
            return Y
        }), t.attachIdSystem = b, t.init = y;
        var S, w, C, T, x = n(10),
            E = n.n(x),
            I = n(3),
            A = n(8),
            ...
            V = [],
            J = [],
            Y = Object(R.b)("sync", function (e, t) {
                return {
                    userIdModules: e,
                    hasValidated: t && t.hasValidated
                }
            }, "validateGdprEnforcement");
        y(I.b), Object(R.c)("userId", b)
    }
}, [836]), pbjs.processQueue();
var PREBID_REFRESHED = !1,
    ...
    conversantIds = [],
    globalTCData = {};
conversantIds["puzzle-nonograms.com"] = "120983", conversantIds["puzzle-loop.com"] = "117113", conversantIds["puzzle-bridges.com"] = "120982", conversantIds["puzzle-light-up.com"] = "120981", conversantIds["puzzle-killer-sudoku.com"] = "120954", conversantIds["puzzle-jigsaw-sudoku.com"] = "120955", conversantIds["puzzle-nurikabe.com"] = "120953", conversantIds["puzzle-shikaku.com"] = "120952";
var oftmediaIds = [];
oftmediaIds["puzzle-shakashaka.com"] = "14084593", oftmediaIds["puzzle-words.com"] = "14241789", oftmediaIds["puzzle-futoshiki.com"] = "14529493", oftmediaIds["puzzle-skyscrapers.com"] = "16187369", oftmediaIds["puzzle-kakurasu.com"] = "16547154", oftmediaIds["puzzle-star-battle.com"] = "17119303", oftmediaIds["puzzle-tapa.com"] = "17432292", oftmediaIds["puzzle-aquarium.com"] = "17492798", oftmediaIds["puzzle-masyu.com"] = "18816819", oftmediaIds["puzzle-shingoki.com"] = "18980430", oftmediaIds["puzzle-heyawake.com"] = "19431444", oftmediaIds["puzzle-hitori.com"] = "20167636", oftmediaIds["puzzle-pipes.com"] = "20543162";
var oftmediaIds_side = [];
oftmediaIds_side["puzzle-shakashaka.com"] = "14084594", oftmediaIds_side["puzzle-words.com"] = "14241806", oftmediaIds_side["puzzle-futoshiki.com"] = "14529542", oftmediaIds_side["puzzle-skyscrapers.com"] = "16187369", oftmediaIds_side["puzzle-kakurasu.com"] = "16547154", oftmediaIds_side["puzzle-star-battle.com"] = "17119303", oftmediaIds_side["puzzle-tapa.com"] = "17432292", oftmediaIds_side["puzzle-aquarium.com"] = "17492798", oftmediaIds_side["puzzle-masyu.com"] = "18816819", oftmediaIds_side["puzzle-shingoki.com"] = "18980430", oftmediaIds_side["puzzle-heyawake.com"] = "19431444", oftmediaIds_side["puzzle-hitori.com"] = "20167636", oftmediaIds_side["puzzle-pipes.com"] = "20543162";
...
googletag.cmd = googletag.cmd || [], googletag.cmd.push(function () {
    ...
    pbjs.addAdUnits(adUnits), pbjs.setConfig({
        userSync: {
            iframeEnabled: !0,
            debug: !0,
            userIds: [{
    ...
    function () {
        ...
        topSlot = googletag.defineSlot("/50413977/desktop_header", [
            [728, 90]
        ], "bannerTop").addService(googletag.pubads()), sideSlot = googletag.defineSlot("/50413977/desktop_header", [
            [160, 600]
        ], "bannerSide").addService(googletag.pubads()), googletag.pubads().enableSingleRequest(), googletag.enableServices()
    });
...
var defaultSettings = {
    ...
    puzzleSettings = {
        "bridges.show-crosshair": !1,
        "bridges.touch-mode": "both",
        "bridges.highlight-change": !1,
        "bridges.mark-error-numbers": !1,
        "bridges.sticky-islands": !1,
        "bridges.click-between": !0,
        "bridges.gray-out-completed": !1,
        "bridges.mark-completed-style-gray": !1,
        "bridges.mark-completed-style-strike": !0
    };
...
var Settings = {
    ...
    store: function (t) {
        MVVM && !t && MVVM.apply();
        try {
            return localStorage.setItem("puzzleSettings", JSON.stringify(this.bag)), !0
        } catch (t) {
            return !1
        }
    },
    get: function (t) {
        if (t in this.bag) return this.bag[t]
    },
    set: function (t, e, s) {
        return this.bag = localStorage.getItem("puzzleSettings") ? $.extend($.extend(!0, {}, defaultSettings), JSON.parse(localStorage.getItem("puzzleSettings"))) : $.extend(!0, {}, defaultSettings), this.bag[t] = e, null === e && delete this.bag[t], t in this.onChange && this.onchange[t](e), window.ga && t.indexOf(".saved-scale") == -1 && t.indexOf(".draw-style") == -1 && "undefined" != typeof defaultSettings[t] && ga("send", "event", "Settings", t, e.toString()), this.store(s)
    },
    remove: function (t) {
        this.set(t, null)
    },
    resetDefault: function () {
        this.bag = defaultSettings, this.store()
    }
};
! function (t) {
    t.fn.hashi = function (e) {
        this.slug = "bridges";
        var s = t.extend({
            gutter: 5,
            puzzleWidth: 15,
        ...
        return this.task = [], this.currentState = {
            cellStatus: [],
            taskStatus: [],
            lastMove: {},
            index: 0
            ...
            bottomBridges: [],
            cells: []
        }, this.currentCell = 0, this.currentDomCell = null, s.minScale || 7 != s.puzzleWidth || (s.minScale = 1), s.minScale || 10 != s.puzzleWidth || (s.minScale = .8), s.minScale || 15 != s.puzzleWidth || (s.minScale = .7), s.minScale || 25 != s.puzzleWidth || (s.minScale = .4), this.getCellsSize = function (t) {
            ...
            if (s.task) {
                this.loaded = !0, this.task = [];
                for (var e = 0, i = 0; i < s.task.length; i++) t.isNumeric(s.task[i]) ? (this.task.push({
                    index: this.task.length,
                    number: s.task[i],
                    row: this.rowFromIndex(e),
                    col: this.colFromIndex(e)
                }), e++) : e += this.decodeChar(s.task[i])
            } else s.task = "", this.loaded = !1;
            ...
            if (this.currentState = {
                ...
                for (var t = 0; t < this.task.length; t++) this.currentState.cellStatus[t] = {
                    row: this.task[t].row,
                    col: this.task[t].col,
                    right: {
                        index: 0,
                    ...
                    bl: this.getClosestIsland(t, "left") === !1 ? -1 : 0,
                    bt: this.getClosestIsland(t, "up") === !1 ? -1 : 0,
                    bb: this.getClosestIsland(t, "down") === !1 ? -1 : 0,
                    br: this.getClosestIsland(t, "right") === !1 ? -1 : 0,
                    total: 0
                }, this.currentState.taskStatus[t] = 0;
            this.solved = !1, this.states = [Util.clone(this.currentState)]
        }, this.updateTotals = function () {
            for (var t = 0; t < this.task.length; t++) this.currentState.cellStatus[t].total = this.currentState.cellStatus[t].right.bridges + this.currentState.cellStatus[t].bottom.bridges, this.currentState.cellStatus[t].bl != -1 && (this.currentState.cellStatus[t].bl = 0), this.currentState.cellStatus[t].bt != -1 && (this.currentState.cellStatus[t].bt = 0), this.currentState.cellStatus[t].bb != -1 && (this.currentState.cellStatus[t].bb = 0), this.currentState.cellStatus[t].br != -1 && (this.currentState.cellStatus[t].br = 0);
            for (var t = 0; t < this.task.length; t++) this.currentState.cellStatus[t].right.bridges && (this.currentState.cellStatus[this.currentState.cellStatus[t].right.index].total += this.currentState.cellStatus[t].right.bridges, this.currentState.cellStatus[this.currentState.cellStatus[t].right.index].bl = this.currentState.cellStatus[t].right.bridges, this.currentState.cellStatus[t].br = this.currentState.cellStatus[t].right.bridges), this.currentState.cellStatus[t].bottom.bridges && (this.currentState.cellStatus[this.currentState.cellStatus[t].bottom.index].total += this.currentState.cellStatus[t].bottom.bridges, this.currentState.cellStatus[this.currentState.cellStatus[t].bottom.index].bt = this.currentState.cellStatus[t].bottom.bridges, this.currentState.cellStatus[t].bb = this.currentState.cellStatus[t].bottom.bridges)
        }, this.applyCurrentMoveToState = function () {
            var t = this.currentMove.cells.length;
            if (2 == t) {
                if (this.currentMove.startPoint.index < this.currentMove.endPoint.index) var e = this.currentMove.startPoint,
                    s = this.currentMove.endPoint;
                else var e = this.currentMove.endPoint,
                    s = this.currentMove.startPoint;
                "left" == this.currentMove.direction || "right" == this.currentMove.direction ? (this.currentState.cellStatus[e.index].right.bridges = this.getNextStatus(this.currentState.cellStatus[e.index].right.bridges), this.currentState.cellStatus[e.index].right.col = s.col, this.currentState.cellStatus[e.index].right.index = s.index) : (this.currentState.cellStatus[e.index].bottom.bridges = this.getNextStatus(this.currentState.cellStatus[e.index].bottom.bridges), this.currentState.cellStatus[e.index].bottom.row = s.row, this.currentState.cellStatus[e.index].bottom.index = s.index), this.updateTotals(), this.currentState.lastMove = Util.clone(this.currentMove), this.storeCurrentState()
            }
        }, this.drawMoveHelpers = function () {
            this.drawing && (t(".task-current").removeClass("task-current"), this.dom.task[this.currentMove.startPoint.index].addClass("task-current"), this.dom.task[this.currentMove.endPoint.index].addClass("task-current"))
        }, this.hideMoveHelpers = function () {
            t(".task-current").removeClass("task-current")
        }, this.drawCurrentMove = function () {
            this.drawMoveHelpers()
        }, this.draw = function () {
            this.boardWidth = this.getBoardWidth(), this.boardHeight = this.getBoardHeight(), this.css({
                width: this.boardWidth,
                height: this.boardHeight
            }), this.loaded ? this.css("visibility", "visible") : this.css("visibility", "hidden"), this.empty();
            var e = document.createDocumentFragment(),
                i = 0,
                r = 0;
            i = s.gutter, r = s.gutter;
            var a = t('<div class="board-back">').css({
                position: "absolute",
                top: i + "px",
                left: r + "px",
            ...
            e.appendChild(a[0]);
            var o = t('<div class="board-grid">').css({
                position: "absolute",
                top: i + "px",
                left: r + "px",
                width: this.getCellsSize(s.puzzleWidth) + "px",
            ...
            e.appendChild(o[0]);
            var n = t('<div class="board-bridges">').css({
                position: "absolute",
                top: i + "px",
                left: r + "px",
                width: this.getCellsSize(s.puzzleWidth) + "px",
            ...
            if (e.appendChild(n[0]), this.task.length)
                for (var h = Config.bridgesSize ? Config.bridgesSize : 3, l = 0; l < this.task.length; l++) {
                    var d = this.getClosestIsland(l, "right");
                    if (d !== !1) {
                        var c = this.task[l].row,
                            u = this.task[l].col,
                            g = s.squareSize * c + s.separatorWidth * (c + 1) + Math.ceil(s.squareSize / 2),
                            f = s.squareSize * u + s.separatorWidth * (u + 1) + Math.ceil(s.squareSize / 2),
                            p = this.task[d].row,
                            S = this.task[d].col,
                            v = s.squareSize * p + s.separatorWidth * (p + 1) + Math.ceil(s.squareSize / 2),
                            b = s.squareSize * S + s.separatorWidth * (S + 1) + Math.ceil(s.squareSize / 2),
                            m = t('<div class="bridges-bridge bridges-bridge-right">').css({
                                position: "absolute",
                                top: g - h - Math.floor(h / 2) + "px",
                                left: f + "px",
                                width: b - f + "px",
                                height: h + "px",
                                "border-width": h + "px 0"
                            });
                        this.dom.rightBridges[l] = m, n[0].appendChild(m[0])
                    }
                    var d = this.getClosestIsland(l, "down");
                    if (d !== !1) {
                        var c = this.task[l].row,
                            u = this.task[l].col,
                            g = s.squareSize * c + s.separatorWidth * (c + 1) + s.squareSize / 2,
                            f = s.squareSize * u + s.separatorWidth * (u + 1) + s.squareSize / 2,
                            p = this.task[d].row,
                            S = this.task[d].col,
                            v = s.squareSize * p + s.separatorWidth * (p + 1) + s.squareSize / 2,
                            b = s.squareSize * S + s.separatorWidth * (S + 1) + s.squareSize / 2,
                            m = t('<div class="bridges-bridge bridges-bridge-down">').css({
                                position: "absolute",
                                top: g + "px",
                                left: f - h - Math.floor(h / 2) + "px",
                                width: h + "px",
                                height: v - g + "px",
                                "border-width": "0 " + h + "px"
                            });
                        this.dom.bottomBridges[l] = m, n[0].appendChild(m[0])
                    }
                }
            var a = t('<div class="board-tasks selectable">').css({
                position: "absolute",
                top: i + "px",
                left: r + "px",
                width: this.getCellsSize(s.puzzleWidth) + "px",
            ...
            if (this.task.length)
                for (var l = 0; l < this.task.length; l++) {
                    var c = this.task[l].row,
                        u = this.task[l].col,
                        k = this.task[l].number,
                        g = s.squareSize * c + s.separatorWidth * (c + 1),
                        f = s.squareSize * u + s.separatorWidth * (u + 1),
                        C = t('<div class="bridges-task-cell selectable" tabindex="1">').css({
                            position: "absolute",
                            top: g + "px",
                            left: f + "px",
                    ...
                    C[0].row = c, C[0].col = u, C[0].top = g, C[0].left = f, C[0].index = l, C.text(k), C.focus(function () {
                        $this.setCurrentCell({
                            index: this.index
                        }, !0)
                    });
                    var w = t('<div class="dots dots-top dots-na"><i class="d1"></i><i class="d2"></i><i class="d3"></i></div>').css({
                        position: "absolute",
                        top: "-2px",
                        left: "7px",
                        width: "9px",
                        ...
                        M = t('<div class="dots dots-left dots-na"><i class="d1"></i><i class="d2"></i><i class="d3"></i></div>').css({
                            position: "absolute",
                            top: "7px",
                            left: "-1px",
                            width: "3px",
                        ...
                        x = t('<div class="dots dots-bottom dots-na"><i class="d1"></i><i class="d2"></i><i class="d3"></i></div>').css({
                            position: "absolute",
                            top: s.squareSize - 3 + "px",
                            left: "7px",
                            width: "9px",
                            height: "3px"
                        }),
                        z = t('<div class="dots dots-right dots-na"><i class="d1"></i><i class="d2"></i><i class="d3"></i></div>').css({
                            position: "absolute",
                            top: "7px",
                            left: s.squareSize - 3 + "px",
                            width: "3px",
                            height: "9px"
                        });
                    C[0].appendChild(w[0]), C[0].appendChild(M[0]), C[0].appendChild(x[0]), C[0].appendChild(z[0]), this.dom.task[l] = C, a[0].appendChild(C[0])
                }
            e.appendChild(a[0]), this.initCrosshairHelper(), this[0].appendChild(e), this.task.length && this.drawCurrentState()
        }, this.getClosestIsland = function (t, e) {
            if ("right" == e)
                for (var s = t + 1; s < this.task.length; s++)
                    if (this.task[t].row == this.task[s].row) return s;
            if ("down" == e)
                for (var s = t + 1; s < this.task.length; s++)
                    if (this.task[t].col == this.task[s].col) return s;
            if ("left" == e)
                for (var s = t - 1; s >= 0; s--)
                    if (this.task[t].row == this.task[s].row) return s;
            if ("up" == e)
                for (var s = t - 1; s >= 0; s--)
                    if (this.task[t].col == this.task[s].col) return s;
            return !1
        }, this.getClosestIslandByCoords = function (t, e, s) {
            if ("right" == s)
                for (var i = 0; i < this.task.length; i++)
                    if (e < this.task[i].col && t == this.task[i].row) return i;
            if ("down" == s)
                for (var i = 0; i < this.task.length; i++)
                    if (t < this.task[i].row && e == this.task[i].col) return i;
            if ("left" == s)
            ...
            if ("up" == s)
                for (var i = this.task.length - 1; i >= 0; i--)
                    if (t > this.task[i].row && e == this.task[i].col) return i;
            return !1
        }, this.drawIslandBridgesRight = function (t, e) {
            "undefined" != typeof this.dom.rightBridges[t] && this.dom.rightBridges[t].removeClass("bridges0 bridges1 bridges2").addClass("bridges" + e)
        }, this.drawIslandBridgesBottom = function (t, e) {
            "undefined" != typeof this.dom.bottomBridges[t] && this.dom.bottomBridges[t].removeClass("bridges0 bridges1 bridges2").addClass("bridges" + e)
        }, this.drawIslandBridges = function (t) {
            "undefined" != typeof this.currentState.cellStatus[t] && (this.drawIslandBridgesRight(t, this.currentState.cellStatus[t].right.bridges), this.drawIslandBridgesBottom(t, this.currentState.cellStatus[t].bottom.bridges), "undefined" != typeof this.currentState.taskStatus[t] && this.dom.task[t].toggleClass("completed", !!this.currentState.taskStatus[t]))
        }, this.updateDotHelpers = function () {
            t(".bridges-task-cell .dots").removeClass("dots-na dots0 dots1 dots2");
            for (var e = 0; e < this.task.length; e++) {
                var s = this.currentState.cellStatus[e];
                this.dom.task[e].find(".dots-top").addClass(s.bt == -1 ? "dots-na" : "dots" + s.bt), this.dom.task[e].find(".dots-left").addClass(s.bl == -1 ? "dots-na" : "dots" + s.bl), this.dom.task[e].find(".dots-bottom").addClass(s.bb == -1 ? "dots-na" : "dots" + s.bb), this.dom.task[e].find(".dots-right").addClass(s.br == -1 ? "dots-na" : "dots" + s.br)
            }
        }, this.drawCurrentState = function () {
            for (var t = 0; t < this.task.length; t++) this.drawIslandBridges(t);
            this.updateDotHelpers(), this.drawHelperHighlightMove(this.currentState.lastMove)
        }, this.onEndMove = function () {
            if (this.currentMove.startPoint.row == this.currentMove.endPoint.row && this.currentMove.startPoint.col == this.currentMove.endPoint.col) {
                var t = (new Date).getTime() - this.dragMonitor.timeStart;
                if (t > Config.panTimeThreshold) return;
                if (Settings.get(this.slug + ".gray-out-completed")) return;
                this.currentState.taskStatus[this.currentMove.startPoint.index] = !this.currentState.taskStatus[this.currentMove.startPoint.index], this.storeCurrentState()
            } else this.setCurrentCell(this.currentMove.startPoint, !0);
            this.drawHelperHighlightMove(this.currentMove), this.drawIslandBridges(this.currentMove.startPoint.index), this.drawIslandBridges(this.currentMove.endPoint.index), this.updateDotHelpers()
        }, this.setCurrentCell = function (t, e) {
            this.currentCell = t.index, this.currentDomCell = this.dom.task[t.index], e || this.currentDomCell.focus()
        }, this.getNumberOfDrawnCells = function () {
            return this.supper.getNumberOfDrawnCells()
        }, this.checkCompletedTask = function () {
            t(".task-err").removeClass("task-err");
            for (var e = 0; e < this.task.length; e++) Settings.get(this.slug + ".gray-out-completed") && (this.currentState.taskStatus[e] = this.currentState.cellStatus[e].total == this.task[e].number, this.dom.task[e].toggleClass("completed", !!this.currentState.taskStatus[e])), Settings.get(this.slug + ".mark-error-numbers") && this.currentState.cellStatus[e].total > this.task[e].number && this.dom.task[e].addClass("task-err")
        }, this.drawHelperHighlightMove = function (e) {
            if (Settings.get(this.slug + ".highlight-change") && (t(".cell-active").removeClass("cell-active"), e.cells))
                for (var s = 0; s < e.cells.length; s++) this.highlightCell(e.cells[s], "cell-active");
            ...
            var t, e = this.boardWidth,
                i = this.boardHeight,
                r = /matrix\((.*)\)/;
            t = (t = this.css("transform").match(r)) ? t[1].split(",") : [0, 0, 0, 0, 0, 0];
            var a = parseFloat(t[0]),
                o = parseFloat(t[4]),
                n = parseFloat(t[5]);
            if (a || (a = 1), Settings.get(this.slug + ".sticky-islands")) {
                var h = (Config.headerHeight - (n + i / 2 * (1 - a) + Config.paddingTop + 1) + Layout.scrollTop()) / a,
                    l = -(o + e / 2 * (1 - a)) / a,
                    d = (a * e / 2 - (Layout.clientWidth() - e / 2 - o) + (Layout.portrait ? 0 : Config.sidebarWidth)) / a + s.separatorWidth + s.squareSize,
                    c = (a * i / 2 - (Layout.clientHeight() - i / 2 - n) + Config.paddingTop - Layout.scrollTop() + (Layout.portrait ? Config.footerHeight : 0) + (Layout.isSubscriber() ? 0 : Config.bannerHeight) - (Layout.hiddenHeader() ? Config.headerHeight : 0)) / a + s.separatorWidth + s.squareSize,
                    u = s.gutter;
                h < s.gutter && (h = s.gutter), l < u && (l = u), d < u && (d = u), d = e - d, c < u && (c = u), c = i - c;
                for (var g = [], f = [], p = 0; p < this.task.length; p++) {
                    var S = this.dom.task[p],
                        v = S[0].left + s.separatorWidth + 2,
                        b = S[0].top + s.separatorWidth + 2,
                        m = 0,
                        k = 0;
                    if (l > v) S.addClass("outside-left"), m = l - v + "px", "undefined" != typeof this.dom.bottomBridges[p] && this.dom.bottomBridges[p].hide(), "undefined" != typeof g[S[0].row] && (g[S[0].row].css("transform", "translate(0, 0)"), g[S[0].row][0].translateX = 0, g[S[0].row][0].translateY = 0), g[S[0].row] = S;
                    else if (S.removeClass("outside-left"), m = 0, "undefined" != typeof this.dom.bottomBridges[p] && this.dom.bottomBridges[p].show(), l > v - s.separatorWidth - s.squareSize && "undefined" != typeof g[S[0].row]) {
                        var C = l - (v - s.separatorWidth - s.squareSize),
                            w = l - g[S[0].row][0].left - s.separatorWidth - 2 - C;
                        g[S[0].row].css("transform", "translate(" + w + "px, 0)"), g[S[0].row][0].translateX = w, g[S[0].row][0].translateY = 0
                    }
                    if (h > b) S.addClass("outside-top"), k = h - b + "px", "undefined" != typeof this.dom.rightBridges[p] && this.dom.rightBridges[p].hide(), "undefined" != typeof f[S[0].col] && (f[S[0].col].css("transform", "translate(0, 0)"), f[S[0].col][0].translateX = 0, f[S[0].col][0].translateY = 0), f[S[0].col] = S;
                    else if (S.removeClass("outside-top"), k = 0, "undefined" != typeof this.dom.rightBridges[p] && this.dom.rightBridges[p].show(), h > b - s.separatorWidth - s.squareSize && "undefined" != typeof f[S[0].col]) {
                        var C = h - (b - s.separatorWidth - s.squareSize),
                            M = h - f[S[0].col][0].top - s.separatorWidth - 2 - C;
                        f[S[0].col].css("transform", "translate(0, " + M + "px)"), f[S[0].col][0].translateX = 0, f[S[0].col][0].translateY = M
                    }
                    S[0].translateX = m, S[0].translateY = k, S.css("transform", "translate(" + m + ", " + k + ")")
                }
                for (var g = [], f = [], p = this.task.length - 1; p >= 0; p--) {
                    var S = this.dom.task[p],
                        v = S[0].left + s.separatorWidth - 2,
                        b = S[0].top + s.separatorWidth - 2,
                        m = 0,
                        k = 0;
                    if (d < v) S.addClass("outside-right"), m = d - v + "px", "undefined" != typeof this.dom.bottomBridges[p] && this.dom.bottomBridges[p].hide(), "undefined" != typeof g[S[0].row] && g[S[0].row].css("transform", "translate(0, 0)"), g[S[0].row] = S;
                    else if (S.removeClass("outside-right"), m = S[0].translateX, "undefined" == typeof this.dom.bottomBridges[p] || S.hasClass("outside-left") || this.dom.bottomBridges[p].show(), d < v + s.separatorWidth + s.squareSize && "undefined" != typeof g[S[0].row]) {
                        var C = d - (v + s.separatorWidth + s.squareSize);
                        g[S[0].row].css("transform", "translate(" + (d - g[S[0].row][0].left - s.separatorWidth + 2 - C) + "px, 0)")
                    }
                    if (c < b) S.addClass("outside-bottom"), k = c - b + "px", "undefined" != typeof this.dom.rightBridges[p] && this.dom.rightBridges[p].hide(), "undefined" != typeof f[S[0].col] && f[S[0].col].css("transform", "translate(0, 0)"), f[S[0].col] = S;
                    else if (S.removeClass("outside-bottom"), k = S[0].translateY, "undefined" == typeof this.dom.rightBridges[p] || S.hasClass("outside-top") || this.dom.rightBridges[p].show(), c < b + s.separatorWidth + s.squareSize && "undefined" != typeof f[S[0].col]) {
                        var C = c - (b + s.separatorWidth + s.squareSize);
                        f[S[0].col].css("transform", "translate(0, " + (c - f[S[0].col][0].top - s.separatorWidth + 2 - C) + "px)")
                    }
                    S.css("transform", "translate(" + m + ", " + k + ")")
                }
                Settings.get(this.slug + ".show-crosshair") && this.highlightCrosshairCell(this.getCrosshairTarget())
            }
        }, this.stateChangedBoard = function () {
            for (var t = 0; t < this.task.length; t++)
                if (this.currentState.cellStatus[t].right.bridges != this.states[this.currentState.index].cellStatus[t].right.bridges || this.currentState.cellStatus[t].bottom.bridges != this.states[this.currentState.index].cellStatus[t].bottom.bridges) return !0;
            return !1
        }, this.stateChangedTask = function () {
            for (var t = 0; t < this.task.length; t++)
                if (this.currentState.taskStatus[t] != this.states[this.currentState.index].taskStatus[t]) return !0;
            return !1
        }, this.loadState = function (t) {
            if (t.active && t.active.length) {
                this.currentState.lastMove.cells = [];
                for (var e = 0; e < t.active.length; e++) this.currentState.lastMove.cells.push({
                    index: t.active[e]
                })
            }
            if (t.board) {
                for (var s = t.board.split(";"), e = 0; e < s.length; e++) {
                    var i = s[e].split(",");
                    this.currentState.cellStatus[e].row = this.task[e].row, this.currentState.cellStatus[e].col = this.task[e].col;
                    var r = this.getClosestIsland(e, "right");
                    this.currentState.cellStatus[e].right.bridges = parseInt(i[0]), this.currentState.cellStatus[e].right.col = r === !1 ? 0 : this.task[r].col, this.currentState.cellStatus[e].right.index = r === !1 ? 0 : r;
                    var r = this.getClosestIsland(e, "down");
                    this.currentState.cellStatus[e].bottom.bridges = parseInt(i[1]), this.currentState.cellStatus[e].bottom.row = r === !1 ? 0 : this.task[r].row, this.currentState.cellStatus[e].bottom.index = r === !1 ? 0 : r
                }
                this.updateTotals()
            }
        }, this.serializeState = function () {
            for (var t = [], e = 0; e < this.task.length; e++) t.push(this.currentState.cellStatus[e].right.bridges + "," + this.currentState.cellStatus[e].bottom.bridges);
            return t.join(";")
        }, this.serializeSolution = function () {
            for (var t = [], e = 0; e < this.task.length; e++) this.currentState.cellStatus[e].right.bridges && t.push(this.currentState.cellStatus[e].col + "," + this.currentState.cellStatus[e].row + "," + this.currentState.cellStatus[e].right.col + "," + this.currentState.cellStatus[e].row + "," + this.currentState.cellStatus[e].right.bridges), this.currentState.cellStatus[e].bottom.bridges && t.push(this.currentState.cellStatus[e].col + "," + this.currentState.cellStatus[e].row + "," + this.currentState.cellStatus[e].col + "," + this.currentState.cellStatus[e].bottom.row + "," + this.currentState.cellStatus[e].bottom.bridges);
            return t.join(";")
        }, this.getMarkedCount = function () {
            for (var t = 0, e = 0; e < this.task.length; e++) t += this.currentState.cellStatus[e].right.bridges, t += this.currentState.cellStatus[e].bottom.bridges;
            return t
        }, this.getNextStatus = function (t) {
            return this.currentMove.invert ? t-- : t++, (t + s.maxBridges + 1) % (s.maxBridges + 1)
        }, this.highlightCell = function (t, e) {
            this.dom.task[t.index].addClass(e)
        }, this.drawHelperNumberOfDrawnCells = function (t, e) { }, this.canConnect = function (t, e) {
            if (t = this.task[t], e = this.task[e], t.row == e.row) {
                if (t.col == e.col) return !1;
                for (var s = Math.min(t.col, e.col), i = Math.max(t.col, e.col), r = 0; r < this.task.length; r++)
                    if (this.task[r].col > s && this.task[r].col < i) {
                        if (this.task[r].row == t.row) return !1;
                        if (this.task[r].row < t.row && this.currentState.cellStatus[r].bottom.bridges > 0 && this.currentState.cellStatus[r].bottom.row > t.row) return !1
                    }
            } else {
                if (t.col != e.col) return !1;
                for (var s = Math.min(t.row, e.row), i = Math.max(t.row, e.row), r = 0; r < this.task.length; r++)
                    if (this.task[r].row > s && this.task[r].row < i) {
                        if (this.task[r].col == t.col) return !1;
                        if (this.task[r].col < t.col && this.currentState.cellStatus[r].right.bridges > 0 && this.currentState.cellStatus[r].right.col > t.col) return !1
                    }
            }
            return !0
        }, this.startMove = function (t, e, s) {
            this.initCurrentMove(), this.drawing = !0, this.currentMove.direction = "", this.currentMove.invert = e, this.currentMove.cellStatus = e, this.currentMove.startPoint = {
                row: t.row,
                col: t.col,
                index: t.index
            }, this.currentMove.endPoint = {
                ...
                col: t.col,
                index: t.index
            }), s && this.drawCurrentMove()
        }, this.cancelMove = function () {
            this.resetCurrentMoveDraw(), this.initCurrentMove(), this.hideMoveHelpers(), t(".task-current").removeClass("task-current")
        }, this.performMove = function (t) {
            if (this.drawing) {
                var e = this.currentMove.startPoint.index,
                    s = this.getClosestIsland(e, t);
                s !== !1 && this.canConnect(e, s) && (this.currentMove.direction = t, this.currentMove.endPoint = {
                    row: this.task[s].row,
                    col: this.task[s].col,
                    index: this.task[s].index
                }, this.currentMove.cells = [this.currentMove.startPoint, this.currentMove.endPoint]), this.drawCurrentMove()
            }
        }, this.getClickBetweenIslands = function (t) {
            t.offsetX -= s.separatorWidth / 2, t.offsetY -= s.separatorWidth / 2;
            var e = Math.floor(t.offsetY / (s.squareSize + s.separatorWidth)),
                i = Math.floor(t.offsetX / (s.squareSize + s.separatorWidth)),
                r = Math.abs(t.offsetX % (s.squareSize + s.separatorWidth) - (s.squareSize + s.separatorWidth) / 2),
                a = Math.abs(t.offsetY % (s.squareSize + s.separatorWidth) - (s.squareSize + s.separatorWidth) / 2),
                o = this.getClosestIslandByCoords(e, i, "up"),
                n = this.getClosestIslandByCoords(e, i, "down"),
                h = this.getClosestIslandByCoords(e, i, "left"),
                l = this.getClosestIslandByCoords(e, i, "right"),
                d = 0;
            return o !== !1 && n !== !1 && this.canConnect(o, n) && h !== !1 && l !== !1 && this.canConnect(h, l) && (this.task[n].row - this.task[o].row < this.task[l].col - this.task[h].col && (d = s.squareSize / 5), this.task[n].row - this.task[o].row > this.task[l].col - this.task[h].col && (d = -s.squareSize / 5)), r - d < a && o !== !1 && n !== !1 && this.canConnect(o, n) ? {
                from: o,
                to: n,
                direction: "down"
            } : h !== !1 && l !== !1 && this.canConnect(h, l) ? {
                from: h,
                to: l,
                direction: "right"
            } : !(o === !1 || n === !1 || !this.canConnect(o, n)) && {
                from: o,
                to: n,
                direction: "down"
            }
        }, this.clickBetween = function (t, e, s) {
            var i;
            return this.canMove = !1, (i = this.getClickBetweenIslands(t)) ? (this.startMove(this.dom.task[i.from][0], e, s), void this.performMove(i.direction)) : void this.cancelMove()
        }, this.handleTouchStart = function (e) {
            var s = !1,
                i = !1;
            if (this.inputMode = "mouse", this.toggleClass("kbd", !1), "touchstart" == e.type) var r = e.originalEvent.changedTouches[0];
            else {
                var r = e.originalEvent;
                s = 3 == e.which || 1 == e.which && (e.ctrlKey || e.metaKey), i = !0
            }
            this.canMove = !0;
            var a = t(document.elementFromPoint(r.clientX, r.clientY));
            a.hasClass("bridges-task-cell") ? (this.panTimeThreshold = 1, this.clientX = r.clientX, this.clientY = r.clientY, this.startMove(a[0], s, i), a.addClass("task-current"), this.setCurrentCell({
                index: a[0].index
            }), i && (this.dragMonitor.decision = "draw")) : Settings.get($this.slug + ".click-between") && a.hasClass("selectable") ? (this.panTimeThreshold = Config.panTimeThreshold, this.clickBetween(this.getOffset(e), s, i), i && (this.dragMonitor.decision = "draw")) : ("draw" != Settings.get($this.slug + ".touch-mode") ? this.dragMonitor.decision = "panzoom" : this.dragMonitor.decision = "none", this.cancelMove()), this.dragMonitor.timeStart = (new Date).getTime()
        }, this.getDirection = function (t, e) {
            return Math.abs(e.clientX - t.clientX) > Math.abs(e.clientY - t.clientY) ? e.clientX > t.clientX ? "right" : "left" : e.clientY > t.clientY ? "down" : "up"
        }, this.eventsTouchMove = function (e) {
            var s = this.getChangedTouch(e),
                i = t(document.elementFromPoint(s.clientX, s.clientY));
            this.drawing && this.canMove && (i.hasClass("bridges-task-cell") && this.currentMove.startPoint.row == i[0].row && this.currentMove.startPoint.col == i[0].col ? (this.currentMove.endPoint = {
                row: this.currentMove.startPoint.row,
                col: this.currentMove.startPoint.col,
                index: this.currentMove.startPoint.index
            }, this.currentMove.cells = [this.currentMove.startPoint], this.drawCurrentMove()) : this.performMove(this.getDirection(this, s)))
        }, this.getCrosshairOffset = function (t) {
            var e = this.getScale(),
                i = this.getTransform(this),
                r = this.boardWidth,
                a = this.boardHeight;
            offsetX = parseFloat(i[4]), offsetY = parseFloat(i[5]);
            var o = -(Config.headerHeight - (offsetY + a / 2 * (1 - e) + Config.paddingTop + 1) + Layout.scrollTop() - (Layout.hiddenHeader() ? 0 : Config.headerHeight)),
                n = offsetX + r / 2 * (1 - e);
            return {
                offsetX: (t.chLeft - n) / e - s.gutter,
                offsetY: (t.chTop - o) / e - s.gutter
            }
        }, this.eventsCrosshairMouseDown = function (t, e) {
            this.canMove = !0, e.hasClass("bridges-task-cell") ? (this.clientX = e.chLeft, this.clientY = e.chTop, this.startMove(e[0], 3 == t.which, !0), this.dragMonitor.decision = "draw") : Settings.get($this.slug + ".click-between") && e.hasClass("selectable") ? (this.clickBetween(this.getCrosshairOffset(e), 3 == t.which, !0), this.dragMonitor.decision = "draw") : this.cancelMove()
        }, this.eventsCrosshairMove = function (t) {
            this.mouseDown && this.drawing && this.canMove && (t.hasClass("bridges-task-cell") && this.currentMove.startPoint.row == t[0].row && this.currentMove.startPoint.col == t[0].col ? (this.currentMove.endPoint = {
                row: this.currentMove.startPoint.row,
                col: this.currentMove.startPoint.col,
                index: this.currentMove.startPoint.index
            }, this.currentMove.cells = [this.currentMove.startPoint], this.drawCurrentMove()) : this.performMove(this.getDirection(this, {
                clientX: t.chLeft,
                clientY: t.chTop
            })))
        }, this.crosshairPan = function () {
            var t = this.getCrosshairTarget();
            if (this.highlightCrosshairCell([]), t.length) {
                if (t.hasClass("bridges-task-cell")) this.highlightCrosshairCell(t);
                else if (Settings.get($this.slug + ".click-between") && t.hasClass("selectable")) {
                    var e;
                    (e = this.getClickBetweenIslands(this.getCrosshairOffset(t))) && this.highlightCrosshairCell(this.dom.task[e.from].add(this.dom.task[e.to]))
                }
                this.eventsCrosshairMove(t)
            }
        }, this.focusLeft = function () {
            for (var t = this.currentCell, e = t - 1; e >= 0; e--) return this.task[t].row == this.task[e].row && (this.setCurrentCell({
                index: e
            }), !0)
        }, this.focusRight = function () {
            for (var t = this.currentCell, e = t + 1; e < this.task.length; e++) return this.task[t].row == this.task[e].row && (this.setCurrentCell({
                index: e
            }), !0)
        }, this.focusUp = function () {
            for (var t = this.currentCell, e = this.task[t].row, s = this.task[t].col, i = e - 1; i >= 0; i--) {
                var r = this.getClosestIslandByCoords(i, s + 1, "left"),
                    a = this.getClosestIslandByCoords(i, s - 1, "right");
                if (r !== !1 || a !== !1) return r === !1 ? (this.setCurrentCell({
                    index: a
                }), !0) : a === !1 ? (this.setCurrentCell({
                    index: r
                }), !0) : (this.task[t].col - this.task[r].col > this.task[a].col - this.task[t].col ? this.setCurrentCell({
                    index: a
                }) : this.setCurrentCell({
                    index: r
                }), !0)
            ...
            for (var t = this.currentCell, e = this.task[t].row, i = this.task[t].col, r = e + 1; r < s.puzzleHeight; r++) {
                var a = this.getClosestIslandByCoords(r, i + 1, "left"),
                    o = this.getClosestIslandByCoords(r, i - 1, "right");
                if (a !== !1 || o !== !1) return a === !1 ? (this.setCurrentCell({
                    index: o
                }), !0) : o === !1 ? (this.setCurrentCell({
                    index: a
                }), !0) : (this.task[t].col - this.task[a].col > this.task[o].col - this.task[t].col ? this.setCurrentCell({
                    index: o
                }) : this.setCurrentCell({
                    index: a
                }), !0)
            ...
            this.super.initEventHandlers(), this.attr("tabindex", 1), this.on("keydown", function (t) {
                if (!$this.mouseDown) {
                    if ($this.keyMove = !1, [37, 38, 39, 40, 65, 68, 83, 87].indexOf(t.keyCode) != -1) {
                        if (t.preventDefault(), "undefined" == typeof $this.currentCell) $this.setCurrentCell({
                            index: 0
                        });
                        else {
                            switch ($this.zoomingPanning = !1, t.keyCode) {
                                case 65:
                                case 37:
                                    t.ctrlKey || t.shiftKey || t.metaKey ? ($this.startMove($this.task[$this.currentCell], t.shiftKey, !0), $this.performMove("left"), $this.endMove()) : $this.focusLeft();
                                    break;
                                case 87:
                                case 38:
                                    t.ctrlKey || t.shiftKey || t.metaKey ? ($this.startMove($this.task[$this.currentCell], t.shiftKey, !0), $this.performMove("up"), $this.endMove()) : $this.focusUp();
                                    break;
                                case 68:
                                case 39:
                                    t.ctrlKey || t.shiftKey || t.metaKey ? ($this.startMove($this.task[$this.currentCell], t.shiftKey, !0), $this.performMove("right"), $this.endMove()) : $this.focusRight();
                                    break;
                                case 83:
                                case 40:
                                    t.ctrlKey || t.shiftKey || t.metaKey ? ($this.startMove($this.task[$this.currentCell], t.shiftKey, !0), $this.performMove("down"), $this.endMove()) : $this.focusDown()
                            }(t.ctrlKey || t.shiftKey || t.metaKey) && ($this.keyMove = !0)
                        }
                        $this.inputMode = "keyboard"
                    } else 90 == t.keyCode && t.ctrlKey ? $this.undo() : 89 == t.keyCode && t.ctrlKey ? $this.redo() : 32 == t.keyCode && "keyboard" == $this.inputMode ? (t.preventDefault(), $this.keyMove = !0, $this.startMove($this.task[$this.currentCell], !1, !0), $this.dragMonitor.timeStart = (new Date).getTime(), $this.zoomingPanning = !1, $this.endMove()) : 27 == t.keyCode ? ($this.inputMode = "mouse", $this.focus()) : $this.cancelMove();
                    $this.toggleClass("kbd", "keyboard" == $this.inputMode)
                }
            }), this.focus()
        }, this.super = new puzzle(s), t.extend(Util.clone(this.super), this)
    }
}(jQuery);